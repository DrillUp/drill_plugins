 ...
 *      通常为白色和透明为主。
 *   (2.rmmv单个图块的像素是48x48。所需素材的大小通常较大，
 ...
 * 
 * 插件指令：>自定义照明 : 高级透视镜变量[21] : 创建 : 样式[3]
 * 插件指令：>自定义照明 : 高级透视镜变量[21] : 清除
 * 插件指令：>自定义照明 : 获取未创建的高级透视镜编号[100-200] : 变量[21]
 * 
 * 插件指令：>自定义照明 : 高级透视镜变量[2] : 设置生命 : 持续时间[180]
 * 插件指令：>自定义照明 : 高级透视镜变量[2] : 暂停生命流逝
 * 插件指令：>自定义照明 : 高级透视镜变量[2] : 继续生命流逝
 * 
 ...
 * @param 平移-透视镜 Y
 * @parent ---贴图---
 * @desc x轴方向平移，单位像素。0表示透视镜中心贴在事件中心。正数向下，负数向上。
 * @default 0
 * 
 * @param 缩放 X
 * @parent ---贴图---
...
//		最坏情况		无
//		备注			特效管理层中，能稳定在9帧的状态（与遮罩管理层一样。）
//						这里同时开了很多高级透视镜，消耗相较比起来有上升，但是不多。
//
//插件记录：
//		★大体框架与功能如下：
//			图片动态遮罩板A：
	...
	if( command === ">图片动态遮罩板A" ){
		...
		if( args.length >= 2 ){
			...
			if( temp1.indexOf("高级透视镜[") != -1 ){
				...
				if( marker == undefined ){
					alert( "【Drill_LayerIllumination.js 地图 - 图片动态遮罩板A】\n" +
...
// ** 资源预加载
//=============================================================================
var _drill_PDMA_temp_preload = Game_Temp.prototype.initialize;
Game_Temp.prototype.initialize = function() {
	_drill_PDMA_temp_preload.call(this);
	...
	for( var i = 0; i < DrillUp.g_PDMA_childData.length; i++ ){
		...
		for( var j = 0; j < temp_data['gif_src'].length; j++ ){
			this._drill_PDMA_preloadTank.push( ImageManager.loadBitmap( temp_data['gif_src_file'], temp_data['gif_src'][j], 0, true ) );
		}
	}
}
...
Game_Map.prototype.drill_PDMA_addSimplePerspect_characterId = function( character_id, style_id ){	
	...
	if( data == undefined ){
		alert("【Drill_PictureDynamicMaskA.js  图片 - 图片动态遮罩板A】\n插件指令错误，不存在id为" + (style_id+1) +"的透视镜样式配置。");
		return;
	}
...
Scene_Base.prototype.drill_PDMA_updatePictureSprite = function(){	
	...
	for(var i=0; i < sprite_list.length; i++ ){
		...
		if( picture._drill_PDMA_maskBind == "图片动态遮罩板A" ){
			...
			temp_sprite.mask = temp_sprite._drill_PDMA_maskSprite;
		}
...
//=============================================================================
		...
		alert(
			"【Drill_PictureDynamicMaskA.js  图片 - 图片动态遮罩板A】\n缺少基础插件，去看看下列插件是不是 未添加 / 被关闭 / 顺序不对："+
