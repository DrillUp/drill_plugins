...
/*:
 * @plugindesc [v1.0]        游戏窗体 - 天窗层的多层动态快照
 * @author Drill_up
 * 
 ...
 *      但不包括天窗层的贴图，以及动态快照自己。
 * 细节：
 *   (1.默认情况下 所有动态快照 都是隐藏的，需要插件指令手动显示。
 ...
 * 插件指令：>天窗层动态快照 : 动态快照[11] : 修改单属性 : 混合模式[0]
...
　　var DrillUp = DrillUp || {}; 
	...
	for (var i = 0; i < DrillUp.g_HDSS_layers_length; i++) {
		if( DrillUp.parameters["动态快照层-" + String(i+1) ] != undefined &&
			...
			DrillUp.g_HDSS_layers[i] = null;		//（强制设为空值，节约存储资源）
		}
	}
...
Game_System.prototype.drill_HDSS_initSysData_Private = function() {
	...
	for(var i = 0; i < DrillUp.g_HDSS_layers.length; i++){
		...
		var temp_controller = new Drill_HDSS_Controller( temp_data );
...
// * 控制器 - 初始化
//==============================
Drill_HDSS_Controller.prototype.initialize = function( data ){
	this._drill_data = {};
...
//##############################
Drill_HDSS_Controller.prototype.drill_controller_resetData = function( data ){
...
Drill_HDSS_Controller.prototype.drill_controller_initData = function(){
	...
	// > 贴图
	if( data['src_img_mask'] == undefined ){ data['src_img_mask'] = "" };							//贴图 - 遮罩资源
...
//			说明：	data对象中的参数【可以缺项】。
//==============================
Drill_HDSS_Controller.prototype.drill_controller_resetData_Private = function( data ){
...
Drill_HDSS_Controller.prototype.drill_controller_updateCheckNaN = function(){
	...
	if( DrillUp.g_HDSS_checkNaN == true ){
		if( isNaN( this._drill_x ) ){
			...
			alert( DrillUp.drill_HDSS_getPluginTip_ParamIsNaN( "_drill_x" ) );
		}
		if( isNaN( this._drill_y ) ){
			DrillUp.g_HDSS_checkNaN = false;
			alert( DrillUp.drill_HDSS_getPluginTip_ParamIsNaN( "_drill_y" ) );
		}
		if( isNaN( this._drill_opacity ) ){
			DrillUp.g_HDSS_checkNaN = false;
			alert( DrillUp.drill_HDSS_getPluginTip_ParamIsNaN( "_drill_opacity" ) );
		}
		if( isNaN( this._drill_scaleX ) ){
			DrillUp.g_HDSS_checkNaN = false;
			alert( DrillUp.drill_HDSS_getPluginTip_ParamIsNaN( "_drill_scaleX" ) );
		}
		if( isNaN( this._drill_scaleY ) ){
			DrillUp.g_HDSS_checkNaN = false;
			alert( DrillUp.drill_HDSS_getPluginTip_ParamIsNaN( "_drill_scaleY" ) );
		}
	}
}
...
// * B基本变化 - 帧刷新 位置
//==============================
Drill_HDSS_Controller.prototype.drill_controller_updateChange_Position = function(){
	var data = this._drill_data;
