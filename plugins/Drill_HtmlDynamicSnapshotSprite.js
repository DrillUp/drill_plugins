...
/*:
 * @plugindesc [v1.4]        游戏窗体 - 天窗层的多层动态快照
 * @author Drill_up
 * 
 ...
 * 插件指令：>天窗层动态快照 : 动态快照[11] : 还原混合模式
 * 插件指令：>天窗层动态快照 : 动态快照[11] : 切换图片层级[10]
 * 插件指令：>天窗层动态快照 : 动态快照[11] : 还原图片层级
 ...
 * 插件指令：>天窗层动态快照 : 动态快照[11] : 切换混合模式(延迟)[0] : 延迟执行时间[20]
 * 插件指令：>天窗层动态快照 : 动态快照[11] : 还原混合模式(延迟) : 延迟执行时间[20]
 * 插件指令：>天窗层动态快照 : 动态快照[11] : 切换图片层级(延迟)[10] : 延迟执行时间[20]
 * 插件指令：>天窗层动态快照 : 动态快照[11] : 还原图片层级(延迟) : 延迟执行时间[20]
 ...
 * 上述的插件指令中，你还可以设置增加延迟：
 * 
 * 插件指令：>天窗层动态快照 : 动态快照[11] : 全部延迟指令增加延迟 : 时间[10]
 * 插件指令：>天窗层动态快照 : 动态快照[11] : 全部延迟指令增加延迟 : 时间[周期播放完的时间]
 * 
 * 1.前半部分（动态快照[11]）和 后半部分（全部延迟指令增加延迟 : 时间[10]）
 ...
 * 插件指令：>天窗层动态快照 : 动态快照[11] : 添加周期-切换混合模式(周期)[0] : 周期时长[90] : 周期内开始时间[0]
 * 插件指令：>天窗层动态快照 : 动态快照[11] : 添加周期-还原混合模式(周期) : 周期时长[90] : 周期内开始时间[0]
 * 插件指令：>天窗层动态快照 : 动态快照[11] : 添加周期-切换图片层级(周期)[10] : 周期时长[90] : 周期内开始时间[0]
 * 插件指令：>天窗层动态快照 : 动态快照[11] : 添加周期-还原图片层级(周期) : 周期时长[90] : 周期内开始时间[0]
 ...
 * 插件指令：>天窗层动态快照 : 动态快照[11] : 暂停周期指令
 * 插件指令：>天窗层动态快照 : 动态快照[11] : 继续周期指令
 * 插件指令：>天窗层动态快照 : 动态快照[11] : 清空周期指令(立刻)
 * 
 ...
 * 上述的插件指令中，可以设置：
 * 
 * 插件指令：>天窗层动态快照 : 动态快照[11] : 暂停周期指令(延迟) : 延迟执行时间[20]
 * 插件指令：>天窗层动态快照 : 动态快照[11] : 继续周期指令(延迟) : 延迟执行时间[20]
 * 插件指令：>天窗层动态快照 : 动态快照[11] : 清空周期指令(延迟) : 延迟执行时间[20]
 * 
 * 1.前半部分（动态快照[11]）和 后半部分（ 暂停周期指令(延迟) : 延迟执行时间[20] ）
 ...
 * 上述的插件指令中，可以设置：
 * 
 * 插件指令：>天窗层动态快照 : 动态快照[11] : 暂停周期指令(周期末尾时)
 * 插件指令：>天窗层动态快照 : 动态快照[11] : 清空周期指令(周期末尾时)
 * 插件指令：>天窗层动态快照 : 动态快照[11] : 执行等待(周期末尾时)
 * 
...
Game_Interpreter.prototype.drill_HDSS_pluginCommand = function( command, args ){
	DrillUp.drill_HDSS_globalPluginCommand( command, args, this );
}
//==============================
// * 插件指令 - 『作用全局的插件指令』
//
...
//==============================
DrillUp.drill_HDSS_globalPluginCommand = function( command, args, gameInterpreter ){
	if( command === ">天窗层动态快照" ){ // >天窗层动态快照 : 动态快照[1] : 显示
		...
		if( args.length == 8 ){
			...
			if( type == "修改单属性" ){
				...
				if( temp1.indexOf("透明度[") != -1 ||
					...
					var num_list = DrillUp.drill_HDSS_getArgNumList(temp1);
					for( var k=0; k < controllers.length; k++ ){
		...
		if( args.length == 6 ){
			...
			if( type == "显示(延迟)" ){
				...
				for( var k=0; k < controllers.length; k++ ){
					controllers[k].drill_controller_setDelayingCommand(
						Drill_HDSS_Controller.METHOD__drill_controller_setVisible, [true], delay_time
					);
				}
			}
			if( type == "隐藏(延迟)" ){
				...
				for( var k=0; k < controllers.length; k++ ){
					controllers[k].drill_controller_setDelayingCommand(
						Drill_HDSS_Controller.METHOD__drill_controller_setVisible, [false], delay_time
					);
				}
			}
			if( type == "暂停(延迟)" ){
				...
				for( var k=0; k < controllers.length; k++ ){
					controllers[k].drill_controller_setDelayingCommand(
						Drill_HDSS_Controller.METHOD__drill_controller_setPause, [true], delay_time
					);
				}
			}
			if( type == "继续(延迟)" ){
				...
				for( var k=0; k < controllers.length; k++ ){
					controllers[k].drill_controller_setDelayingCommand(
						Drill_HDSS_Controller.METHOD__drill_controller_setPause, [false], delay_time
					);
				}
			}
			if( type == "暂停周期指令(延迟)" ){
				...
				for( var k=0; k < controllers.length; k++ ){
					controllers[k].drill_controller_setDelayingCommand(
						Drill_HDSS_Controller.METHOD__drill_controller_periodizeCommand_setPause, [true], delay_time
					);
				}
			}
			if( type == "继续周期指令(延迟)" ){
				...
				for( var k=0; k < controllers.length; k++ ){
					controllers[k].drill_controller_setDelayingCommand(
						Drill_HDSS_Controller.METHOD__drill_controller_periodizeCommand_setPause, [false], delay_time
					);
				}
			}
			if( type == "清空周期指令(延迟)" ){
				...
				for( var k=0; k < controllers.length; k++ ){
					controllers[k].drill_controller_setDelayingCommand(
						Drill_HDSS_Controller.METHOD__drill_controller_periodizeCommand_clear, [], delay_time
					);
				}
			}
			...
			if( type.indexOf("切换混合模式(延迟)[") != -1 ){
				...
				for( var k=0; k < controllers.length; k++ ){
					controllers[k].drill_controller_setDelayingCommand(
						Drill_HDSS_Controller.METHOD__drill_controller_setBlendMode, [Number(type)], delay_time
					);
				}
			}
			if( type.indexOf("切换图片层级(延迟)[") != -1 ){
				...
				for( var k=0; k < controllers.length; k++ ){
					controllers[k].drill_controller_setDelayingCommand(
						Drill_HDSS_Controller.METHOD__drill_controller_setZIndex, [Number(type)], delay_time
					);
				}
			}
			if( type == "还原混合模式(延迟)" ){
				...
				for( var k=0; k < controllers.length; k++ ){
					controllers[k].drill_controller_setDelayingCommand(
						Drill_HDSS_Controller.METHOD__drill_controller_restoreBlendMode, [], delay_time
					);
				}
			}
			if( type == "还原图片层级(延迟)" ){
				...
				for( var k=0; k < controllers.length; k++ ){
					controllers[k].drill_controller_setDelayingCommand(
						Drill_HDSS_Controller.METHOD__drill_controller_restoreZIndex, [], delay_time
					);
				}
			}
			if( type == "还原所有单属性(延迟)" ){
				...
				for( var k=0; k < controllers.length; k++ ){
					controllers[k].drill_controller_setDelayingCommand(
						Drill_HDSS_Controller.METHOD__drill_controller_commandChange_restoreAttr, [], delay_time
					);
				}
			}
			if( type == "移动到(延迟)-延迟归位" ){
				...
				for( var k=0; k < controllers.length; k++ ){
					controllers[k].drill_controller_setDelayingCommand(
						Drill_HDSS_Controller.METHOD__drill_controller_commandChange_restoreMove, [], delay_time
					);
				}
			}
		}
		if( args.length == 10 ){
			...
			if( type == "修改单属性(延迟)" ){
				...
				if( temp1.indexOf("透明度[") != -1 ||
					...
					for( var k=0; k < controllers.length; k++ ){
						controllers[k].drill_controller_setDelayingCommand(
							Drill_HDSS_Controller.METHOD__drill_controller_commandChange_setOpacity, 
				...
				if( temp1.indexOf("旋转[") != -1 ||
					...
					for( var k=0; k < controllers.length; k++ ){
						controllers[k].drill_controller_setDelayingCommand(
							Drill_HDSS_Controller.METHOD__drill_controller_commandChange_setRotate,
							[ "匀速变化", num_list[0], Number(temp2) ], delay_time
						);
					}
				}
				if( temp1.indexOf("转速[") != -1 ||
					...
					for( var k=0; k < controllers.length; k++ ){
						controllers[k].drill_controller_setDelayingCommand(
							Drill_HDSS_Controller.METHOD__drill_controller_commandChange_setRotateSpeed,
							["匀速变化", num_list[0], Number(temp2) ], delay_time
						);
					}
				}
				if( temp1.indexOf("缩放X[") != -1 ||
					...
					for( var k=0; k < controllers.length; k++ ){
						controllers[k].drill_controller_setDelayingCommand(
							Drill_HDSS_Controller.METHOD__drill_controller_commandChange_setScaleX,
							[ "匀速变化", num_list[0], Number(temp2) ], delay_time
						);
					}
				}
				if( temp1.indexOf("缩放Y[") != -1 ||
					...
					for( var k=0; k < controllers.length; k++ ){
						controllers[k].drill_controller_setDelayingCommand(
							Drill_HDSS_Controller.METHOD__drill_controller_commandChange_setScaleY,
							[ "匀速变化", num_list[0], Number(temp2) ], delay_time
						);
					}
				}
				if( temp1.indexOf("斜切X[") != -1 ||
					...
					for( var k=0; k < controllers.length; k++ ){
						controllers[k].drill_controller_setDelayingCommand(
							Drill_HDSS_Controller.METHOD__drill_controller_commandChange_setSkewX,
							[ "匀速变化", num_list[0], Number(temp2) ], delay_time
						);
					}
				}
				if( temp1.indexOf("斜切Y[") != -1 ||
					...
					for( var k=0; k < controllers.length; k++ ){
						controllers[k].drill_controller_setDelayingCommand(
							Drill_HDSS_Controller.METHOD__drill_controller_commandChange_setSkewY,
							[ "匀速变化", num_list[0], Number(temp2) ], delay_time
						);
					}
				}
			...
			if( type == "移动到(延迟)-匀速移动" ){
				...
				if( temp1.indexOf("位置[") != -1 ||
					...
					for( var k=0; k < controllers.length; k++ ){
						controllers[k].drill_controller_setDelayingCommand(
							Drill_HDSS_Controller.METHOD__drill_controller_commandChange_setMove,
		...
		if( args.length == 8 ){
			...
			if( type == "添加周期-显示(周期)" ){
				...
				for( var k=0; k < controllers.length; k++ ){
					controllers[k].drill_controller_setPeriodizeCommand(
						Drill_HDSS_Controller.METHOD__drill_controller_setVisible, [true], time_period, time_start
					);
				}
			}
			if( type == "添加周期-隐藏(周期)" ){
				...
				for( var k=0; k < controllers.length; k++ ){
					controllers[k].drill_controller_setPeriodizeCommand(
						Drill_HDSS_Controller.METHOD__drill_controller_setVisible, [false], time_period, time_start
					);
				}
			}
			if( type == "添加周期-暂停(周期)" ){
				...
				for( var k=0; k < controllers.length; k++ ){
					controllers[k].drill_controller_setPeriodizeCommand(
						Drill_HDSS_Controller.METHOD__drill_controller_setPause, [true], time_period, time_start
					);
				}
			}
			if( type == "添加周期-继续(周期)" ){
				...
				for( var k=0; k < controllers.length; k++ ){
					controllers[k].drill_controller_setPeriodizeCommand(
						Drill_HDSS_Controller.METHOD__drill_controller_setPause, [false], time_period, time_start
					);
				}
			}
			if( type.indexOf("添加周期-切换混合模式(周期)[") != -1 ){
				...
				for( var k=0; k < controllers.length; k++ ){
					controllers[k].drill_controller_setPeriodizeCommand(
						Drill_HDSS_Controller.METHOD__drill_controller_setBlendMode, [Number(type)], time_period, time_start
					);
				}
			}
			if( type.indexOf("添加周期-切换图片层级(周期)[") != -1 ){
				...
				for( var k=0; k < controllers.length; k++ ){
					controllers[k].drill_controller_setPeriodizeCommand(
						Drill_HDSS_Controller.METHOD__drill_controller_setZIndex, [Number(type)], time_period, time_start
					);
				}
			}
			if( type == "添加周期-还原混合模式(周期)" ){
				...
				for( var k=0; k < controllers.length; k++ ){
					controllers[k].drill_controller_setPeriodizeCommand(
						Drill_HDSS_Controller.METHOD__drill_controller_restoreBlendMode, [], time_period, time_start
					);
				}
			}
			if( type == "添加周期-还原图片层级(周期)" ){
				...
				for( var k=0; k < controllers.length; k++ ){
					controllers[k].drill_controller_setPeriodizeCommand(
						Drill_HDSS_Controller.METHOD__drill_controller_restoreZIndex, [], time_period, time_start
					);
				}
			}
			if( type == "添加周期-还原所有单属性(周期)" ){
				...
				for( var k=0; k < controllers.length; k++ ){
					controllers[k].drill_controller_setPeriodizeCommand(
						Drill_HDSS_Controller.METHOD__drill_controller_commandChange_restoreAttr, [], time_period, time_start
					);
				}
			}
			if( type == "添加周期-移动到(周期)-执行归位" ){
				...
				for( var k=0; k < controllers.length; k++ ){
					controllers[k].drill_controller_setPeriodizeCommand(
						Drill_HDSS_Controller.METHOD__drill_controller_commandChange_restoreMove, [], time_period, time_start
					);
				}
			}
		}
...
//==============================
DrillUp.drill_HDSS_getArgNumList = function( arg_str ){
...
//==============================
Drill_HDSS_Controller.METHOD__drill_controller_setVisible                   = 11;
Drill_HDSS_Controller.METHOD__drill_controller_setPause                     = 12;
Drill_HDSS_Controller.METHOD__drill_controller_periodizeCommand_setPause    = 13;
Drill_HDSS_Controller.METHOD__drill_controller_periodizeCommand_clear       = 14;

Drill_HDSS_Controller.METHOD__drill_controller_setBlendMode                 = 21;
Drill_HDSS_Controller.METHOD__drill_controller_setZIndex                    = 23;
Drill_HDSS_Controller.METHOD__drill_controller_restoreBlendMode             = 24;
Drill_HDSS_Controller.METHOD__drill_controller_restoreZIndex                = 26;

Drill_HDSS_Controller.METHOD__drill_controller_commandChange_setOpacity     = 31;
Drill_HDSS_Controller.METHOD__drill_controller_commandChange_setRotate      = 34;
Drill_HDSS_Controller.METHOD__drill_controller_commandChange_setRotateSpeed = 35;

Drill_HDSS_Controller.METHOD__drill_controller_commandChange_setScaleX      = 41;
Drill_HDSS_Controller.METHOD__drill_controller_commandChange_setScaleY      = 42;
Drill_HDSS_Controller.METHOD__drill_controller_commandChange_setSkewX       = 43;
Drill_HDSS_Controller.METHOD__drill_controller_commandChange_setSkewY       = 44;
Drill_HDSS_Controller.METHOD__drill_controller_commandChange_restoreAttr    = 45;

Drill_HDSS_Controller.METHOD__drill_controller_commandChange_setMove        = 51;
Drill_HDSS_Controller.METHOD__drill_controller_commandChange_restoreMove    = 52;
...
//			返回：	> 无
//##############################
Drill_HDSS_Controller.prototype.drill_controller_restoreBlendMode = function(){
...
//			返回：	> 无
//##############################
Drill_HDSS_Controller.prototype.drill_controller_restoreZIndex = function(){
	var data = this.drill_data();
...
// * A主体 - 是否 显示/隐藏（开放函数）
//==============================
Drill_HDSS_Controller.prototype.drill_controller_isVisible = function(){
	return this._drill_visible;
}
//==============================
// * A主体 - 是否 暂停/继续（开放函数）
//==============================
Drill_HDSS_Controller.prototype.drill_controller_isPause = function(){
...
Drill_HDSS_Controller.prototype.drill_controller_updateDelayingCommandImportant = function(){
	...
	for(var i = 0; i < this._drill_curDelayingCommandTank.length; i++ ){
		...
		if( dc_data['left_time'] < 0 ){
			...
			if( method == Drill_HDSS_Controller.METHOD__drill_controller_setPause ){
				this.drill_controller_setPause( paramList[0] );
			}else if( method == Drill_HDSS_Controller.METHOD__drill_controller_periodizeCommand_setPause ){  //暂停周期指令(延迟)
				this.drill_controller_periodizeCommand_setPause( paramList[0] );
			}else if( method == Drill_HDSS_Controller.METHOD__drill_controller_periodizeCommand_clear ){  //清空周期指令(延迟)
...
Drill_HDSS_Controller.prototype.drill_controller_updateDelayingCommand = function(){
	...
	for(var i = 0; i < this._drill_curDelayingCommandTank.length; i++ ){
		...
		if( dc_data['left_time'] < 0 ){
			...
			switch( method ){
				case Drill_HDSS_Controller.METHOD__drill_controller_setVisible:
					this.drill_controller_setVisible( paramList[0] );
					break;
				
				case Drill_HDSS_Controller.METHOD__drill_controller_setBlendMode:
					this.drill_controller_setBlendMode( paramList[0] );
					break;
				case Drill_HDSS_Controller.METHOD__drill_controller_setZIndex:
					this.drill_controller_setZIndex( paramList[0] );
					break;
				case Drill_HDSS_Controller.METHOD__drill_controller_restoreBlendMode:
					this.drill_controller_restoreBlendMode();
					break;
				case Drill_HDSS_Controller.METHOD__drill_controller_restoreZIndex:
					this.drill_controller_restoreZIndex();
					break;
				
				case Drill_HDSS_Controller.METHOD__drill_controller_commandChange_setOpacity:
					this.drill_controller_commandChange_setOpacity( paramList[0], paramList[1], paramList[2] );
					break;
				case Drill_HDSS_Controller.METHOD__drill_controller_commandChange_setRotate:
					this.drill_controller_commandChange_setRotate( paramList[0], paramList[1], paramList[2] );
					break;
				case Drill_HDSS_Controller.METHOD__drill_controller_commandChange_setRotateSpeed:
					this.drill_controller_commandChange_setRotateSpeed( paramList[0], paramList[1], paramList[2] );
					break;
					
				case Drill_HDSS_Controller.METHOD__drill_controller_commandChange_setScaleX:
					this.drill_controller_commandChange_setScaleX( paramList[0], paramList[1], paramList[2] );
					break;
				case Drill_HDSS_Controller.METHOD__drill_controller_commandChange_setScaleY:
					this.drill_controller_commandChange_setScaleY( paramList[0], paramList[1], paramList[2] );
					break;
				case Drill_HDSS_Controller.METHOD__drill_controller_commandChange_setSkewX:
					this.drill_controller_commandChange_setSkewX( paramList[0], paramList[1], paramList[2] );
					break;
				case Drill_HDSS_Controller.METHOD__drill_controller_commandChange_setSkewY:
					this.drill_controller_commandChange_setSkewY( paramList[0], paramList[1], paramList[2] );
					break;
				case Drill_HDSS_Controller.METHOD__drill_controller_commandChange_restoreAttr:
					this.drill_controller_commandChange_restoreAttr();
					break;
				
				case Drill_HDSS_Controller.METHOD__drill_controller_commandChange_setMove:
					this.drill_controller_commandChange_setMove( paramList[0], paramList[1], paramList[2], paramList[3] );
					break;
				case Drill_HDSS_Controller.METHOD__drill_controller_commandChange_restoreMove:
...
Drill_HDSS_Controller.prototype.drill_controller_setDelayingCommand = function( method, paramList, delay_time ){
	...
	switch( method ){
		case Drill_HDSS_Controller.METHOD__drill_controller_setVisible:
		case Drill_HDSS_Controller.METHOD__drill_controller_setPause:
		case Drill_HDSS_Controller.METHOD__drill_controller_periodizeCommand_setPause:
		case Drill_HDSS_Controller.METHOD__drill_controller_periodizeCommand_clear:
...
// * E延迟指令 - 增加延迟（开放函数）
//==============================
Drill_HDSS_Controller.prototype.drill_controller_addDelayingTime = function( time ){
...
//==============================
Drill_HDSS_Controller.prototype.drill_controller_periodizeCommand_getWaitAndClearTime = function(){
...
//==============================
Drill_HDSS_Controller.prototype.drill_controller_periodizeCommand_setPause = function( enabled ){
...
//==============================
Drill_HDSS_Controller.prototype.drill_controller_periodizeCommand_waitAndPause = function(){
...
//==============================
Drill_HDSS_Controller.prototype.drill_controller_periodizeCommand_clear = function(){
	this._drill_curPeriodizeCommandTank = [];
...
//==============================
Drill_HDSS_Controller.prototype.drill_controller_periodizeCommand_waitAndClear = function(){
...
Drill_HDSS_Sprite.prototype.drill_sprite_isOptimizationPassed = function(){
	if( this._drill_controller.drill_controller_isVisible() == false ){	//（不显示时，不工作）
