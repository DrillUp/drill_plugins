...
//##############################
Scene_Battle.prototype.drill_BBa_layerMoveingReference = function( x, y, reference, option ){
	return this.drill_BBa_layerMoveingReference_Private( x, y, reference, option );
}
//=============================================================================
// ** 战斗层级（接口实现）
//=============================================================================
...
// * 战斗层级 - 参照的位移（私有）
//==============================
Scene_Battle.prototype.drill_BBa_layerMoveingReference_Private = function( xx, yy, reference, option ){
	/*
		旧代码：
		// > 数据变化
...
Scene_Battle.prototype.drill_BBa_updateBase = function() {
	...
	for( var i = 0; i < this._drill_BBa_spriteTank.length; i++ ){
		...
		if( temp_s_data['area_index'] == '下层' ||		//（上层和下层 位于_battleField中）
			temp_s_data['area_index'] == '上层' ){
			var pos = this.drill_BBa_layerMoveingReference(xx, yy, "战斗参照 -> 战斗参照", temp_s_data);
			xx = pos['x'];
			yy = pos['y'];
		}else{
			var pos = this.drill_BBa_layerMoveingReference(xx, yy, "战斗参照 -> 镜头参照", temp_s_data);
			xx = pos['x'];
			yy = pos['y'];
		}
