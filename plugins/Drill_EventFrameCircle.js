 ...
 * 
 * 1.如果切换事件页时，"槽[1]"的前一页和后一页样式设置相同，则魔法圈不会变化。
 * 
 * -----------------------------------------------------------------------------
 * ----可选设定
 * 你也可以通过插件指令来进行设置：
...
//=============================================================================
	...
	// * 提示信息 - 报错 - 找不到样式
	//==============================
	DrillUp.drill_EFCi_getPluginTip_StyleNotFind = function( style_id ){
	...
	// * 提示信息 - 报错 - NaN校验值
	//==============================
	DrillUp.drill_EFCi_getPluginTip_ParamIsNaN = function( param_name, check_tank ){
		var text = "【" + DrillUp.g_EFCi_PluginTip_curName + "】\n检测到参数"+param_name+"出现了NaN值，请及时检查你的函数。";
		if( check_tank ){
...
Game_Interpreter.prototype.pluginCommand = function( command, args ){
	...
	this.drill_EFCi_pluginCommand( command, args );
}
//==============================
// * 插件指令 - 指令执行
//==============================
Game_Interpreter.prototype.drill_EFCi_pluginCommand = function( command, args ){
	if( command === ">多层行走图魔法圈" ){
...
Game_CharacterBase.prototype.drill_EFCi_createController = function( slot_id, style_id ){
	...
	// > 旧控制器
	var old_controller = this.drill_EFCi_getController( slot_id );
	...
	var cur_styleData = DrillUp.g_EFCi_style[ style_id ];
	if( cur_styleData == undefined || 
		cur_styleData['inited'] == false ){
		alert( DrillUp.drill_EFCi_getPluginTip_StyleNotFind(cur_styleId) );
		return;
	}
	
	// > 『控制器与贴图的样式-』 - 创建控制器
	var controller = new Drill_EFCi_Controller( cur_styleData );
...
// * 物体绑定 - 获取控制器（开放函数）
//==============================
Game_CharacterBase.prototype.drill_EFCi_getController = function( slot_id ){
	if( this._drill_EFCi_controllerTank == undefined ){ return null; }
	if( this._drill_EFCi_controllerTank[ slot_id ] == undefined ){ return null; }
	return this._drill_EFCi_controllerTank[ slot_id ];
}
//==============================
// * 物体绑定 - 帧刷新
//
