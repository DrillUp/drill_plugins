...
/*:
 * @plugindesc [v1.5]        控件 - 商店节假日的折扣[扩展]
 * @author Drill_up
 * 
 * @Drill_LE_param "折扣-%d"
...
//=============================================================================
	...
	// * 提示信息 - 报错 - 窗口字符底层校验
	//==============================
	DrillUp.drill_XSSD_getPluginTip_NeedUpdate_drawText = function(){
...
//=============================================================================
	...
	for (var i = 0; i < DrillUp.g_XSSD_list_length; i++) {
		if( DrillUp.parameters["折扣-" + String(i+1) ] != undefined &&
			...
			DrillUp.g_XSSD_list[i] = undefined;		//（设为空值，节约静态数据占用容量）
		}
	}
...
Game_Interpreter.prototype.pluginCommand = function( command, args ){
	...
	this.drill_XSSD_pluginCommand( command, args );
}
//==============================
// * 插件指令 - 指令执行
//==============================
Game_Interpreter.prototype.drill_XSSD_pluginCommand = function( command, args ){
	if( command === ">商店节假日的折扣" ){
...
Drill_XSSD_Window.prototype.drill_initAttr = function() {
	...
	// > 私有变量初始化
	this._drill_windowWidth = 0;
	this._drill_windowHeight = 0;
...
Drill_XSSD_Window.prototype.drill_updatePosition = function() {
	...
	if( cal_x + this._drill_windowWidth > Graphics.boxWidth ){	//横向贴边控制
		cal_x = Graphics.boxWidth - this._drill_windowWidth;
	}
	if( cal_y + this._drill_windowHeight > Graphics.boxHeight ){	//纵向贴边控制
		cal_y = Graphics.boxHeight - this._drill_windowHeight;
	}
	this.x = cal_x;
	this.y = cal_y;
}
...
Drill_XSSD_Window.prototype.drill_resetData_Skin = function( data ){
	...
	// > 插件自定义值『窗口皮肤』
	var window_pic_file1 = "img/Menu__shopDiscount/";
...
// * C窗口皮肤 - 帧刷新 - 高宽变化『窗口皮肤』
//==============================
Drill_XSSD_Window.prototype.drill_updateSkin_skinBackground = function() {
...
if( typeof(_drill_COWC_drawText_functionExist) == "undefined" ){
	alert( DrillUp.drill_XSSD_getPluginTip_NeedUpdate_drawText() );
}
