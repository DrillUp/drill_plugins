 ...
 *      下层、上层、图片层、最顶层
 *   (2.战斗层级之间的关系为：
 *      rmmv底图 《 rmmv平铺GIF 《 下层 《 rmmv敌人/角色 《 上层
...
//
//			战斗界面全层级关系：
//				Spriteset： LowerLayer：	底图 < rmmv平铺GIF < 下层 < rmmv敌人 < rmmv角色 < 上层
...
//##############################
Scene_Battle.prototype.drill_BTG_layerMoveingReference = function( x, y, reference, option ){
	return this.drill_BTG_layerMoveingReference_Private( x, y, reference, option );
}
//=============================================================================
// ** 战斗层级（接口实现）
//=============================================================================
...
Spriteset_Battle.prototype.createPictures = function() {
	_drill_BTG_battle_createPictures.call(this);		//rmmv图片 < 图片层 < rmmv对话框
...
Scene_Battle.prototype.createAllWindows = function() {
	_drill_BTG_battle_createAllWindows.call(this);	//rmmv对话框 < 最顶层
...
// * 战斗层级 - 参照的位移（私有）
//==============================
Scene_Battle.prototype.drill_BTG_layerMoveingReference_Private = function( xx, yy, reference, option ){
	...
	if( Imported.Drill_BattleCamera ){
		xx += $gameTemp._drill_cam_pos[0] * x_per;
		yy += $gameTemp._drill_cam_pos[1] * y_per;
	}
...
Scene_Battle.prototype.drill_BTG_updateBase = function() {
	...
	for( var i = 0; i < this._drill_BTG_spriteTank.length; i++ ){
		...
		// > 参照的位移
		if( temp_s_data['area_index'] == '下层' ||		//（上层和下层 位于_battleField中）
			temp_s_data['area_index'] == '上层' ){
			var pos = this.drill_BTG_layerMoveingReference(xx, yy, "战斗参照 -> 战斗参照", temp_s_data);
			xx = pos['x'];
			yy = pos['y'];
		}else{
			var pos = this.drill_BTG_layerMoveingReference(xx, yy, "战斗参照 -> 镜头参照", temp_s_data);
			xx = pos['x'];
			yy = pos['y'];
		}
