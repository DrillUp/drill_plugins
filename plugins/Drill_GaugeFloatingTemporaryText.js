 ...
 * 插件指令：>地图临时漂浮文字 : 临时对象 : 修改样式属性-逐个绘制 : 开启
 * 插件指令：>地图临时漂浮文字 : 临时对象 : 修改样式属性-逐个绘制 : 关闭
 * 
 ...
 * @desc 漂浮文字的内容配置信息。
 * @default {"标签":"==文字（相对于地图）==","--常规--":"","默认内容文本":"\"\"","--层级--":"","UI基准":"相对于地图","地图层级":"上层","地图图片层级":"80","--窗口--":"","布局模式":"黑底背景","布局透明度":"0","资源-自定义窗口皮肤":"Window","资源-自定义背景图片":"","平移-自定义背景图片 X":"0","平移-自定义背景图片 Y":"0","窗口中心锚点":"正中心","窗口内边距":"10","窗口字体大小":"22","窗口附加宽度":"0","窗口附加高度":"0"}
 * 
 * @param 临时漂浮样式-2
 * @parent ---样式组 1至20---
 * @type struct<DrillGFTTStyle>
 * @desc 漂浮文字的内容配置信息。
 * @default {"标签":"==文字（相对于镜头）==","--常规--":"","默认内容文本":"\"\"","--层级--":"","UI基准":"相对于镜头","地图层级":"上层","地图图片层级":"80","--窗口--":"","布局模式":"黑底背景","布局透明度":"0","资源-自定义窗口皮肤":"Window","资源-自定义背景图片":"","平移-自定义背景图片 X":"0","平移-自定义背景图片 Y":"0","窗口中心锚点":"正中心","窗口内边距":"10","窗口字体大小":"22","窗口附加宽度":"0","窗口附加高度":"0"}
 * 
 * @param 临时漂浮样式-3
 * @parent ---样式组 1至20---
 * @type struct<DrillGFTTStyle>
 * @desc 漂浮文字的内容配置信息。
 * @default {"标签":"==带框文字（相对于地图）==","--常规--":"","默认内容文本":"\"\"","--层级--":"","UI基准":"相对于地图","地图层级":"图片层","地图图片层级":"80","--窗口--":"","布局模式":"默认窗口皮肤","布局透明度":"192","资源-自定义窗口皮肤":"Window","资源-自定义背景图片":"","平移-自定义背景图片 X":"0","平移-自定义背景图片 Y":"0","窗口中心锚点":"正中心","窗口内边距":"10","窗口字体大小":"22","窗口附加宽度":"0","窗口附加高度":"0"}
 * 
 * @param 临时漂浮样式-4
 * @parent ---样式组 1至20---
 * @type struct<DrillGFTTStyle>
 * @desc 漂浮文字的内容配置信息。
 * @default {"标签":"==带框文字（相对于镜头）==","--常规--":"","默认内容文本":"\"\"","--层级--":"","UI基准":"相对于镜头","地图层级":"图片层","地图图片层级":"80","--窗口--":"","布局模式":"默认窗口皮肤","布局透明度":"192","资源-自定义窗口皮肤":"Window","资源-自定义背景图片":"","平移-自定义背景图片 X":"0","平移-自定义背景图片 Y":"0","窗口中心锚点":"正中心","窗口内边距":"10","窗口字体大小":"22","窗口附加宽度":"0","窗口附加高度":"0"}
 * 
 * @param 临时漂浮样式-5
 * @parent ---样式组 1至20---
 * @type struct<DrillGFTTStyle>
...
//			2.插件对比思路：
//				将相似插件复制副本，然后副本的文本替换为：地图、Map、GFTT
...
//=============================================================================
	...
	// * 提示信息 - 报错 - 窗口字符底层校验
	//==============================
	DrillUp.drill_GFTT_getPluginTip_NeedUpdate_drawText = function(){
...
Game_Interpreter.prototype.pluginCommand = function( command, args ){
	...
	this.drill_GFTT_pluginCommand( command, args );
}
//==============================
// * 插件指令 - 指令执行
//==============================
Game_Interpreter.prototype.drill_GFTT_pluginCommand = function( command, args ){
	if( command === ">地图临时漂浮文字" ){
			...
			if( unit == "临时对象" ){
				...
				if( type == "修改样式属性-逐个绘制" ){
					if( temp1 == "启用" || temp1 == "开启" || temp1 == "打开" || temp1 == "启动" ){
						$gameTemp.drill_GFTT_setStyle_timingEnabled( true );
					}
					if( temp1 == "关闭" || temp1 == "禁用" ){
						$gameTemp.drill_GFTT_setStyle_timingEnabled( false );
					}
				}
...
// * 临时对象 - 容器初始化
//==============================
var _drill_GFTT_temp_initialize2 = Game_Temp.prototype.initialize;
Game_Temp.prototype.initialize = function() {
	_drill_GFTT_temp_initialize2.call(this);
...
// * 临时对象 - 修改样式属性-逐个绘制
//==============================
Game_Temp.prototype.drill_GFTT_setStyle_timingEnabled = function( enable ){
	if( this._drill_GFTT_commandBuffer == undefined ){ alert( DrillUp.drill_GFTT_getPluginTip_BufferIsNull() ); return; }
	this._drill_GFTT_commandBuffer['s_data']['timing_enabled'] = enable;
};
//==============================
...
// * C窗口皮肤 - 帧刷新 - 高宽变化『窗口皮肤』
//==============================
Drill_GFTT_Window.prototype.drill_updateSkin_skinBackground = function() {
...
// * D窗口内容 - 帧刷新
//==============================
Drill_GFTT_Window.prototype.drill_updateMessage = function(){
...
// * D窗口内容 - 刷新内容
//==============================
Drill_GFTT_Window.prototype.drill_refreshMessage = function( context ){
...
if( typeof(_drill_COWC_drawText_functionExist) == "undefined" ){
	alert( DrillUp.drill_GFTT_getPluginTip_NeedUpdate_drawText() );
}
