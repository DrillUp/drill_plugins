...
/*:
 * @plugindesc [v1.0]        游戏窗体 - 天窗层的多层平铺GIF
 * @author Drill_up
 * 
...
//		临时全局变量	DrillUp.g_HDSTG_xxx
//		临时局部变量	this._drill_HDSTG_xxx
//		存储数据变量	$gameSystem._drill_HDSTG_xxx
//		全局存储变量	无
//		覆盖重写方法	无
//
//<<<<<<<<性能记录<<<<<<<<
...
//=============================================================================
	...
	DrillUp.drill_HDSTG_tiledGIFInit = function( dataFrom ){
		...
		data['speedX'] = Number( dataFrom["平铺GIFX速度"] || 0);
		data['speedY'] = Number( dataFrom["平铺GIFY速度"] || 0);
		...
		if( args.length == 8 ){
			...
			if( type == "修改单属性" ){
				...
				if( temp1.indexOf("透明度[") != -1 ||
					...
					var num_list = this.drill_HDSTG_getArgNumList(temp1);
					for( var k=0; k < controllers.length; k++ ){
...
//==============================
Game_Interpreter.prototype.drill_HDSTG_getArgNumList = function( arg_str ){
...
// * F周期指令 - 清空全部（开放函数）
//==============================
Drill_HDSTG_Controller.prototype.drill_controller_clearPeriodizeCommand = function(){
	this._drill_curPeriodizeCommandTank = [];
}
