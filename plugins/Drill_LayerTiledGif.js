 ...
 * @on 启用
 * @off 关闭
 * @desc 设置后，平铺GIF会被 地图动态遮罩板 遮住，通过特定的 透视镜 才能看到该平铺GIF的部分画面。
 * @default false
 *
 * @param 关联的动态遮罩板
 * @parent ---动态遮罩---
...
Game_System.prototype.initialize = function() {
	...
	this.drill_LTG_initData();
};
//==============================
// ** 存储数据 - 初始化数据
//==============================
Game_System.prototype.drill_LTG_initData = function() {
...
// * 存档文件 - 载入存档 - 数据赋值
//==============================
var _drill_LTG_extractSaveContents = DataManager.extractSaveContents;
DataManager.extractSaveContents = function(contents){
	_drill_LTG_extractSaveContents.call( this, contents );
	
	if( DrillUp.g_LTG_saveEnabled == false ){	//（未开参数存储，则直接覆盖初始化数据）
		$gameSystem.drill_LTG_initData();
...
//##############################
Scene_Map.prototype.drill_LTG_layerMoveingReference = function (x, y, reference, option) {
    return this.drill_LTG_layerMoveingReference_Private(x, y, reference, option);
}
//=============================================================================
// ** 地图层级（接口实现）
//=============================================================================
...
Spriteset_Map.prototype.createParallax = function() {
	_drill_LTG_map_createParallax.call(this);		//rmmv远景 < 下层 < rmmv图块
...
Spriteset_Map.prototype.createTilemap = function() {
	_drill_LTG_map_createTilemap.call(this);		//rmmv图块 < 中层 < rmmv玩家
...
Spriteset_Map.prototype.createDestination = function() {
	_drill_LTG_map_createDestination.call(this);	//rmmv鼠标目的地 < 上层 < rmmv天气
...
Spriteset_Map.prototype.createPictures = function() {
	_drill_LTG_map_createPictures.call(this);		//rmmv图片 < 图片层 < rmmv对话框
...
Scene_Map.prototype.createAllWindows = function() {
	_drill_LTG_map_createAllWindows.call(this);	//rmmv对话框 < 最顶层
...
// * 地图层级 - 参照的位移（私有）
//==============================
Scene_Map.prototype.drill_LTG_layerMoveingReference_Private = function( xx, yy, reference, option ){
	...
	for(var i=0; i< sprite_tank.length; i++){
		...
		if( temp_sprite['_drill_src_bitmaps'].length != 0 ){
			...
			if (temp_data['layer_index'] == "下层" ||
			    ...
			    var pos = this.drill_LTG_layerMoveingReference(xx, yy, "地图参照 -> 地图参照", temp_data);
			    xx = pos['x'];
			    yy = pos['y'];
			}
			if (temp_data['layer_index'] == "图片层" ||
                temp_data['layer_index'] == "最顶层") {
			    var pos = this.drill_LTG_layerMoveingReference(xx, yy, "地图参照 -> 镜头参照", temp_data);
			    xx = pos['x'];
			    yy = pos['y'];
			}
