	...
	if( command === ">地图动态遮罩板B" ){
		...
		if( args.length >= 2 ){
			...
			if( temp1.indexOf("高级透视镜[") != -1 ){
				...
				if( marker == undefined ){
					alert( "【Drill_LayerIllumination.js 地图 - 地图动态遮罩板B】\n" +
...
Game_Temp.prototype.initialize = function() {
	...
	for( var i = 0; i < DrillUp.g_LDMB_childData.length; i++ ){
		...
		for( var j = 0; j < temp_data['gif_src'].length; j++ ){
			this._drill_LDMB_preloadTank.push( ImageManager.loadBitmap( temp_data['gif_src_file'], temp_data['gif_src'][j], 0, true ) );
		}
	}
}
...
Game_Map.prototype.drill_LDMB_addSimplePerspect_characterId = function( character_id, style_id ){	
	...
	if( data == undefined ){
		alert("【Drill_LayerDynamicMaskB.js  地图 - 地图动态遮罩板B】\n插件指令错误，不存在id为" + (style_id+1) +"的透视镜样式配置。");
		return;
	}
...
//=============================================================================
		...
		alert(
			"【Drill_LayerDynamicMaskB.js  地图 - 地图动态遮罩板B】\n缺少基础插件，去看看下列插件是不是 未添加 / 被关闭 / 顺序不对："+
