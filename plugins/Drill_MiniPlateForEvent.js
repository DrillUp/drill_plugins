...
//			2.感觉后续这个插件还会再被加强（目前都改到了2.6版本了），先停一辆皮卡车在这里。
//				         __
//				._______H__]\____.
//				|_______|'    _ _:
//				'   /(_) """"/(_)
...
//=============================================================================
	...
	// * 提示信息 - 报错 - 窗口字符底层校验
	//==============================
	DrillUp.drill_MPFE_getPluginTip_NeedUpdate_drawText = function(){
...
//=============================================================================
	...
	DrillUp.g_MPFE_lineheight_type = String(DrillUp.parameters["行高控制模式"] || "默认补正");
	DrillUp.g_MPFE_lineheight_custom = Number(DrillUp.parameters["自定义补正值"] || 30);
	DrillUp.g_MPFE_lineheight_lock = Number(DrillUp.parameters["锁定行高值"] || 30);
...
Game_Interpreter.prototype.pluginCommand = function( command, args ){
	...
	this.drill_MPFE_pluginCommand( command, args );
}
//==============================
// * 插件指令 - 指令执行
//==============================
Game_Interpreter.prototype.drill_MPFE_pluginCommand = function( command, args ){
	if( command === ">事件说明窗口" ){
...
Sprite_Character.prototype._refresh = function(){
	...
	if( this.bitmap == undefined ){
		this._character._drill_MPFE_bean.drill_bean_resetFrame(0,0,0,0);
		return;
	}
...
//==============================
Drill_MPFE_Window.prototype.lineHeight = function(){
	var base_lineHeight = Window_Base.prototype.lineHeight.call(this);
	var cur_lineHeight = base_lineHeight;
	
	if( DrillUp.g_MPFE_lineheight_type == "默认补正" ){
		cur_lineHeight = base_lineHeight;
	}
	if( DrillUp.g_MPFE_lineheight_type == "自定义补正" ){
		cur_lineHeight = DrillUp.g_MPFE_lineheight_custom;
	}
	if( DrillUp.g_MPFE_lineheight_type == "锁定行高" ){
		cur_lineHeight = DrillUp.g_MPFE_lineheight_lock;
	}
	if( DrillUp.g_MPFE_lineheight_type == "关闭行高控制" ){
		cur_lineHeight = 0;
	}
...
// * D窗口皮肤 - 帧刷新 - 高宽变化『窗口皮肤』
//==============================
Drill_MPFE_Window.prototype.drill_updateSkin_skinBackground = function() {
...
Drill_MPFE_Window.prototype.drill_refreshMessage = function( context_list ){
	...
	// > 附加宽高
	ww += $gameSystem._drill_MPFE_ex_width;
	hh += $gameSystem._drill_MPFE_ex_height;
...
if( typeof(_drill_COWC_drawText_functionExist) == "undefined" ){
	alert( DrillUp.drill_MPFE_getPluginTip_NeedUpdate_drawText() );
}
