...
//=============================================================================
	...
	// * 提示信息 - 报错 - 找不到样式
	//==============================
	DrillUp.drill_ASu_getPluginTip_StyleNotFind = function( style_id ){
		return "【" + DrillUp.g_ASu_PluginTip_curName + "】\n对象创建失败，id为"+style_id+"的样式配置为空或不存在。";
	};
	//==============================
	// * 提示信息 - 报错 - NaN校验值
	//==============================
	DrillUp.drill_ASu_getPluginTip_ParamIsNaN = function( param_name, check_tank ){
		var text = "【" + DrillUp.g_ASu_PluginTip_curName + "】\n检测到参数"+param_name+"出现了NaN值，请及时检查你的函数。";
		if( check_tank ){
...
Game_Interpreter.prototype.pluginCommand = function( command, args ){
	...
	this.drill_ASu_pluginCommand( command, args );
}
//==============================
// * 插件指令 - 指令执行
//==============================
Game_Interpreter.prototype.drill_ASu_pluginCommand = function( command, args ){
	if( command === ">动画环绕球" ){
...
Game_System.prototype.drill_ASu_checkSysData_Private = function() {
	...
	// > 容器的 空数据 检查
	for( var i = 0; i < DrillUp.g_ASu_style.length; i++ ){
		var temp_data = DrillUp.g_ASu_style[i];
	...
	for( var i = 0; i < DrillUp.g_ASu_style.length; i++ ){
		...
		if( this._animation.id == anim_data['anim'] ){
			...
			if( cur_styleData == undefined || 
				...
				alert( DrillUp.drill_ASu_getPluginTip_StyleNotFind(cur_styleId) );
				return;
			}
