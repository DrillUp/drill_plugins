 ...
 * 细节：
 *   (1.对话框是一个多个功能组合的形态，如下。
 *      对话框 = 逐个绘制 + 指令阻塞 + 子窗口管理 + 脸图管理 + 按键管理
 *      详细可以去看看文档介绍。
 *   (2.对话框支持连按立即显示功能。
 *      即对话框被打开后，字符会一个一个显示，如果这时再按一次确定键，
 *      那么当前对话页内的全部剩余文本，会立即显示出来
 * 设计：
 *   (1.对话框优化核心可以支持实时切换脸图，
 ...
 * 2.该插件专门管理 对话框 的字符绘制，子窗口变换。
 *   由于对话框功能复杂，但好在对话框只有一个，会有一些消耗，但不多。
 * 
 * -----------------------------------------------------------------------------
 * ----更新日志
 * [v1.0]
...
//		★性能测试因素	对话框管理层
//		★性能测试消耗	2025/4/30：
//							》5.6ms（drill_CODi_message_update）0.9ms（drill_CODi_message_isPageWaitingInput）
//							》『TotalTime』：560.9ms（drill_CODi_message_update）
//		★最坏情况		暂无
//		★备注			由于覆盖了对话框底层，并且对话框只有一个，所以消耗不大。
//		
//		★优化记录		
//							
//<<<<<<<<插件记录<<<<<<<<
...
Window_Message.prototype.drill_CODi_message_newPage = function(){
	...
	// > 自带参数初始化
	this.drill_CODi_message_initBitmapDefault();
...
// * 2Q绘制页 - 执行新建页 - 自带参数初始化
//
//			说明：	> 新建页时，没有重建画布，但画布参数需要重刷。
//==============================
Window_Message.prototype.drill_CODi_message_initBitmapDefault = function(){
	
	// > 对话框 的bitmap
	this.contents.drill_COCD_org_initBitmapDefault();
	
	// > 对话框子窗口 的bitmap
	var sub_window_list = this.subWindows();
	for( var i = 0; i < sub_window_list.length; i++ ){
		var sub_window = sub_window_list[i];
		if( sub_window == undefined ){ continue; }
		if( sub_window.contents == undefined ){ continue; }
		sub_window.contents.drill_COCD_org_initBitmapDefault();
	}
};
//==============================
// * 2Q绘制页 - 帧刷新
