...
Game_Interpreter.prototype.drill_MGi_pluginCommand = function( command, args ){
	if( command === ">菜单GIF" || command === ">菜单gif" ){
		...
		if( args.length == 4 ){
			...
			if( type === "显示" ){
				$gameSystem._drill_MGi_visible[temp1] = true;
			}
			if( type === "隐藏" ){
				$gameSystem._drill_MGi_visible[temp1] = false;
			}
		}
	}
};
...
Game_System.prototype.drill_MGi_initSysData_Private = function() {
	...
	this._drill_MGi_visible = [];
	for(var i = 0; i< DrillUp.g_MGi_style.length ;i++){
		var temp_data = DrillUp.g_MGi_style[i];
		if( temp_data == undefined ){ continue; }
		this._drill_MGi_visible[i] = temp_data['visible'];
	}
};
//==============================
// * 存储数据 - 载入存档时检查数据（私有）
...
Game_System.prototype.drill_MGi_checkSysData_Private = function() {
	...
	// > 旧存档数据自动补充
	if( this._drill_MGi_visible == undefined ){
		this.drill_MGi_initSysData();
	}
	...
	for(var i = 0; i < DrillUp.g_MGi_style.length; i++ ){
		...
		if( temp_data != undefined ){
			...
			// > 未存储的，重新初始化
			if( this._drill_MGi_visible[i] == undefined ){
				this._drill_MGi_visible[i] = temp_data['visible'];
...
Scene_MenuBase.prototype.drill_MGi_create = function() {	
	...
	if( this._drill_MGi_dataTank == undefined ){
		this._drill_MGi_dataTank = [];
	}
	...
	for( var i = 0; i < DrillUp.g_MGi_style.length; i++ ){
		...
		if( this.drill_MGi_checkKeyword( temp_data ) ){
			...
			temp_sprite.visible = $gameSystem._drill_MGi_visible[i] || false;
			...
			this.drill_MGi_layerAddSprite( temp_layer, temp_sprite_data['layerIndex'] );
		}
	}
	this.drill_MGi_sortByZIndex();
};
...
Drill_MGi_Controller.prototype.drill_controller_updateAttr = function(){
	...
	// > 显示/隐藏 设置
	this._drill_visible = $gameSystem._drill_MGi_visible[ this._drill_data_id ];
...
// * F周期指令 - 清空全部（开放函数）
//==============================
Drill_MGi_Controller.prototype.drill_controller_clearPeriodizeCommand = function(){
	this._drill_curPeriodizeCommandTank = [];
}
