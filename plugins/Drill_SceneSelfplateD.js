...
// * 正常 - 检查数据
//==============================
Game_System.prototype.drill_SSpD_sysCheckData = function() {
	if( this._drill_SSpD_enableTank == null ){ this._drill_SSpD_enableTank = []; }
	if( this._drill_SSpD_lockTank == null   ){ this._drill_SSpD_lockTank = []; }
	
	for( var i = 1; i <= DrillUp.g_SSpD_context_list_length ; i++ ){
		var temp_c = DrillUp.g_SSpD_context_list[i];
		
		// > 指定数据为空时
		if( this._drill_SSpD_enableTank[i] == null ){
			if( temp_c == null ){		//（无内容配置，跳过）
				this._drill_SSpD_enableTank[i] = null;
			}else{						//（有内容配置，初始化默认）
	...
	for( var i = 1; i <= DrillUp.g_SSpD_context_list_length ; i++ ){
		...
		// > 指定数据为空时
		if( this._drill_SSpD_lockTank[i] == null ){
			if( temp_c == null ){		//（无内容配置，跳过）
				this._drill_SSpD_lockTank[i] = null;
			}else{						//（有内容配置，初始化默认）
...
Game_Interpreter.prototype.pluginCommand = function(command, args) {
	...
	if( command === ">信息面板D" ){
		$gameSystem.drill_SSpD_sysCheckData();	//（初始化）
		if(args.length == 2){
...
Scene_Drill_SSpD.prototype.initialize = function() {
	...
	this._cur_index = -1;
	$gameSystem.drill_SSpD_sysCheckData();
};
//==============================
// * 信息面板D - 创建
//==============================
