...
Game_Character.prototype.drill_EFOE_updateHidingShrink = function() {
	...
	if( ef.fA_time < ef.fA_dTime ){
		...
		// > 锚点(0.5,1.0)锁定
		var fix_point = $gameTemp.drill_EFOE_getFixPointInAnchor( ef.anchor_x,ef.anchor_y, 0.5,1.0, ef.real_width,ef.real_height, ef.rotation, ef.scale_x, ef.scale_y );
		ef.x = fix_point.x;
...
Game_Character.prototype.drill_EFOE_updateHidingShrinkSpring = function() {
	...
	if( ef.f_time < ef.f_dTime ){
		...
		// > 锚点锁定
		var fix_point = $gameTemp.drill_EFOE_getFixPointInAnchor( ef.anchor_x,ef.anchor_y, ef.f_anchor_x, ef.f_anchor_y, ef.real_width,ef.real_height, ef.rotation, ef.scale_x, ef.scale_y );
		ef.x = fix_point.x;
		ef.y = fix_point.y;
