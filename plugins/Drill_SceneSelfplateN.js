...
//=============================================================================
	...
	// * 提示信息 - 报错 - 窗口字符底层校验
	//==============================
	DrillUp.drill_SSpN_getPluginTip_NeedUpdate_drawText = function(){
		return "【" + DrillUp.g_SSpN_PluginTip_curName + "】\n检测到窗口字符核心版本过低。\n由于底层变化巨大，你需要更新 全部 窗口字符相关插件。\n去看看\"23.窗口字符 > 关于窗口字符底层全更新说明.docx\"进行更新。";
	};
...
//=============================================================================
	...
	DrillUp.drill_SSpN_initCommandWindow = function( dataFrom ){
		...
		if( dataFrom["选项窗口指针与边框"] != undefined &&
			dataFrom["选项窗口指针与边框"] != "" ){
	...
	DrillUp.drill_SSpN_initDescWindow = function( dataFrom ){
		...
		if( dataFrom["描述窗口移动动画"] != undefined &&
			dataFrom["描述窗口移动动画"] != "" ){
	...
	DrillUp.drill_SSpN_initContext = function( dataFrom ){
		...
		// > 描述图片处理
		data['pic_1'] = String(dataFrom["资源-描述图片-1"] || "");
		data['pic_2'] = String(dataFrom["资源-描述图片-2"] || "");
		data['pic_3'] = String(dataFrom["资源-描述图片-3"] || "");
		data['pic_4'] = String(dataFrom["资源-描述图片-4"] || "");
		
		// > 描述内容处理
		if( dataFrom["描述内容-1"] != undefined &&
			dataFrom["描述内容-1"] != "" ){
			data['context1'] = JSON.parse( dataFrom["描述内容-1"] );
		}else{
			data['context1'] = "";
		}
		if( dataFrom["描述内容-2"] != undefined &&
			dataFrom["描述内容-2"] != "" ){
			data['context2'] = JSON.parse( dataFrom["描述内容-2"] );
		}else{
			data['context2'] = "";
		}
		if( dataFrom["描述内容-3"] != undefined &&
			dataFrom["描述内容-3"] != "" ){
			data['context3'] = JSON.parse( dataFrom["描述内容-3"] );
		}else{
			data['context3'] = "";
		}
		if( dataFrom["描述内容-4"] != undefined &&
			dataFrom["描述内容-4"] != "" ){
			data['context4'] = JSON.parse( dataFrom["描述内容-4"] );
		}else{
			data['context4'] = "";
		}
	...
	/*-----------------面板跳转------------------*/
	DrillUp.g_SSpN_add_to_menu = String(DrillUp.parameters["是否在主菜单窗口中显示"] || "true") === "true";	
    DrillUp.g_SSpN_menu_name = String(DrillUp.parameters["主菜单窗口显示名"] || "");
	...
	for( var i = 0; i < DrillUp.g_SSpN_context_list_length; i++ ){
	...
	DrillUp.g_SSpN_locked_name = DrillUp.g_SSpN_locked_name.replace(/\\\\/g,"\\");	//（为了支持\\n的写法）
	if( DrillUp.parameters["用语-锁定的描述内容"] != undefined && 
		DrillUp.parameters["用语-锁定的描述内容"] != "" ){
		DrillUp.g_SSpN_locked_context = JSON.parse( DrillUp.parameters["用语-锁定的描述内容"] );
	}else{
		if( DrillUp.parameters["用语-锁定的选项内容"] != undefined &&	//（旧名称）
			DrillUp.parameters["用语-锁定的选项内容"] != "" ){
			DrillUp.g_SSpN_locked_context = JSON.parse( DrillUp.parameters["用语-锁定的选项内容"] );
		}else{
			DrillUp.g_SSpN_locked_context = "";
		}
	}
	...
	/*-----------------未读提示------------------*/
	DrillUp.g_SSpN_watch_enabled = String(DrillUp.parameters["是否启用未读提示"] || "true") === "true";	
	DrillUp.g_SSpN_watch_fontsize = Number(DrillUp.parameters["未读提示字体大小"] || 20);
	DrillUp.g_SSpN_watch_setCorner = String(DrillUp.parameters["未读提示是否放右下角"] || "true") == "true";
	if( DrillUp.parameters["用语-未读提示信息"] != undefined && 
		DrillUp.parameters["用语-未读提示信息"] != "" ){
		DrillUp.g_SSpN_watch_text = JSON.parse( DrillUp.parameters["用语-未读提示信息"] );
	}else{
		DrillUp.g_SSpN_watch_text = "";
	}
	
	/*-----------------全局存储对象------------------*/
	DrillUp.g_SSpN_globalSetting_enabled = String(DrillUp.parameters["数据是否全局存储"] || "false") === "true";
    DrillUp.g_SSpN_globalSetting_fileId = Number(DrillUp.parameters["全局存储的文件路径"] || 1);
	DrillUp.global_SSpN_enableTank = null;
	DrillUp.global_SSpN_lockTank = null;
	DrillUp.global_SSpN_watchedTank = null;
	...
	for( var i = 0; i < DrillUp.g_SSpN_context_list.length; i++ ){
		...
		if( DrillUp.global_SSpN_enableTank[i] == null ){
			if( temp_data == null ){		//（无内容配置，跳过）
				...
				DrillUp.global_SSpN_enableTank[i] = temp_data['enabled'];
			}
	...
	for( var i = 0; i < DrillUp.g_SSpN_context_list.length; i++ ){
		...
		if( DrillUp.global_SSpN_lockTank[i] == null ){
			if( temp_data == null ){		//（无内容配置，跳过）
				...
				DrillUp.global_SSpN_lockTank[i] = temp_data['locked'];
			}
...
// * 『全局存储』 - 载入时检查数据 - 已读情况
//==============================
DrillUp.drill_SSpN_gCheckData_watched = function(){
	for( var i = 0; i < DrillUp.g_SSpN_context_list.length; i++ ){
		var temp_data = DrillUp.g_SSpN_context_list[i];
		
		// > 指定数据为空时
		if( DrillUp.global_SSpN_watchedTank[i] == null ){
			if( temp_data == null ){		//（无内容配置，跳过）
				DrillUp.global_SSpN_watchedTank[i] = null;
			}else{							//（有内容配置，初始化默认）
				DrillUp.global_SSpN_watchedTank[i] = temp_data['watched'];
			}
...
// * 『全局存储』 - 载入
//==============================
	var global_fileId = DrillUp.g_SSpN_globalSetting_fileId;
	var global_data = StorageManager.drill_COGS_loadData( global_fileId, "SSpN" );  //『全局存储执行函数』
	...
	// > 已读情况
	if( DrillUp.global_SSpN_watchedTank == null ){	
		var data = global_data["global_watchedTank"];
		if( data == undefined ){ data = [] };
		DrillUp.global_SSpN_watchedTank = data;
		DrillUp.drill_SSpN_gCheckData_watched();
	}
...
StorageManager.drill_SSpN_saveData = function(){
	var file_id = DrillUp.g_SSpN_globalSetting_fileId;
	var data = {};
	data["global_enableTank"] = DrillUp.global_SSpN_enableTank;
	data["global_lockTank"] = DrillUp.global_SSpN_lockTank;
	data["global_watchedTank"] = DrillUp.global_SSpN_watchedTank;
	this.drill_COGS_saveData( file_id, "SSpN", data );  //『全局存储执行函数』
};
...
Game_System.prototype.drill_SSpN_checkSysData_Private = function() {
	...
	for( var i = 0; i < DrillUp.g_SSpN_context_list.length; i++ ){
		...
		if( temp_data != undefined ){
			...
			// > 未存储的，重新初始化
			if( this._drill_SSpN_watchedTank[i] == undefined ){
				this._drill_SSpN_watchedTank[i] = temp_data['watched'];
...
Game_Interpreter.prototype.pluginCommand = function( command, args ){
	...
	this.drill_SSpN_pluginCommand( command, args );
}
//==============================
// * 插件指令 - 指令执行
//==============================
Game_Interpreter.prototype.drill_SSpN_pluginCommand = function( command, args ){
	if( command === ">信息面板N" ){
		...
		if( args.length == 4 ){
			...
			if( type == "显示选项" ){
				DrillUp.global_SSpN_enableTank[ Number(temp1)-1 ] = true;			//全局存储
				$gameSystem._drill_SSpN_enableTank[ Number(temp1)-1 ] = true;		//正常存储
				StorageManager.drill_SSpN_saveData();
			}
			if( type == "隐藏选项" ){
				DrillUp.global_SSpN_enableTank[ Number(temp1)-1 ] = false;			//全局存储
				$gameSystem._drill_SSpN_enableTank[ Number(temp1)-1 ] = false;		//正常存储
				StorageManager.drill_SSpN_saveData();
			}
			if( type == "锁定选项" ){
				DrillUp.global_SSpN_lockTank[ Number(temp1)-1 ] = true;				//全局存储
				$gameSystem._drill_SSpN_lockTank[ Number(temp1)-1 ] = true;			//正常存储
				StorageManager.drill_SSpN_saveData();
			}
			if( type == "解锁选项" ){
				DrillUp.global_SSpN_lockTank[ Number(temp1)-1 ] = false;			//全局存储
				$gameSystem._drill_SSpN_lockTank[ Number(temp1)-1 ] = false;		//正常存储
				StorageManager.drill_SSpN_saveData();
			}
...
Game_Temp.prototype.initialize = function() {	
	...
	this._drill_SSpN_visibleContextDataList = [];
};
//==============================
// * 面板控制 - 判断 锁定情况（开放函数）
//==============================
Game_Temp.prototype.drill_SSpN_isLocked = function( context_realIndex ){
	...
	// > 全局存储控制
	if( DrillUp.g_SSpN_globalSetting_enabled == true ){
...
// * 面板控制 - 判断 已读情况
//==============================
Game_Temp.prototype.drill_SSpN_isWatched = function( context_realIndex ){
	
	// > 全局存储控制
	if( DrillUp.g_SSpN_globalSetting_enabled == true ){
		if( DrillUp.global_SSpN_watchedTank[ context_realIndex ] == true ){
			return true;
		}else{
			return false;
		}
	...
	// > 正常存储控制
		...
		if( $gameSystem._drill_SSpN_watchedTank[ context_realIndex ] == true ){
			return true;
		}else{
			return false;
		}
...
// * A主体 - 创建
//==============================
Scene_Drill_SSpN.prototype.drill_createAttr = function() {
...
// * B选项窗口 - 帧刷新
//==============================
Scene_Drill_SSpN.prototype.drill_updateSelect = function() {
	
	// > 初始化 - 插件指令选中
	var max_index = $gameTemp._drill_SSpN_visibleContextDataList.length -1;
	if( $gameSystem._drill_SSpN_context_index != undefined ){
		var index = $gameSystem._drill_SSpN_context_index;
		if( index < 0 ){ index = 0; };
...
// * B选项窗口 - 帧刷新 - 选项变化时
//==============================
Scene_Drill_SSpN.prototype.drill_updateSelectChanged = function() {
	if( $gameTemp._drill_SSpN_visibleContextDataList.length == 0 ){ return };
	...
	if( this._drill_curSelectedIndex != this._window_select._index ){
		...
		if( DrillUp.g_SSpN_desc_1['replay'] ){
			this._window_desc_1.drill_COWA_resetAttrMove(); //『辅助核心动画』-重播窗口动画
			this._window_desc_1.drill_COWA_resetAttrOpacity(); //『辅助核心动画』-重播窗口动画
		}
		if( DrillUp.g_SSpN_desc_2['replay'] ){
			this._window_desc_2.drill_COWA_resetAttrMove(); //『辅助核心动画』-重播窗口动画
			this._window_desc_2.drill_COWA_resetAttrOpacity(); //『辅助核心动画』-重播窗口动画
		}
		if( DrillUp.g_SSpN_desc_3['replay'] ){
			this._window_desc_3.drill_COWA_resetAttrMove(); //『辅助核心动画』-重播窗口动画
			this._window_desc_3.drill_COWA_resetAttrOpacity(); //『辅助核心动画』-重播窗口动画
		}
		if( DrillUp.g_SSpN_desc_4['replay'] ){
			this._window_desc_4.drill_COWA_resetAttrMove(); //『辅助核心动画』-重播窗口动画
			this._window_desc_4.drill_COWA_resetAttrOpacity(); //『辅助核心动画』-重播窗口动画
		}
		// > 描述窗口 - 重绘内容
		this._window_desc_1.drill_refreshDescText(this._drill_curSelectedIndex);
		this._window_desc_2.drill_refreshDescText(this._drill_curSelectedIndex);
		this._window_desc_3.drill_refreshDescText(this._drill_curSelectedIndex);
		this._window_desc_4.drill_refreshDescText(this._drill_curSelectedIndex);
		
		// > 描述图 - 重播贴图动画
		if( DrillUp.g_SSpN_descPic_1['replay'] ){
			this._sprite_descPic_1.drill_COWA_resetAttrMove(); //『辅助核心动画』-重播贴图动画
		}
		if( DrillUp.g_SSpN_descPic_2['replay'] ){
			this._sprite_descPic_2.drill_COWA_resetAttrMove(); //『辅助核心动画』-重播贴图动画
		}
		if( DrillUp.g_SSpN_descPic_3['replay'] ){
			this._sprite_descPic_3.drill_COWA_resetAttrMove(); //『辅助核心动画』-重播贴图动画
		}
		if( DrillUp.g_SSpN_descPic_4['replay'] ){
			this._sprite_descPic_4.drill_COWA_resetAttrMove(); //『辅助核心动画』-重播贴图动画
		}
		// > 描述图 - 切换描述图
...
Scene_Drill_SSpN.prototype.drill_createDesc = function() {
	...
	this._window_desc_1.drill_COWA_changeParamData( DrillUp.g_SSpN_desc_1 ); //『辅助核心初始化』-窗口基本属性
	this._window_desc_2 = new Drill_SSpN_DescWindow(0, 0, 0, 0,  2);
	this._window_desc_2.drill_COWA_changeParamData( DrillUp.g_SSpN_desc_2 ); //『辅助核心初始化』-窗口基本属性
	this._window_desc_3 = new Drill_SSpN_DescWindow(0, 0, 0, 0,  3);
	this._window_desc_3.drill_COWA_changeParamData( DrillUp.g_SSpN_desc_3 ); //『辅助核心初始化』-窗口基本属性
	this._window_desc_4 = new Drill_SSpN_DescWindow(0, 0, 0, 0,  4);
	this._window_desc_4.drill_COWA_changeParamData( DrillUp.g_SSpN_desc_4 ); //『辅助核心初始化』-窗口基本属性
	
	this._drill_field.addChild(this._window_desc_1);
	this._drill_field.addChild(this._window_desc_2);
	this._drill_field.addChild(this._window_desc_3);
	this._drill_field.addChild(this._window_desc_4);
};
//==============================
// * C描述窗口 - 帧刷新（x4）
//==============================
Scene_Drill_SSpN.prototype.drill_updateDesc = function() {
	//（不操作，动画自己会帧刷新）
};
...
Scene_Drill_SSpN.prototype.drill_createDescPic = function() {
	...
	this._sprite_descPic_1.drill_COWA_setAttrMove( DrillUp.g_SSpN_descPic_1 ); //『辅助核心初始化』-贴图基本属性
	this._sprite_descPic_2 = new Sprite();
	this._sprite_descPic_2.drill_COWA_setAttrMove( DrillUp.g_SSpN_descPic_2 ); //『辅助核心初始化』-贴图基本属性
	this._sprite_descPic_3 = new Sprite();
	this._sprite_descPic_3.drill_COWA_setAttrMove( DrillUp.g_SSpN_descPic_3 ); //『辅助核心初始化』-贴图基本属性
	this._sprite_descPic_4 = new Sprite();
	this._sprite_descPic_4.drill_COWA_setAttrMove( DrillUp.g_SSpN_descPic_4 ); //『辅助核心初始化』-贴图基本属性
...
Scene_Drill_SSpN.prototype.drill_refreshDescPic = function( cur_index ){
	var temp_curData = $gameTemp._drill_SSpN_visibleContextDataList[ cur_index ];	//（当前选项）
	
	// > 资源全加载 1
	var src_tank_1 = this._sprite_descPic_1._drill_bitmaps;
	if( src_tank_1.length == 0 ){
		src_tank_1[0] = ImageManager.load_MenuSelfDef(DrillUp.g_SSpN_locked_pic);
		for( var i = 0; i < $gameTemp._drill_SSpN_visibleContextDataList.length; i++ ){
			var context_index = $gameTemp._drill_SSpN_visibleContextDataList[i]['index'];
...
//==============================
// * 信息面板N（菜单界面基类） - 当前角色切换时
//==============================
Scene_Drill_SSpN.prototype.onActorChange = function() {
...
Drill_SSpN_SelectWindow.prototype.refresh = function() {
	...
	// > 重刷可见项列表
	$gameTemp._drill_SSpN_visibleContextDataList = [];
	...
	for( var i = 0; i < DrillUp.g_SSpN_context_list.length; i++ ){
		...
		if( $gameTemp.drill_SSpN_isEnabled( i ) == true ){
			$gameTemp._drill_SSpN_visibleContextDataList.push( temp_data );
	...
	for( var j = 0; j < $gameTemp._drill_SSpN_visibleContextDataList.length ;j++ ){
		var temp_data = $gameTemp._drill_SSpN_visibleContextDataList[j];
		var context_realIndex = temp_data['index'];
		
		// > 选项锁定
		if( $gameTemp.drill_SSpN_isLocked( context_realIndex ) == true ){
			this._drill_selectionNameList[j] = String(DrillUp.g_SSpN_locked_name);
			continue;
		}
...
// * 2A选中 - 参数初始化
//==============================
Drill_SSpN_SelectWindow.g_drill_lastTopRow = 0;
Drill_SSpN_SelectWindow.g_drill_lastIndex  = 0;
//==============================
// * 2A选中 - 设置选项
//==============================
Drill_SSpN_SelectWindow.prototype.drill_window_initSelect = function() {
	if( Drill_SSpN_SelectWindow.g_drill_lastIndex >= this._drill_selectionNameList.length ){
		Drill_SSpN_SelectWindow.g_drill_lastIndex = this._drill_selectionNameList.length-1;
	}
	this.setTopRow(Drill_SSpN_SelectWindow.g_drill_lastTopRow);
	this.select(Drill_SSpN_SelectWindow.g_drill_lastIndex);
}
//==============================
// * 2A选中 - 退出时暂存选项（继承）
//==============================
Drill_SSpN_SelectWindow.prototype.processCancel = function() {
	...
	Drill_SSpN_SelectWindow.g_drill_lastTopRow = this.topRow();
	Drill_SSpN_SelectWindow.g_drill_lastIndex = this.index();
};
...
//==============================
Drill_SSpN_SelectWindow.prototype.drill_window_drawAllItem = function(){
...
//==============================
Drill_SSpN_SelectWindow.prototype.drill_window_drawItem = function( index ){
	...
	// > 参数准备 - 窗口字符对齐方式
	var cur_align = DrillUp.g_SSpN_command_window['align'];
...
// * 2C已读情况 - 绘制新标签
//==============================
Drill_SSpN_SelectWindow.prototype.drill_window_drawNewTag = function( index ){
	if( DrillUp.g_SSpN_watch_enabled != true ){ return; }
	
	var temp_data = $gameTemp._drill_SSpN_visibleContextDataList[index];	//（当前选项）
	if( temp_data == undefined ){ return; }
	var rect = this.itemRectForText(index);
	var context_realIndex = temp_data['index'];
	
	// > 已读则跳出
	if( $gameTemp.drill_SSpN_isWatched( context_realIndex ) == true ){ return; }
	// > 锁定则跳出
	if( $gameTemp.drill_SSpN_isLocked( context_realIndex ) == true ){ return; }
	
	
	// > 参数准备 - 校验
	var temp_bitmap = this.contents;
	if( temp_bitmap == undefined ){ return; }
	var org_text = DrillUp.g_SSpN_watch_text;
	if( org_text == undefined ){ return; }
	if( org_text == "" ){ return; }
	...
	options['rowParam']['alignHor_maxWidth'] = rect.width;		//『选项的文本域范围』
	options['rowParam']['alignVer_maxHeight'] = rect.height;	//
	if( DrillUp.g_SSpN_watch_setCorner == true ){
		options['rowParam']['alignHor_type'] = "right";	//（右下角）
		options['rowParam']['alignVer_type'] = "bottom";
	}
	
	options['baseParam'] = {};
	options['baseParam']['fontSize'] = DrillUp.g_SSpN_watch_fontsize;
...
//==============================
Drill_SSpN_SelectWindow.prototype.drill_window_watchedChanged = function( last_index ){
	if( DrillUp.g_SSpN_watch_enabled != true ){ return; }
	
	var temp_data = $gameTemp._drill_SSpN_visibleContextDataList[ last_index ];	//（当前选项）
	if( temp_data == undefined ){ return; }
	var context_realIndex = temp_data['index'];
	...
	// > 新标签 数据变化
	if( $gameTemp.drill_SSpN_isWatched( context_realIndex ) != true ){
		DrillUp.global_SSpN_watchedTank[ context_realIndex ] = true;
		$gameSystem._drill_SSpN_watchedTank[ context_realIndex ] = true;
...
// * 显示窗口 - 重绘内容
//==============================
Drill_SSpN_DescWindow.prototype.drill_refreshDescText = function( cur_index ){
	var temp_data = $gameTemp._drill_SSpN_visibleContextDataList[ cur_index ];	//（当前选项）
	if( temp_data == undefined ){ return; }
	
	// > 切换描述内容
	var context = "";
	if( $gameTemp.drill_SSpN_isLocked( temp_data['index'] ) == true && 
		(DrillUp.g_SSpN_locked_type == "锁定描述图和描述内容" || 
		...
		if( this._drill_wi == 1 ){ 
			if( temp_data['lockedContextEnabled1'] == true ){
				...
				context = DrillUp.g_SSpN_locked_context;	//用语-锁定的描述内容
			}
		}
		if( this._drill_wi == 2 ){ 
		...
		if( this._drill_wi == 2 ){ context = temp_data['context2']; }
		if( this._drill_wi == 3 ){ context = temp_data['context3']; }
		if( this._drill_wi == 4 ){ context = temp_data['context4']; }
	}
...
if( typeof(_drill_COWC_drawText_functionExist) == "undefined" ){
	alert( DrillUp.drill_SSpN_getPluginTip_NeedUpdate_drawText() );
}
