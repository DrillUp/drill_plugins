...
//		
//		★必要注意事项：
//			1.SceneManager控制的场景流程如下：
//				进入场景：
//					初始化（initialize） > 创建（create） > 等待加载（isReady） > 开始运行（start）
//				正常push场景：（慢慢执行changeScene）
//					终止（terminate） > 结束运行（stop）
//				直接goto场景：（立刻终止）
//					结束运行（stop）
//			  其中，startFadeIn  在 start 中执行。
//			  其中，startFadeOut 在 stop  中执行。
//			  但并不是所有界面的转场都会执行这两函数，子插件需要 手动提供 两基础函数的执行。
//			
//			2.实际游戏中，start 可能会卡。
//			  要多考虑 startFadeIn淡入 的动画延迟时间。
//			
//			3.动画转场 【不管】 公共事件的执行。
//		
//		★其它说明细节：
//			1.这个插件是界面跳转的动画管理器。
//			  依稀记得第一次写的时候超级难受，现在回头来看，似乎很完美，没有什么要优化的地方。
//				 ― 、-―――-、_-、
//				(  ス 　  ＼   )
//				 \/　　 　@ ヽ/ 
//				（  　　　　/ } 
...
//			返回：	> 无
//
//			说明：	> 如果想继续播放淡入淡出动画，可以设置参数为true；如果不想，设置参数为false。
//					> 界面截图兼容：天窗层动画+静态快照截图，设置参数为true。效果为动画正常播放但颜色透明。
//##############################
...
//					> 界面截图兼容：天窗层动画+静态快照截图，设置参数为空字符串。效果为动画正常播放但颜色透明。
//##############################
