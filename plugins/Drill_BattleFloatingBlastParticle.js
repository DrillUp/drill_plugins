...
//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
//
//		插件简称		BFBPa（Battle_Floating_Blast_Particle）
//		覆盖重写方法	无
//
//<<<<<<<<性能记录<<<<<<<<
//
...
//=============================================================================
	...
	// * 提示信息 - 报错 - 找不到样式
	//==============================
	DrillUp.drill_BFBPa_getPluginTip_StyleNotFind = function( style_id ){
...
//=============================================================================
	...
	for (var i = 0; i < DrillUp.g_BFBPa_style_length; i++) {
		if( DrillUp.parameters['粒子样式-' + String(i+1) ] != undefined && 
			...
			DrillUp.g_BFBPa_style[i]['id'] = i+1;
...
Game_Interpreter.prototype.pluginCommand = function( command, args ){
	...
	this.drill_BFBPa_pluginCommand( command, args );
}
//==============================
// * 插件指令 - 指令执行
//==============================
Game_Interpreter.prototype.drill_BFBPa_pluginCommand = function( command, args ){
...
Game_Temp.prototype.drill_BFBPa_createSimple = function( pos, style_id ){
	...
	// > 基本参数初始化
	var data = {};
	data['s_data'] = JSON.parse(JSON.stringify( DrillUp.g_BFBPa_style[ style_id ] ));  //『深拷贝-『临时对象』』
...
Scene_Battle.prototype.drill_BFBPa_updateWindowAddChild = function() {
	for( var i = $gameTemp._drill_BFBPa_commandSeq.length-1; i >= 0; i-- ){
		...
		if( cur_styleData == undefined || 
			...
			alert( DrillUp.drill_BFBPa_getPluginTip_StyleNotFind(cur_styleId) );
		...
		if( temp_controller._drill_data['second_enable'] == true ){
			...
			this.drill_BFBPa_layerAddSprite( temp_secSprite, cur_styleData['second_layerIndex'] );
		}
		
		
		// > 添加贴图到层级
		$gameTemp._drill_BFBPa_spriteTank.push( temp_sprite );
		this.drill_BFBPa_layerAddSprite( temp_sprite, cur_styleData['layerIndex'] );
