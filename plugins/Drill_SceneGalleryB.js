 ...
 * 
 * 插件指令：>画廊B : 显示选项 : 选项[1]
 * 插件指令：>画廊B : 显示选项 : 选项变量[21]
 * 插件指令：>画廊B : 隐藏选项 : 选项[1]
 * 插件指令：>画廊B : 隐藏选项 : 选项变量[21]
 * 插件指令：>画廊B : 显示全部选项
 * 插件指令：>画廊B : 隐藏全部选项
 * 
 * 插件指令：>画廊B : 锁定选项 : 选项[1]
 * 插件指令：>画廊B : 锁定选项 : 选项变量[21]
 * 插件指令：>画廊B : 解锁选项 : 选项[1]
 * 插件指令：>画廊B : 解锁选项 : 选项变量[21]
 * 插件指令：>画廊B : 锁定全部选项
 * 插件指令：>画廊B : 解锁全部选项
 *
...
//				画廊B
//				Drill_SceneGalleryB
//			2. 2024/11/14：这插件功能那么多，乌云也那么多，整理起来烦死欧拉。
//				 ╭◜◝ ͡  ◜◝ ╮ 
//				(  按钮组  ) 
//				 ╰◟ ͜ ╭◜◝ ͡  ◜◝ ͡  ╮
//				    (   单图查看  )
...
//=============================================================================
	...
	// * 提示信息 - 报错 - 窗口字符底层校验
	//==============================
	DrillUp.drill_SGaB_getPluginTip_NeedUpdate_drawText = function(){
		return "【" + DrillUp.g_SGaB_PluginTip_curName + "】\n检测到窗口字符核心版本过低。\n由于底层变化巨大，你需要更新 全部 窗口字符相关插件。\n去看看\"23.窗口字符 > 关于窗口字符底层全更新说明.docx\"进行更新。";
	};
...
//=============================================================================
	...
	/*-----------------面板跳转------------------*/
	DrillUp.g_SGaB_add_to_menu = String(DrillUp.parameters["是否在主菜单窗口中显示"] || "true") === "true";	
    DrillUp.g_SGaB_menu_name = String(DrillUp.parameters["主菜单窗口显示名"] || "");
	...
	/*-----------------原图查看器------------------*/
	DrillUp.g_SGaB_imgExpandPic_enabled = String(DrillUp.parameters["是否启用原图查看器"] || "true") === "true";
	...
	DrillUp.g_SGaB_singlePicture_hideLayout = String(DrillUp.parameters["单图模式时是否隐藏整体布局"] || "true") === "true";
	...
	DrillUp.g_SGaB_compWin_word = String(DrillUp.parameters["完成度用语"] || "");
	...
	for( var i = 0; i < DrillUp.g_SGaB_context_list_length; i++ ){
	...
	DrillUp.g_SGaB_locked_name = DrillUp.g_SGaB_locked_name.replace(/\\\\/g,"\\");	//（为了支持\\n的写法）
	if( DrillUp.parameters["用语-锁定的描述内容"] != undefined &&
		DrillUp.parameters["用语-锁定的描述内容"] != "" ){
		DrillUp.g_SGaB_locked_context = JSON.parse( DrillUp.parameters["用语-锁定的描述内容"] );
	}else{
	...
	/*-----------------未读提示------------------*/
	DrillUp.g_SGaB_watch_enabled = String(DrillUp.parameters["是否启用未读提示"] || "true") === "true";	
	DrillUp.g_SGaB_watch_fontsize = Number(DrillUp.parameters["未读提示字体大小"] || 20);
	DrillUp.g_SGaB_watch_setCorner = String(DrillUp.parameters["未读提示是否放右下角"] || "true") == "true";
	if( DrillUp.parameters["用语-未读提示信息"] != undefined && 
		DrillUp.parameters["用语-未读提示信息"] != "" ){
		DrillUp.g_SGaB_watch_text = JSON.parse( DrillUp.parameters["用语-未读提示信息"] );
	}else{
		DrillUp.g_SGaB_watch_text = "";
	}
	
	/*-----------------全局存储对象------------------*/
	DrillUp.g_SGaB_globalSetting_enabled = String(DrillUp.parameters["数据是否全局存储"] || "false") === "true";	
    DrillUp.g_SGaB_globalSetting_fileId = Number(DrillUp.parameters["全局存储的文件路径"] || 1);
	DrillUp.global_SGaB_enableTank = null;
	DrillUp.global_SGaB_lockTank = null;
	DrillUp.global_SGaB_watchedTank = null;
	...
	for( var i = 0; i < DrillUp.g_SGaB_context_list.length; i++ ){
		...
		if( DrillUp.global_SGaB_enableTank[i] == null ){
			if( temp_data == null ){		//（无内容配置，跳过）
				...
				DrillUp.global_SGaB_enableTank[i] = temp_data['enabled'];
			}
	...
	for( var i = 0; i < DrillUp.g_SGaB_context_list.length; i++ ){
		...
		if( DrillUp.global_SGaB_lockTank[i] == null ){
			if( temp_data == null ){		//（无内容配置，跳过）
				...
				DrillUp.global_SGaB_lockTank[i] = temp_data['locked'];
			}
...
// * 『全局存储』 - 载入时检查数据 - 已读情况
//==============================
DrillUp.drill_SGaB_gCheckData_watched = function(){
	for( var i = 0; i < DrillUp.g_SGaB_context_list.length; i++ ){
		var temp_data = DrillUp.g_SGaB_context_list[i];
		
		// > 指定数据为空时
		if( DrillUp.global_SGaB_watchedTank[i] == null ){
			if( temp_data == null ){		//（无内容配置，跳过）
				DrillUp.global_SGaB_watchedTank[i] = null;
			}else{							//（有内容配置，初始化默认）
				DrillUp.global_SGaB_watchedTank[i] = temp_data['watched'];
			}
...
// * 『全局存储』 - 载入
//==============================
	var global_fileId = DrillUp.g_SGaB_globalSetting_fileId;
	var global_data = StorageManager.drill_COGS_loadData( global_fileId, "SGaB" );  //『全局存储执行函数』
	...
	// > 已读情况
	if( DrillUp.global_SGaB_watchedTank == null ){	
		var data = global_data["global_watchedTank"];
		if( data == undefined ){ data = [] };
		DrillUp.global_SGaB_watchedTank = data;
		DrillUp.drill_SGaB_gCheckData_watched();
	}
...
StorageManager.drill_SGaB_saveData = function(){
	var file_id = DrillUp.g_SGaB_globalSetting_fileId;
	var data = {};
	data["global_enableTank"] = DrillUp.global_SGaB_enableTank;
	data["global_lockTank"] = DrillUp.global_SGaB_lockTank;
	data["global_watchedTank"] = DrillUp.global_SGaB_watchedTank;
	this.drill_COGS_saveData( file_id, "SGaB", data );  //『全局存储执行函数』
};
...
Game_System.prototype.drill_SGaB_checkSysData_Private = function() {
	...
	for( var i = 0; i < DrillUp.g_SGaB_context_list.length; i++ ){
		...
		if( temp_data != undefined ){
			...
			// > 未存储的，重新初始化
			if( this._drill_SGaB_watchedTank[i] == undefined ){
				this._drill_SGaB_watchedTank[i] = temp_data['watched'];
...
Game_Interpreter.prototype.pluginCommand = function( command, args ){
	...
	this.drill_SGaB_pluginCommand( command, args );
}
//==============================
// * 插件指令 - 指令执行
//==============================
Game_Interpreter.prototype.drill_SGaB_pluginCommand = function( command, args ){
	if( command === ">画廊B" ){
		...
		if( args.length == 4 ){
			...
			if( type == "打开面板(单图查看模式)" ){
				...
				$gameTemp._drill_SGaB_isSinglePictureIndex = temp1;
				SceneManager.push(Scene_Drill_SGaB);
			}
		}
		...
		if( args.length == 4 ){
			...
			if( type == "显示选项" ){
				DrillUp.global_SGaB_enableTank[ Number(temp1)-1 ] = true;			//全局存储
				$gameSystem._drill_SGaB_enableTank[ Number(temp1)-1 ] = true;		//正常存储
				StorageManager.drill_SGaB_saveData();
			}
			if( type == "隐藏选项" ){
				DrillUp.global_SGaB_enableTank[ Number(temp1)-1 ] = false;			//全局存储
				$gameSystem._drill_SGaB_enableTank[ Number(temp1)-1 ] = false;		//正常存储
				StorageManager.drill_SGaB_saveData();
			}
			if( type == "锁定选项" ){
				DrillUp.global_SGaB_lockTank[ Number(temp1)-1 ] = true;				//全局存储
				$gameSystem._drill_SGaB_lockTank[ Number(temp1)-1 ] = true;			//正常存储
				StorageManager.drill_SGaB_saveData();
			}
			if( type == "解锁选项" ){
				DrillUp.global_SGaB_lockTank[ Number(temp1)-1 ] = false;			//全局存储
				$gameSystem._drill_SGaB_lockTank[ Number(temp1)-1 ] = false;		//正常存储
				StorageManager.drill_SGaB_saveData();
			}
...
Game_Temp.prototype.initialize = function() {	
	...
	this._drill_SGaB_visibleContextDataList = [];	//可见的列表
};
//==============================
// * 面板控制 - 判断 锁定情况（开放函数）
//==============================
Game_Temp.prototype.drill_SGaB_isLocked = function( context_realIndex ){
	...
	// > 全局存储控制
	if( DrillUp.g_SGaB_globalSetting_enabled == true ){
...
// * 面板控制 - 判断 已读情况
//==============================
Game_Temp.prototype.drill_SGaB_isWatched = function( context_realIndex ){
	
	// > 全局存储控制
	if( DrillUp.g_SGaB_globalSetting_enabled == true ){
		if( DrillUp.global_SGaB_watchedTank[ context_realIndex ] == true ){
			return true;
		}else{
			return false;
		}
	...
	// > 正常存储控制
		...
		if( $gameSystem._drill_SGaB_watchedTank[ context_realIndex ] == true ){
			return true;
		}else{
			return false;
		}
...
// * A主体 - 创建 - 整体布局
//==============================
Scene_Drill_SGaB.prototype.drill_createAttr = function() {
	...
	if( $gameTemp._drill_SGaB_isSinglePictureMode == true &&
		DrillUp.g_SGaB_singlePicture_hideLayout == true ){
		//（单图模式则直接不添加）
	}else{
...
Scene_Drill_SGaB.prototype.drill_createSelect = function() {
	...
	if( DrillUp.g_SGaB_imgExpandPic_enabled == true ){							//（单图模式开关）
...
// * B选项窗口 - 帧刷新
//==============================
Scene_Drill_SGaB.prototype.drill_updateSelect = function() {
	
	// > 初始化 - 插件指令选中
	var max_index = $gameTemp._drill_SGaB_visibleContextDataList.length -1;
	if( $gameSystem._drill_SGaB_context_index != undefined ){
		var index = $gameSystem._drill_SGaB_context_index;
		if( index < 0 ){ index = 0; };
...
// * B选项窗口 - 帧刷新 - 选项变化时
//==============================
Scene_Drill_SGaB.prototype.drill_updateSelectChanged = function() {
	if( $gameTemp._drill_SGaB_visibleContextDataList.length == 0 ){ return };
...
// * C描述窗口 - 帧刷新
//==============================
Scene_Drill_SGaB.prototype.drill_updateDesc = function() {
	//（不操作，动画自己会帧刷新）
};
...
Scene_Drill_SGaB.prototype.drill_refreshThumbnail = function( cur_index ) {
	...
	if( src_tank.length == 0 ){
		...
		for( var i = 0; i < $gameTemp._drill_SGaB_visibleContextDataList.length; i++ ){
			var temp_data = $gameTemp._drill_SGaB_visibleContextDataList[i];
	...
	// > 切换缩略图
	var temp_data = $gameTemp._drill_SGaB_visibleContextDataList[ cur_index ];	//（当前选项）
	if( $gameTemp.drill_SGaB_isLocked( temp_data['index'] ) == true && 
...
Scene_Drill_SGaB.prototype.drill_createSinglePictureMode = function() {
	...
	this.drill_selectOk();		//（插件指令设置时，强制走确认流程）
};
//==============================
// * G单图查看模式 - 帧刷新
//==============================
...
Scene_Drill_SGaB.prototype.drill_selectOk = function() {
	...
	// > 获取选中的内容
	var temp_data = $gameTemp._drill_SGaB_visibleContextDataList[ this._window_select.index() ];
	if(!temp_data ){ 
...
// * 画廊B（菜单界面基类） - 当前角色切换时
//==============================
Scene_Drill_SGaB.prototype.onActorChange = function() {
...
Drill_SGaB_SelectWindow.prototype.refresh = function() {
	...
	// > 单图查看模式
	if( $gameTemp._drill_SGaB_isSinglePictureMode == true ){
		$gameTemp._drill_SGaB_visibleContextDataList = [];
		...
		$gameTemp._drill_SGaB_visibleContextDataList.push( DrillUp.g_SGaB_context_list[index] );
		this._drill_COSB_indexList.push( index );
	...
	// > 一般模式
		...
		for( var i = 0; i < DrillUp.g_SGaB_context_list.length; i++ ){
			...
			if( $gameTemp.drill_SGaB_isEnabled( i ) == true ){
				$gameTemp._drill_SGaB_visibleContextDataList.push( temp_data );
		...
		this._drill_selectionNameList = [];
		for( var j = 0; j < $gameTemp._drill_SGaB_visibleContextDataList.length ;j++ ){
			var temp_data = $gameTemp._drill_SGaB_visibleContextDataList[j];
			var context_realIndex = temp_data['index'];
			
			// > 选项锁定
			if( $gameTemp.drill_SGaB_isLocked( context_realIndex ) == true ){
				this._drill_selectionNameList[j] = String(DrillUp.g_SGaB_locked_name);
				continue;
			}
...
// * 2A选中 - 参数初始化
//==============================
Drill_SGaB_SelectWindow.g_drill_lastTopRow = 0;
Drill_SGaB_SelectWindow.g_drill_lastIndex  = 0;
//==============================
// * 2A选中 - 设置选项
//==============================
Drill_SGaB_SelectWindow.prototype.drill_window_initSelect = function() {
	if( Drill_SGaB_SelectWindow.g_drill_lastIndex >= this._drill_selectionNameList.length ){
		Drill_SGaB_SelectWindow.g_drill_lastIndex = this._drill_selectionNameList.length-1;
	}
	this.setTopRow(Drill_SGaB_SelectWindow.g_drill_lastTopRow);
	this.select(Drill_SGaB_SelectWindow.g_drill_lastIndex);
}
//==============================
// * 2A选中 - 执行 确定键（继承）
//==============================
Drill_SGaB_SelectWindow.prototype.processOk = function(){
	if( DrillUp.g_SGaB_imgExpandPic_enabled != true ){ return; }	//（单图模式开关）
...
Drill_SGaB_SelectWindow.prototype.processCancel = function() {
	...
	Drill_SGaB_SelectWindow.g_drill_lastTopRow = this.topRow();
	Drill_SGaB_SelectWindow.g_drill_lastIndex = this.index();
};
...
//==============================
Drill_SGaB_SelectWindow.prototype.drill_window_drawAllItem = function(){
...
//==============================
Drill_SGaB_SelectWindow.prototype.drill_window_drawItem = function( index ){
	...
	// > 参数准备 - 窗口字符对齐方式
	var cur_align = DrillUp.g_SGaB_command_window['align'];
...
// * 2B绘制选项 - 绘制单个小缩略图
//==============================
Drill_SGaB_SelectWindow.prototype.drill_window_drawIcon = function( index ){
	if( DrillUp.g_SGaB_command_window['icon_enabled'] != true ){ return; }
	
	var temp_data = $gameTemp._drill_SGaB_visibleContextDataList[index];	//（当前选项）
...
// * 2C已读情况 - 绘制新标签
//==============================
Drill_SGaB_SelectWindow.prototype.drill_window_drawNewTag = function( index ){
	if( DrillUp.g_SGaB_watch_enabled != true ){ return; }
	...
	// > 已读则跳出
	if( $gameTemp.drill_SGaB_isWatched( context_realIndex ) == true ){ return; }
	// > 锁定则跳出
	if( $gameTemp.drill_SGaB_isLocked( context_realIndex ) == true ){ return; }
	
	
	// > 参数准备 - 校验
	var temp_bitmap = this.contents;
	if( temp_bitmap == undefined ){ return; }
	var org_text = DrillUp.g_SGaB_watch_text;
	if( org_text == undefined ){ return; }
	if( org_text == "" ){ return; }
	...
	options['rowParam']['alignHor_maxWidth'] = rect.width;		//『选项的文本域范围』
	options['rowParam']['alignVer_maxHeight'] = rect.height;	//
	if( DrillUp.g_SGaB_watch_setCorner == true ){
		options['rowParam']['alignHor_type'] = "right";	//（右下角）
		options['rowParam']['alignVer_type'] = "bottom";
	}
	
	options['baseParam'] = {};
	options['baseParam']['fontSize'] = DrillUp.g_SGaB_watch_fontsize;
...
//==============================
Drill_SGaB_SelectWindow.prototype.drill_window_watchedChanged = function( last_index ){
	if( DrillUp.g_SGaB_watch_enabled != true ){ return; }
	
	var temp_data = $gameTemp._drill_SGaB_visibleContextDataList[ last_index ];	//（当前选项）
	if( temp_data == undefined ){ return; }
	var context_realIndex = temp_data['index'];
	...
	// > 新标签 数据变化
	if( $gameTemp.drill_SGaB_isWatched( context_realIndex ) != true ){
		DrillUp.global_SGaB_watchedTank[ context_realIndex ] = true;
		$gameSystem._drill_SGaB_watchedTank[ context_realIndex ] = true;
...
// * 显示窗口 - 重绘内容
//==============================
Drill_SGaB_DescWindow.prototype.drill_refreshDescText = function( cur_index ){
	var temp_data = $gameTemp._drill_SGaB_visibleContextDataList[ cur_index ];	//（当前选项）
	if( temp_data == undefined ){ return; }
...
if( typeof(_drill_COWC_drawText_functionExist) == "undefined" ){
	alert( DrillUp.drill_SGaB_getPluginTip_NeedUpdate_drawText() );
}
...
Drill_SGaB_CompletionWindow.prototype.drill_refresh = function(){
	var temp_list = $gameTemp._drill_SGaB_visibleContextDataList;		//可见项列表
