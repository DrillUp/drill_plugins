...
// * 正常 - 检查数据
//==============================
Game_System.prototype.drill_SGaB_sysCheckData = function() {
	if( this._drill_SGaB_enableTank == null ){ this._drill_SGaB_enableTank = []; }
	if( this._drill_SGaB_lockTank == null   ){ this._drill_SGaB_lockTank = []; }
	
	for( var i = 1; i <= DrillUp.g_SGaB_context_list_length ; i++ ){
		var temp_c = DrillUp.g_SGaB_context_list[i];
		
		// > 指定数据为空时
		if( this._drill_SGaB_enableTank[i] == null ){
			if( temp_c == null ){		//（无内容配置，跳过）
				this._drill_SGaB_enableTank[i] = null;
			}else{						//（有内容配置，初始化默认）
				this._drill_SGaB_enableTank[i] = temp_c['enabled'];
			}
	...
	for( var i = 1; i <= DrillUp.g_SGaB_context_list_length ; i++ ){
		...
		// > 指定数据为空时
		if( this._drill_SGaB_lockTank[i] == null ){
			if( temp_c == null ){		//（无内容配置，跳过）
				this._drill_SGaB_lockTank[i] = null;
			}else{						//（有内容配置，初始化默认）
				this._drill_SGaB_lockTank[i] = temp_c['locked'];
			}
...
Game_Interpreter.prototype.pluginCommand = function(command, args) {
	...
	if( command === ">画廊B" ){
		$gameSystem.drill_SGaB_sysCheckData();	//（初始化）
		if(args.length == 2){
...
Scene_Drill_SGaB.prototype.initialize = function() {
	...
	this._cur_index = -1;
	$gameSystem.drill_SGaB_sysCheckData();
};
//==============================
// * 画廊B - 创建
//============================== 
...
// * 创建 - 资源预加载
//==============================
Scene_Drill_SGaB.prototype.drill_preloadSrc = function() {
