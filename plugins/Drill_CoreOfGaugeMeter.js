...
// * 参数条 - 帧刷新 - 延迟初始化
//==============================
Drill_COGM_MeterSprite.prototype.drill_sprite_updateDelayingInit = function() {
...
Drill_COGM_MeterSprite.prototype.drill_initData = function() {
	...
	/*
		DrillUp.drill_COGM_initStyle 提供了部分数据库设置的样式数据，
...
Drill_COGM_MeterSprite.prototype.drill_COGM_destroy_Private = function() {
	...
	this._drill_contextLayerMask = null;
	this._drill_attr_needInit = false;
...
Drill_COGM_MeterSprite.prototype.drill_initAttr = function() {
	...
	this._drill_contextLayerMask = null;						//A主体 - 内容层遮罩
	...
	// > 层级初始化
	this._drill_contextLayerMask = new Sprite();			//内容层遮罩
	...
	if( data['meter_src_mask'] != "" ){
		...
		this._drill_contextLayerMask.bitmap = bitmap;
		
		this._drill_contextLayer.addChild(this._drill_contextLayerMask);
		this._drill_contextLayer.mask = this._drill_contextLayerMask;
	}
};
//==============================
// * A主体 - 延迟初始化
...
Drill_COGM_MeterSprite.prototype.drill_COGM_sortByZIndex = function() {
   this._drill_contextLayer.children.sort(function(a, b){return a.zIndex-b.zIndex});		//内容层
   this._drill_outerLayer.children.sort(function(a, b){return a.zIndex-b.zIndex});		//外层
};
...
Drill_COGM_MeterSprite.prototype.drill_initSection = function() {
	...
	this._drill_contextLayer.addChild(this._drill_sectionUp_sprite);
	...
	this._drill_contextLayer.addChild(this._drill_sectionDown_sprite);
...
Drill_COGM_MeterSprite.prototype.drill_initLeak = function() {
	...
	this._drill_contextLayer.addChild(this._drill_leak_sprite);
...
Drill_COGM_MeterSprite.prototype.drill_delayingInitVernier = function() {
	...
	if( data['vernier_maskCover'] == true ){
		...
		this._drill_contextLayer.addChild( temp_sprite );	//（处于内容层，会被遮挡）
	}else{
		temp_sprite.zIndex = 10;
		this._drill_outerLayer.addChild( temp_sprite );		//（游标位于弹出层）
	}
}
//==============================
// * H游标 - 帧刷新
...
// ** 
// **		说明：	> 参数条专用。
// ** 				> 配置参数见 drill_resetData 函数的默认值。
// ** 				> 先初始化，再重刷。初始化时一定要赋值高宽，确定最基本的图片属性。
// ** 				> 使用的与 上段 相同的bitmap，然后进行setFrame。
//=============================================================================
//==============================
// * 弹出条 - 定义
//==============================
...
// ** 
// **		说明：	> 参数条专用。
// **				> 配置参数见 drill_initData 函数。
// **				> 初始化后即用，不需要其他函数。要有高度宽度数据。
// **				> 黑色部分和透明部分都表示遮挡。最初的状态是全遮挡的。该遮罩可以挡住弹出条。
//=============================================================================
//==============================
// * 加满遮罩 - 定义
//==============================
