...
　　var DrillUp = DrillUp || {}; 
	...
	for( var i = 0; i < DrillUp.g_LG_layers_length; i++ ){
		if( DrillUp.parameters['背景层-' + String(i+1) ] != "" ){
			...
			DrillUp.g_LG_layers[i]['inited'] = true;
		}else{
			DrillUp.g_LG_layers[i] = DrillUp.drill_LG_backgroundInit( {} );
			DrillUp.g_LG_layers[i]['id'] = Number(i)+1;
			DrillUp.g_LG_layers[i]['inited'] = false;
		}
	}
...
Game_System.prototype.initialize = function() {
	...
	this.drill_LG_initData();
};
//==============================
// ** 存储数据 - 初始化数据
//==============================
Game_System.prototype.drill_LG_initData = function() {
	...
	for(var i = 0; i< DrillUp.g_LG_layers.length ;i++){
		var data = JSON.parse(JSON.stringify( DrillUp.g_LG_layers[i] ));	//深拷贝数据
...
// * 存档文件 - 载入存档 - 数据赋值
//==============================
var _drill_LG_extractSaveContents = DataManager.extractSaveContents;
DataManager.extractSaveContents = function(contents){
	_drill_LG_extractSaveContents.call( this, contents );
	
	if( DrillUp.g_LG_saveEnabled == false ){	//（未开参数存储，则直接覆盖初始化数据）
		$gameSystem.drill_LG_initData();
...
// * 地图层级 - 参照的位移（私有）
//==============================
Scene_Map.prototype.drill_LG_layerMoveingReference_Private = function( xx, yy, reference, option ){
	...
	for(var i=0; i< sprite_tank.length; i++){
		...
		if( temp_sprite.bitmap.isReady() ){
			...
			if (temp_data['layer_index'] == "下层" ||
			    ...
			    var pos = this.drill_LG_layerMoveingReference(xx, yy, "地图参照 -> 地图参照", temp_data);
			    xx = pos['x'];
			    yy = pos['y'];
			}
			if (temp_data['layer_index'] == "图片层" ||
                temp_data['layer_index'] == "最顶层") {
			    var pos = this.drill_LG_layerMoveingReference(xx, yy, "地图参照 -> 镜头参照", temp_data);
			    xx = pos['x'];
			    yy = pos['y'];
			}
