 ...
 * @Drill_LE_var "DrillUp.g_TTG_style_length"
 * 
 * 
 * @help
 * =============================================================================
...
//				->不考虑销毁情况
//			
//			->标题平铺GIF控制器【Drill_TTG_Controller】
//			->标题平铺GIF贴图【Drill_TTG_Sprite】
//
//
//		★家谱：
//			无
...
//=============================================================================
	...
	DrillUp.g_TTG_PluginTip_baseList = [
	...
	// * 提示信息 - 报错 - 找不到样式
	//==============================
	DrillUp.drill_TTG_getPluginTip_StyleNotFind = function( style_id ){
		return "【" + DrillUp.g_TTG_PluginTip_curName + "】\n对象创建失败，id为"+style_id+"的样式配置为空或不存在。";
	};
	//==============================
	// * 提示信息 - 报错 - NaN校验值
	//==============================
	DrillUp.drill_TTG_getPluginTip_ParamIsNaN = function( param_name, check_tank ){
		var text = "【" + DrillUp.g_TTG_PluginTip_curName + "】\n检测到参数"+param_name+"出现了NaN值，请及时检查你的函数。";
		if( check_tank ){
	...
	// * 提示信息 - 报错 - 底层版本过低
	//==============================
	DrillUp.drill_TTG_getPluginTip_LowVersion = function(){
		return "【" + DrillUp.g_TTG_PluginTip_curName + "】\n游戏底层版本过低，插件基本功能无法执行。\n你可以去看\"rmmv软件版本（必看）.docx\"中的 \"旧工程升级至1.6版本\" 章节，来升级你的游戏底层版本。";
	};
	//==============================
	// * 提示信息 - 报错 - 强制更新提示
	//==============================
	DrillUp.drill_TTG_getPluginTip_NeedUpdate_Ballistics = function(){
		return "【" + DrillUp.g_TTG_PluginTip_curName + "】\n弹道核心插件版本过低，你需要更新 弹道核心 至少v2.2及以上版本。";
	};
	//==============================
	// * 提示信息 - 报错 - 控制器的非数字参数
	//==============================
	DrillUp.drill_TTG_getPluginTip_controllerData_NotId = function( class_name ){
...
//=============================================================================
	...
	DrillUp.drill_TTG_tiledGifInit = function( dataFrom ){
		...
		Drill_TTG_Controller.drill_controller_initData( data );
	...
	// * 静态数据 - 最后继承1级
	//==============================
	var _drill_TTG_scene_initialize = SceneManager.initialize;
	SceneManager.initialize = function() {
		_drill_TTG_scene_initialize.call(this);
		
		/*-----------------平铺GIF------------------*/
		for( var i = 0; i < DrillUp.g_TTG_style.length; i++ ){
			var temp = DrillUp.g_TTG_style[i];
			if( temp != undefined ){
				DrillUp.g_TTG_style[i] = DrillUp.drill_TTG_tiledGifInit( temp );
			}
		}
	}
	/*-----------------平铺GIF------------------*/
	DrillUp.g_TTG_style_length = 80;
	DrillUp.g_TTG_style = [];
	for( var i = 0; i < DrillUp.g_TTG_style_length; i++ ){
		if( DrillUp.parameters["平铺GIF-" + String(i+1) ] != undefined &&
			DrillUp.parameters["平铺GIF-" + String(i+1) ] != "" ){
			DrillUp.g_TTG_style[i] = JSON.parse(DrillUp.parameters["平铺GIF-" + String(i+1) ]);
		}else{
			DrillUp.g_TTG_style[i] = undefined;		//（设为空值，节约静态数据占用容量）
		}
	}
...
if( Imported.Drill_CoreOfBallistics &&
	...
	if( typeof(Drill_COBa_ExtendTool) == "undefined" ){	//（弹道核心版本检测）
		alert( DrillUp.drill_TTG_getPluginTip_NeedUpdate_Ballistics() );
	}
...
Game_Interpreter.prototype.pluginCommand = function( command, args ){
	...
	this.drill_TTG_pluginCommand( command, args );
}
//==============================
// * 插件指令 - 指令执行
//==============================
Game_Interpreter.prototype.drill_TTG_pluginCommand = function( command, args ){
	if( command === ">标题平铺GIF" ){
...
//==============================
var _drill_TTG_scene_initialize2 = SceneManager.initialize;
SceneManager.initialize = function() {
	_drill_TTG_scene_initialize2.call(this);
	...
	DrillUp.drill_TTG_gCheckData_visible = function(){
		for( var i = 0; i < DrillUp.g_TTG_style_length; i++ ){
			var temp_c = DrillUp.g_TTG_style[i];
...
//##############################
Scene_Title.prototype.drill_TTG_layerAddSprite = function( sprite, layer_index ){
    this.drill_TTG_layerAddSprite_Private(sprite, layer_index);
};
//##############################
// * 标题层级 - 去除贴图【标准函数】
//				
...
//##############################
Scene_Title.prototype.drill_TTG_layerRemoveSprite = function( sprite ){
	this.drill_TTG_layerRemoveSprite_Private( sprite );
};
//##############################
// * 标题层级 - 图片层级排序【标准函数】
//				
...
Scene_Title.prototype.drill_TTG_sortByZIndex = function () {
    this.drill_TTG_sortByZIndex_Private();
};
//=============================================================================
// ** 标题层级（接口实现）
//=============================================================================
...
// * 标题层级 - 层级初始化1
//==============================
var _drill_TTG_titleLayer_createBackground = Scene_Title.prototype.createBackground;
Scene_Title.prototype.createBackground = function() {
	_drill_TTG_titleLayer_createBackground.call(this);
...
// * 标题层级 - 层级初始化2
//==============================
var _drill_TTG_titleLayer_update = Scene_Title.prototype.update;
Scene_Title.prototype.update = function() {
	_drill_TTG_titleLayer_update.call(this);
...
// * 标题层级 - 图片层级排序（私有）
//==============================
Scene_Title.prototype.drill_TTG_sortByZIndex_Private = function() {
...
// * 标题层级 - 去除贴图（私有）
//==============================
Scene_Title.prototype.drill_TTG_layerRemoveSprite_Private = function( sprite ){
...
// * 标题层级 - 添加贴图到层级（私有）
//==============================
Scene_Title.prototype.drill_TTG_layerAddSprite_Private = function( sprite, layer_index ){
...
if( Utils.generateRuntimeId == undefined ){
	alert( DrillUp.drill_TTG_getPluginTip_LowVersion() );
}
//==============================
// * 预加载 - 最后继承1级
//
...
//==============================
var _drill_TTG_scene_initialize3 = SceneManager.initialize;
SceneManager.initialize = function() {
	_drill_TTG_scene_initialize3.call(this);
	
	// > 执行资源预加载
	DrillUp.drill_TTG_preloadInit();
}
//==============================
// * 预加载 - 执行资源预加载
//
...
//==============================
DrillUp.drill_TTG_preloadInit = function(){
	this._drill_TTG_cacheId = Utils.generateRuntimeId();	//资源缓存id
	...
	for( var i = 0; i < DrillUp.g_TTG_style.length; i++ ){
		...
		if( temp_data['preloadType'] == "全部预加载" ){
			...
			for( var k = 0; k < src_img_num; k++ ){
				this._drill_TTG_preloadTank.push( 
					ImageManager.reserveBitmap( temp_data['src_img_file'], temp_data['src_img_gif'][k], temp_data['tint'], temp_data['smooth'], this._drill_TTG_cacheId ) 
				);
			}
		}
...
Scene_Title.prototype.createBackground = function() {
	...
	SceneManager._drill_TTG_created = false;
   	...
   	this._drill_TTG_controllerTank = [];
	
	// > 原函数
	_drill_TTG_createBackground.call(this);
};
//==============================
// * 贴图创建标记 - 退出界面
//==============================
...
Scene_Title.prototype.terminate = function() {
	_drill_TTG_terminate.call(this);
	SceneManager._drill_TTG_created = false;	//（下次进入界面需重新创建）
};
//==============================
// * 贴图创建标记 - 帧刷新
//==============================
...
Scene_Title.prototype.update = function() {
	...
	if( SceneManager.isCurrentSceneStarted() && 
		...
		this.drill_TTG_create();
	}
	// > 帧刷新
	if( SceneManager._drill_TTG_created == true ){
...
Scene_Title.prototype.drill_TTG_create = function() {	
	...
	if( this._drill_TTG_controllerTank == undefined ){
		this._drill_TTG_controllerTank = [];
	}
	...
	for( var i = 0; i < DrillUp.g_TTG_style.length; i++ ){
		...
		// > 创建控制器
		var temp_controller = new Drill_TTG_Controller( i );
		this._drill_TTG_controllerTank.push( temp_controller );
		
		// > 创建贴图
		var temp_sprite = new Drill_TTG_Sprite();
		...
		this.drill_TTG_layerAddSprite( temp_sprite, temp_data['layerIndex'] );
	}
...
Scene_Title.prototype.drill_TTG_update = function() {
	this.drill_TTG_updateController();
	this.drill_TTG_updateAttr();
}
//==============================
// * 贴图控制 - 帧刷新 控制器
//==============================
Scene_Title.prototype.drill_TTG_updateController = function(){
	for(var i = 0; i < this._drill_TTG_controllerTank.length; i++ ){
		var temp_controller = this._drill_TTG_controllerTank[i];
...
// * 贴图控制 - 帧刷新 基础特性
//==============================
Scene_Title.prototype.drill_TTG_updateAttr = function(){
	...
	for(var i = 0; i < this._drill_TTG_spriteTank.length; i++){
		...
		if( temp_sprite.layerIndex != temp_controller._drill_layerIndex ){
			...
			this.drill_TTG_layerAddSprite( temp_sprite, temp_controller._drill_layerIndex );
...
//##############################
Drill_TTG_Controller.prototype.drill_controller_resetData = function( data_id ){
...
//			说明：	> 可放在帧刷新函数中实时调用。
//##############################
Drill_TTG_Controller.prototype.drill_controller_setVisible = function( visible ){
...
//			说明：	> 可放在帧刷新函数中实时调用。
//##############################
Drill_TTG_Controller.prototype.drill_controller_setPause = function( pause ){
...
//			返回：	> 布尔
//##############################
Drill_TTG_Controller.prototype.drill_controller_destroy = function(){
...
//			返回：	> 无
//##############################
Drill_TTG_Controller.prototype.drill_controller_setBlendMode = function( blendMode ){
...
//			返回：	> 无
//##############################
Drill_TTG_Controller.prototype.drill_controller_setLayerIndex = function( layerIndex ){
...
//			说明：	> 从帧数0开始计数。
//##############################
Drill_TTG_Controller.prototype.drill_controller_GIF_setFrame = function( cur_frame ){
...
//			返回：	> 无
//##############################
Drill_TTG_Controller.prototype.drill_controller_GIF_setLocked = function( locked ){
...
//			返回：	> 无
//##############################
Drill_TTG_Controller.prototype.drill_controller_GIF_setOncePlay = function( once_type ){
	var data = this.drill_data();
...
//##############################
Drill_TTG_Controller.drill_controller_initData = function( data ){
...
// * 控制器 - 初始化子功能『控制器与贴图』
//==============================
Drill_TTG_Controller.prototype.drill_controller_initChild = function(){
...
//			说明：	data对象中的参数【可以缺项】。
//==============================
Drill_TTG_Controller.prototype.drill_controller_resetData_Private = function( data_id ){
	
	// > 参数检查
	if( typeof data_id != "number" ){
		alert( DrillUp.drill_TTG_getPluginTip_controllerData_NotId("Drill_TTG_Controller") );
		throw Error( DrillUp.drill_TTG_getPluginTip_controllerData_NotId("Drill_TTG_Controller") );
		return;
	}
...
//##############################
DrillUp.g_TTG_notFindStyleAlertOnce = true;
//##############################
// * 控制器 - 『控制器与贴图的样式-静态数据』空的静态数据
//			
//			说明：	> 空数据会在initData时会进行默认值初始化，在其他地方只读。
//##############################
Drill_TTG_Controller.emptyData = {};
//##############################
...
//##############################
Drill_TTG_Controller.prototype.drill_data = function(){
	var cur_styleId   = this._drill_data_id +1;
	var cur_styleData = DrillUp.g_TTG_style[ this._drill_data_id ];
	if( cur_styleData == undefined ){
		
		// > 『控制器与贴图的样式-静态数据』 - 校验+提示信息（只执行一次）
		if( DrillUp.g_TTG_notFindStyleAlertOnce == true ){
			DrillUp.g_TTG_notFindStyleAlertOnce = false;
			alert( DrillUp.drill_TTG_getPluginTip_StyleNotFind(cur_styleId) );
		}
		return Drill_TTG_Controller.emptyData;
	}
...
// * A主体 - 初始化子功能
//==============================
Drill_TTG_Controller.prototype.drill_controller_initAttr = function(){
	var data = this.drill_data();
...
// * B变换特性 - 初始化子功能
//==============================
Drill_TTG_Controller.prototype.drill_controller_initChange = function(){
	var data = this.drill_data();
...
// * B变换特性 - 帧刷新 位置
//==============================
Drill_TTG_Controller.prototype.drill_controller_updateChange_Position = function(){
...
//##############################
Drill_TTG_Controller.prototype.drill_controller_finalTransform_x = function(){
	return this._drill_change_x + this._drill_change_selfXAcc + this._drill_move_originOffsetX;
}
//##############################
// * B变换特性 - 数据最终变换值 - 位置Y（可继承，开放函数）
//##############################
Drill_TTG_Controller.prototype.drill_controller_finalTransform_y = function(){
	return this._drill_change_y + this._drill_change_selfYAcc + this._drill_move_originOffsetY;
}
//##############################
// * B变换特性 - 数据最终变换值 - 移动速度X（可继承，开放函数）
//##############################
Drill_TTG_Controller.prototype.drill_controller_finalTransform_selfXSpeed = function(){
	return this._drill_change_selfXSpeed;
}
//##############################
// * B变换特性 - 数据最终变换值 - 移动速度Y（可继承，开放函数）
//##############################
Drill_TTG_Controller.prototype.drill_controller_finalTransform_selfYSpeed = function(){
	return this._drill_change_selfYSpeed;
}
//##############################
// * B变换特性 - 数据最终变换值 - 缩放X（可继承，开放函数）
//##############################
Drill_TTG_Controller.prototype.drill_controller_finalTransform_scaleX = function(){
	return this._drill_change_scaleX;
}
//##############################
// * B变换特性 - 数据最终变换值 - 缩放Y（可继承，开放函数）
//##############################
Drill_TTG_Controller.prototype.drill_controller_finalTransform_scaleY = function(){
	return this._drill_change_scaleY;
}
//##############################
// * B变换特性 - 数据最终变换值 - 透明度（可继承，开放函数）
//##############################
Drill_TTG_Controller.prototype.drill_controller_finalTransform_opacity = function(){
	return this._drill_change_opacity;
}
//##############################
// * B变换特性 - 数据最终变换值 - 斜切X（可继承，开放函数）
//##############################
Drill_TTG_Controller.prototype.drill_controller_finalTransform_skewX = function(){
	return this._drill_change_skewX;
}
//##############################
// * B变换特性 - 数据最终变换值 - 斜切Y（可继承，开放函数）
//##############################
Drill_TTG_Controller.prototype.drill_controller_finalTransform_skewY = function(){
	return this._drill_change_skewY;
}
//##############################
// * B变换特性 - 数据最终变换值 - 旋转（可继承，开放函数）
//##############################
Drill_TTG_Controller.prototype.drill_controller_finalTransform_rotate = function(){
	return this._drill_change_rotate;
}
//==============================
// * B变换特性 - 帧刷新 平铺范围
//==============================
Drill_TTG_Controller.prototype.drill_controller_updateChange_MoveRange = function(){
	...
	var ww = Graphics.boxWidth;
	var hh = Graphics.boxHeight;
	var rect = $gameTemp.drill_TTG_Math2D_getRectWithTransform(
		0, 0,
		ww, hh,
		0.5, 0.5,
...
//==============================
Game_Temp.prototype.drill_TTG_Math2D_getRectWithTransform = function( 
					rect_x,rect_y,						//矩形位置 
					rect_w,rect_h, 						//矩形宽高 
	...
	// > 变换后的四个点
	var pp1 = $gameTemp.drill_TTG_Math2D_getPointWithTransform( p1_x,p1_y, c_x,c_y, rotation, scale_x,scale_y, skew_x,skew_y );
	var pp2 = $gameTemp.drill_TTG_Math2D_getPointWithTransform( p2_x,p2_y, c_x,c_y, rotation, scale_x,scale_y, skew_x,skew_y );
	var pp3 = $gameTemp.drill_TTG_Math2D_getPointWithTransform( p3_x,p3_y, c_x,c_y, rotation, scale_x,scale_y, skew_x,skew_y );
	var pp4 = $gameTemp.drill_TTG_Math2D_getPointWithTransform( p4_x,p4_y, c_x,c_y, rotation, scale_x,scale_y, skew_x,skew_y );
...
//					  旋转+缩放+斜切 可为负数。
//==============================
Game_Temp.prototype.drill_TTG_Math2D_getPointWithTransform = function( 
					cur_x,cur_y,						//需要变换的点 
...
// * C校验值 - 初始化子功能
//==============================
Drill_TTG_Controller.prototype.drill_controller_initCheck = function(){
	//（无）
}
//==============================
// * C校验值 - 校验标记
//==============================
DrillUp.g_TTG_checkNaN = true;
//==============================
// * C校验值 - 帧刷新
//==============================
Drill_TTG_Controller.prototype.drill_controller_updateCheckNaN = function(){
	if( $gameTemp == undefined ){ return; }		//（测试版开启功能，发布版关闭功能）
	if( $gameTemp.isPlaytest() != true ){ return; }
	
	// > 校验值
	if( DrillUp.g_TTG_checkNaN == true ){
		if( isNaN( this.drill_controller_finalTransform_x() ) ){
			DrillUp.g_TTG_checkNaN = false;
			var check_tank = {};
			...
			alert( DrillUp.drill_TTG_getPluginTip_ParamIsNaN( "drill_controller_finalTransform_x", check_tank ) );
		}
		if( isNaN( this.drill_controller_finalTransform_y() ) ){
			...
			alert( DrillUp.drill_TTG_getPluginTip_ParamIsNaN( "drill_controller_finalTransform_y", check_tank ) );
		}
		if( isNaN( this.drill_controller_finalTransform_opacity() ) ){
			DrillUp.g_TTG_checkNaN = false;
			alert( DrillUp.drill_TTG_getPluginTip_ParamIsNaN( "drill_controller_finalTransform_opacity" ) );
		}
		if( isNaN( this.drill_controller_finalTransform_rotate() ) ){
			DrillUp.g_TTG_checkNaN = false;
			alert( DrillUp.drill_TTG_getPluginTip_ParamIsNaN( "drill_controller_finalTransform_rotate" ) );
		}
	}
}
...
//==============================
Drill_TTG_Controller.prototype.drill_controller_initCommandChange = function(){
...
// * D指令叠加变化 - 帧刷新
//==============================
Drill_TTG_Controller.prototype.drill_controller_updateCommandChange = function(){
...
// * D指令叠加变化 - 立即还原所有单属性
//==============================
Drill_TTG_Controller.prototype.drill_controller_commandChange_restoreAttr = function(){
...
// * D指令叠加变化 - 立即归位
//==============================
Drill_TTG_Controller.prototype.drill_controller_commandChange_restoreMove = function(){
	this["_drill_command_move_data"] = undefined;
}
//==============================
// * D指令叠加变化 - 修改单属性 - 移动到
//==============================
Drill_TTG_Controller.prototype.drill_controller_commandChange_setMove = function( change_type, tar_valueA, tar_valueB, tar_time ){
	var data = this.drill_data();
...
// * D指令叠加变化 - 修改单属性 - 透明度
//==============================
Drill_TTG_Controller.prototype.drill_controller_commandChange_setOpacity = function( change_type, tar_value, tar_time ){
	var data = this.drill_data();
...
// * D指令叠加变化 - 修改单属性 - 移动速度X
//==============================
Drill_TTG_Controller.prototype.drill_controller_commandChange_setSpeedX = function( change_type, tar_value, tar_time ){
	var data = this.drill_data();
	Drill_COBa_ExtendTool.drill_COBa_Common_controller_setTarget(
...
// * D指令叠加变化 - 修改单属性 - 移动速度Y
//==============================
Drill_TTG_Controller.prototype.drill_controller_commandChange_setSpeedY = function( change_type, tar_value, tar_time ){
	var data = this.drill_data();
	Drill_COBa_ExtendTool.drill_COBa_Common_controller_setTarget(
...
// * D指令叠加变化 - 修改单属性 - 旋转
//==============================
Drill_TTG_Controller.prototype.drill_controller_commandChange_setRotate = function( change_type, tar_value, tar_time ){
	var data = this.drill_data();
	Drill_COBa_ExtendTool.drill_COBa_Common_controller_setTarget(
...
// * D指令叠加变化 - 修改单属性 - 缩放X
//==============================
Drill_TTG_Controller.prototype.drill_controller_commandChange_setScaleX = function( change_type, tar_value, tar_time ){
	var data = this.drill_data();
	Drill_COBa_ExtendTool.drill_COBa_Common_controller_setTarget(
...
// * D指令叠加变化 - 修改单属性 - 缩放Y
//==============================
Drill_TTG_Controller.prototype.drill_controller_commandChange_setScaleY = function( change_type, tar_value, tar_time ){
	var data = this.drill_data();
	Drill_COBa_ExtendTool.drill_COBa_Common_controller_setTarget(
...
// * D指令叠加变化 - 修改单属性 - 斜切X
//==============================
Drill_TTG_Controller.prototype.drill_controller_commandChange_setSkewX = function( change_type, tar_value, tar_time ){
	var data = this.drill_data();
	Drill_COBa_ExtendTool.drill_COBa_Common_controller_setTarget(
...
// * D指令叠加变化 - 修改单属性 - 斜切Y
//==============================
Drill_TTG_Controller.prototype.drill_controller_commandChange_setSkewY = function( change_type, tar_value, tar_time ){
	var data = this.drill_data();
	Drill_COBa_ExtendTool.drill_COBa_Common_controller_setTarget(
...
// * E延迟指令 - 初始化子功能
//==============================
Drill_TTG_Controller.prototype.drill_controller_initDelayingCommand = function(){
...
//==============================
Drill_TTG_Controller.prototype.drill_controller_updateDelayingCommandImportant = function(){
...
// * E延迟指令 - 帧刷新 - 执行延迟指令
//==============================
Drill_TTG_Controller.prototype.drill_controller_updateDelayingCommand = function(){
	if( this._drill_curDelayingCommandTank.length == 0 ){ return; }
...
// * E延迟指令 - 清空全部（开放函数）
//==============================
Drill_TTG_Controller.prototype.drill_controller_clearDelayingCommand = function(){
	this._drill_curDelayingCommandTank = [];
}
...
// * F周期指令 - 帧刷新 - 执行周期指令
//==============================
Drill_TTG_Controller.prototype.drill_controller_updatePeriodizeCommand = function(){
	if( this._drill_curPeriodizeCommandTank.length == 0 ){ return; }
...
// * G自变化效果 - 初始化子功能
//==============================
Drill_TTG_Controller.prototype.drill_controller_initEffect = function(){
	this._drill_curEffectTime = 0;
}
//==============================
// * G自变化效果 - 帧刷新
//==============================
Drill_TTG_Controller.prototype.drill_controller_updateEffect = function(){
...
// * O播放GIF - 初始化子功能
//==============================
Drill_TTG_Controller.prototype.drill_controller_initGIF = function(){
	var data = this.drill_data();
...
// * O播放GIF - 帧刷新
//==============================
Drill_TTG_Controller.prototype.drill_controller_updateGIF = function(){
	var data = this.drill_data();
...
// * P预加载GIF管理 - 初始化子功能
//==============================
Drill_TTG_Controller.prototype.drill_controller_initPreload = function(){
	//	（无）
}
...
// * 平铺GIF贴图 - 定义
//==============================
function Drill_TTG_Sprite() {
    this.initialize.apply(this, arguments);
};
Drill_TTG_Sprite.prototype = Object.create(Sprite.prototype);
Drill_TTG_Sprite.prototype.constructor = Drill_TTG_Sprite;
//==============================
// * 平铺GIF贴图 - 初始化
//==============================
Drill_TTG_Sprite.prototype.initialize = function(){
...
// * 平铺GIF贴图 - 帧刷新
//==============================
Drill_TTG_Sprite.prototype.update = function(){
...
//##############################
Drill_TTG_Sprite.prototype.drill_sprite_setController = function( controller ){
...
//##############################
Drill_TTG_Sprite.prototype.drill_sprite_initChild = function(){
...
//			说明：	> 这里完全 不考虑 延迟加载问题。
//##############################
Drill_TTG_Sprite.prototype.drill_sprite_isReady = function(){
...
//##############################
Drill_TTG_Sprite.prototype.drill_sprite_isOptimizationPassed = function(){
    return true;
};
//##############################
// * 平铺GIF贴图 - 是否需要销毁【标准函数】
...
//##############################
Drill_TTG_Sprite.prototype.drill_sprite_isNeedDestroy = function(){
...
//##############################
Drill_TTG_Sprite.prototype.drill_sprite_destroy = function(){
...
//==============================
Drill_TTG_Sprite.prototype.drill_sprite_initSelf = function(){
...
//==============================
Drill_TTG_Sprite.prototype.drill_sprite_destroyChild = function(){
...
// * 平铺GIF贴图 - 销毁自身『控制器与贴图』
//==============================
Drill_TTG_Sprite.prototype.drill_sprite_destroySelf = function(){
	this._drill_controller = null;				//控制器对象
	this._drill_curSerial = -1;					//当前序列号
};
...
// * A贴图主体 - 初始化子功能
//==============================
Drill_TTG_Sprite.prototype.drill_sprite_initAttr = function(){
...
Drill_TTG_Sprite.prototype.drill_sprite_updateAttr = function(){
	...
	//（其它 基础特性 的帧刷新赋值见：drill_TTG_updateAttr）
}
...
// * B变换特性 - 帧刷新
//==============================
Drill_TTG_Sprite.prototype.drill_sprite_updateChange = function(){
...
//					> 参数使用字符串进行控制，默认为 null 值。
//==============================
Drill_TTG_Sprite.prototype.drill_sprite_initCommandChange = function(){
...
// * D指令叠加变化-控制器用 - 帧刷新
//==============================
Drill_TTG_Sprite.prototype.drill_sprite_updateCommandChange = function(){
	var data = this._drill_controller.drill_data();
...
// * E延迟指令 - 初始化子功能
//==============================
Drill_TTG_Sprite.prototype.drill_sprite_initDelayingCommand = function(){
	//（无）
}
...
// * F周期指令 - 初始化子功能
//==============================
Drill_TTG_Sprite.prototype.drill_sprite_initPeriodizeCommand = function(){
	//（无）
}
...
// * G自变化效果 - 初始化子功能
//==============================
Drill_TTG_Sprite.prototype.drill_sprite_initEffect = function(){
	//（无）
}
//==============================
// * G自变化效果 - 帧刷新
//==============================
Drill_TTG_Sprite.prototype.drill_sprite_updateEffect = function(){
	var data = this._drill_controller.drill_data();
...
// * O播放GIF - 初始化子功能
//==============================
Drill_TTG_Sprite.prototype.drill_sprite_initGIF = function(){
	//	（无）
}
//==============================
// * O播放GIF - 帧刷新
//==============================
Drill_TTG_Sprite.prototype.drill_sprite_updateGIF = function(){
...
// * O播放GIF - 获取资源bitmap
//==============================
Drill_TTG_Sprite.prototype.drill_sprite_getGIFBitmapByIndex = function( index ){
...
//==============================
Drill_TTG_Sprite.prototype.drill_sprite_initPreload = function(){
	this._drill_load_curTime = 0;			//当前加载时间
}
//==============================
// * P预加载GIF管理 - 帧刷新
//==============================
Drill_TTG_Sprite.prototype.drill_sprite_updatePreload = function(){
	if( this.visible != true ){ return; }	//（显示时才帧刷新）
	var data = this._drill_controller.drill_data();
...
//==============================
Drill_TTG_Sprite.prototype.drill_sprite_getReadyTank_ForDebug = function(){
	var data = this._drill_controller.drill_data();
	var isReady_tank = [];
