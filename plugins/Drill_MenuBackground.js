...
//=============================================================================
	...
	DrillUp.drill_MBa_backgroundDefaultInit = function( dataFrom ){
		...
		data['pause'] = false;
		data['blendMode'] = Number( dataFrom["混合模式"] || 0);
		data['layerIndex'] = "菜单后面层";		//（默认背景 固定 层级）
		data['zIndex'] = Number( dataFrom["图片层级"] || 0);
...
Scene_MenuBase.prototype.drill_MBa_create = function() {	
	...
	if( this._drill_MBa_dataTank == undefined ){
	...
	for (var i = 1; i < DrillUp.g_MBa_style.length; i++) {
		...
		if( this.drill_MBa_checkKeyword( temp_data ) ){
			...
			// > 添加贴图到层级
			this.drill_MBa_layerAddSprite( temp_layer, temp_sprite_data['layerIndex'] );
		}
	}
...
Drill_MBa_Controller.prototype.drill_controller_updateAttr = function(){
	...
	// > 显示/隐藏 设置
	this._drill_visible = $gameSystem._drill_MBa_visible[ this._drill_data_id ];
...
// * F周期指令 - 清空全部（开放函数）
//==============================
Drill_MBa_Controller.prototype.drill_controller_clearPeriodizeCommand = function(){
	this._drill_curPeriodizeCommandTank = [];
}
