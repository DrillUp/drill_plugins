 ...
 * 定义：
 *   (1.标签：指 状态元/状态节点/动作元 中添加的自定义标签。任意文本格式。
 *   (2.全标签：指特定格式的一组专用标签。
 *   (3.标签播放：指 通过标签控制 播放 状态元/状态节点/动作元。
 *      需要手动通过插件指令调用，才能实现标签播放。
 *   (4.全标签播放：指一套固定机制的专用标签，通过那些标签自动播放 状态元/状态节点/动作元。
 *      通过插件指令或事件注释 开启 全标签开关，即可自动进行全标签播放。
 * 全标签播放：
 *   (1.全标签包含<行走图-静止>、<行走图-移动>、<行走图-跳跃>、<行走图-奔跑>等，
 *      具体去看看文档中 全标签播放 的章节。
