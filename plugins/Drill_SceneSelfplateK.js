...
//=============================================================================
	...
	// * 提示信息 - 报错 - 窗口字符底层校验
	//==============================
	DrillUp.drill_SSpK_getPluginTip_NeedUpdate_drawText = function(){
...
//=============================================================================
	...
	/*-----------------面板跳转------------------*/
	DrillUp.g_SSpK_add_to_menu = String(DrillUp.parameters["是否在主菜单窗口中显示"] || "true") === "true";	
    DrillUp.g_SSpK_menu_name = String(DrillUp.parameters["主菜单窗口显示名"] || "");
	...
	DrillUp.g_SSpK_locked_name = DrillUp.g_SSpK_locked_name.replace(/\\\\/g,"\\");	//（为了支持\\n的写法）
	if( DrillUp.parameters["用语-锁定的描述内容"] != undefined && 
		DrillUp.parameters["用语-锁定的描述内容"] != "" ){
		DrillUp.g_SSpK_locked_context = JSON.parse( DrillUp.parameters["用语-锁定的描述内容"] );
	}else{
		if( DrillUp.parameters["用语-锁定的选项内容"] != undefined &&	//（旧名称）
			DrillUp.parameters["用语-锁定的选项内容"] != "" ){
			DrillUp.g_SSpK_locked_context = JSON.parse( DrillUp.parameters["用语-锁定的选项内容"] );
		}else{
			DrillUp.g_SSpK_locked_context = "";
		}
	}
	...
	/*-----------------未读提示------------------*/
	DrillUp.g_SSpK_watch_enabled = String(DrillUp.parameters["是否启用未读提示"] || "true") === "true";	
	DrillUp.g_SSpK_watch_fontsize = Number(DrillUp.parameters["未读提示字体大小"] || 20);
	DrillUp.g_SSpK_watch_setCorner = String(DrillUp.parameters["未读提示是否放右下角"] || "true") == "true";
	if( DrillUp.parameters["用语-未读提示信息"] != undefined && 
		DrillUp.parameters["用语-未读提示信息"] != "" ){
		DrillUp.g_SSpK_watch_text = JSON.parse( DrillUp.parameters["用语-未读提示信息"] );
	}else{
		DrillUp.g_SSpK_watch_text = "";
	}
...
Game_System.prototype.drill_SSpK_initSysData_Private = function() {
	...
	this._drill_SSpK_watchedTank = [];				//已读情况
	for( var i = 0; i < DrillUp.g_SSpK_context_list.length; i++ ){
		...
		this._drill_SSpK_watchedTank[i] = temp_data['watched'];
	}
};
//==============================
// * 存储数据 - 载入存档时检查数据（私有）
...
Game_System.prototype.drill_SSpK_checkSysData_Private = function() {
	...
	for( var i = 0; i < DrillUp.g_SSpK_context_list.length; i++ ){
		...
		if( temp_data != undefined ){
			...
			// > 未存储的，重新初始化
			if( this._drill_SSpK_watchedTank[i] == undefined ){
				this._drill_SSpK_watchedTank[i] = temp_data['watched'];
...
Game_Interpreter.prototype.pluginCommand = function( command, args ){
	...
	this.drill_SSpK_pluginCommand( command, args );
}
//==============================
// * 插件指令 - 指令执行
//==============================
Game_Interpreter.prototype.drill_SSpK_pluginCommand = function( command, args ){
	if( command === ">信息面板K" ){
		...
		if( args.length == 4 ){
			...
			if( type == "显示选项" ){
				$gameSystem._drill_SSpK_enableTank[ Number(temp1)-1 ] = true;		//正常存储
			}
			if( type == "隐藏选项" ){
				$gameSystem._drill_SSpK_enableTank[ Number(temp1)-1 ] = false;		//正常存储
			}
			if( type == "锁定选项" ){
				$gameSystem._drill_SSpK_lockTank[ Number(temp1)-1 ] = true;			//正常存储
			}
			if( type == "解锁选项" ){
				$gameSystem._drill_SSpK_lockTank[ Number(temp1)-1 ] = false;		//正常存储
			}
...
Game_Temp.prototype.initialize = function() {	
	...
	this._drill_SSpK_visibleContextDataList = [];
};
//==============================
// * 面板控制 - 判断 锁定情况（开放函数）
//==============================
...
// * 面板控制 - 判断 已读情况
//==============================
Game_Temp.prototype.drill_SSpK_isWatched = function( context_realIndex ){
	
	// > 正常存储控制
	if( $gameSystem._drill_SSpK_watchedTank[ context_realIndex ] == true ){
		return true;
	}else{
		return false;
	}
...
// * A主体 - 创建
//==============================
Scene_Drill_SSpK.prototype.drill_createAttr = function() {
...
// * B选项窗口 - 帧刷新
//==============================
Scene_Drill_SSpK.prototype.drill_updateSelect = function() {
	
	// > 初始化 - 插件指令选中
	var max_index = $gameTemp._drill_SSpK_visibleContextDataList.length -1;
	if( $gameSystem._drill_SSpK_context_index != undefined ){
		var index = $gameSystem._drill_SSpK_context_index;
		if( index < 0 ){ index = 0; };
...
// * B选项窗口 - 帧刷新 - 选项变化时
//==============================
Scene_Drill_SSpK.prototype.drill_updateSelectChanged = function() {
	if( $gameTemp._drill_SSpK_visibleContextDataList.length == 0 ){ return };
...
// * C描述窗口 - 帧刷新
//==============================
Scene_Drill_SSpK.prototype.drill_updateDesc = function() {
	//（不操作，动画自己会帧刷新）
};
...
Scene_Drill_SSpK.prototype.drill_refreshDescPic = function( cur_index ){
	...
	if( src_tank.length == 0 ){
		...
		for( var i = 0; i < $gameTemp._drill_SSpK_visibleContextDataList.length; i++ ){
			var temp_data = $gameTemp._drill_SSpK_visibleContextDataList[i];
	...
	// > 切换描述图
	var temp_curData = $gameTemp._drill_SSpK_visibleContextDataList[ cur_index ];	//（当前选项）
	if( $gameTemp.drill_SSpK_isLocked( temp_curData['index'] ) == true && 
...
Scene_Drill_SSpK.prototype.drill_processOk = function() {
	...
	// > 获取选中的内容
	var temp_context = $gameTemp._drill_SSpK_visibleContextDataList[ this._window_select.index() ];
...
//==============================
// * 信息面板K（菜单界面基类） - 当前角色切换时
//==============================
Scene_Drill_SSpK.prototype.onActorChange = function() {
...
Drill_SSpK_SelectWindow.prototype.refresh = function() {
	...
	// > 重刷可见项列表
	$gameTemp._drill_SSpK_visibleContextDataList = [];
	...
	for( var i = 0; i < DrillUp.g_SSpK_context_list.length; i++ ){
		...
		if( $gameTemp.drill_SSpK_isEnabled( i ) == true ){
			$gameTemp._drill_SSpK_visibleContextDataList.push( temp_data );
	...
	for( var j = 0; j < $gameTemp._drill_SSpK_visibleContextDataList.length ;j++ ){
		var temp_data = $gameTemp._drill_SSpK_visibleContextDataList[j];
		var context_realIndex = temp_data['index'];
		
		// > 选项锁定
		if( $gameTemp.drill_SSpK_isLocked( context_realIndex ) == true ){
			this._drill_selectionNameList[j] = String(DrillUp.g_SSpK_locked_name);
			continue;
		}
...
// * 2A选中 - 参数初始化
//==============================
Drill_SSpK_SelectWindow.g_drill_lastTopRow = 0;
Drill_SSpK_SelectWindow.g_drill_lastIndex  = 0;
//==============================
// * 2A选中 - 设置选项
//==============================
Drill_SSpK_SelectWindow.prototype.drill_window_initSelect = function() {
	if( Drill_SSpK_SelectWindow.g_drill_lastIndex >= this._drill_selectionNameList.length ){
		Drill_SSpK_SelectWindow.g_drill_lastIndex = this._drill_selectionNameList.length-1;
	}
	this.setTopRow(Drill_SSpK_SelectWindow.g_drill_lastTopRow);
	this.select(Drill_SSpK_SelectWindow.g_drill_lastIndex);
}
//==============================
// * 2A选中 - 退出时暂存选项（继承）
//==============================
Drill_SSpK_SelectWindow.prototype.processCancel = function() {
	...
	Drill_SSpK_SelectWindow.g_drill_lastTopRow = this.topRow();
	Drill_SSpK_SelectWindow.g_drill_lastIndex = this.index();
};
...
//==============================
Drill_SSpK_SelectWindow.prototype.drill_window_drawAllItem = function(){
...
//==============================
Drill_SSpK_SelectWindow.prototype.drill_window_drawItem = function( index ){
	...
	// > 参数准备 - 窗口字符对齐方式
	var cur_align = DrillUp.g_SSpK_command_window['align'];
...
// * 2C已读情况 - 绘制新标签
//==============================
Drill_SSpK_SelectWindow.prototype.drill_window_drawNewTag = function( index ){
	if( DrillUp.g_SSpK_watch_enabled != true ){ return; }
	
	var temp_data = $gameTemp._drill_SSpK_visibleContextDataList[index];	//（当前选项）
	if( temp_data == undefined ){ return; }
	var rect = this.itemRectForText(index);
	var context_realIndex = temp_data['index'];
	
	// > 已读则跳出
	if( $gameTemp.drill_SSpK_isWatched( context_realIndex ) == true ){ return; }
	// > 锁定则跳出
	if( $gameTemp.drill_SSpK_isLocked( context_realIndex ) == true ){ return; }
	
	
	// > 参数准备 - 校验
	var temp_bitmap = this.contents;
	if( temp_bitmap == undefined ){ return; }
	var org_text = DrillUp.g_SSpK_watch_text;
	if( org_text == undefined ){ return; }
	if( org_text == "" ){ return; }
	...
	options['rowParam']['alignHor_maxWidth'] = rect.width;		//『选项的文本域范围』
	options['rowParam']['alignVer_maxHeight'] = rect.height;	//
	if( DrillUp.g_SSpK_watch_setCorner == true ){
		options['rowParam']['alignHor_type'] = "right";	//（右下角）
		options['rowParam']['alignVer_type'] = "bottom";
	}
	
	options['baseParam'] = {};
	options['baseParam']['fontSize'] = DrillUp.g_SSpK_watch_fontsize;
...
//==============================
Drill_SSpK_SelectWindow.prototype.drill_window_watchedChanged = function( last_index ){
	if( DrillUp.g_SSpK_watch_enabled != true ){ return; }
	
	var temp_data = $gameTemp._drill_SSpK_visibleContextDataList[ last_index ];	//（当前选项）
	if( temp_data == undefined ){ return; }
	var context_realIndex = temp_data['index'];
	...
	// > 新标签 数据变化
	if( $gameTemp.drill_SSpK_isWatched( context_realIndex ) != true ){
		$gameSystem._drill_SSpK_watchedTank[ context_realIndex ] = true;
...
// * 显示窗口 - 重绘内容
//==============================
Drill_SSpK_DescWindow.prototype.drill_refreshDescText = function( cur_index ){
	var temp_data = $gameTemp._drill_SSpK_visibleContextDataList[ cur_index ];	//（当前选项）
	if( temp_data == undefined ){ return; }
	
	// > 切换描述内容
	var context = "";
	if( $gameTemp.drill_SSpK_isLocked( temp_data['index'] ) == true && 
		(DrillUp.g_SSpK_locked_type == "锁定描述图和描述内容" || 
		 DrillUp.g_SSpK_locked_type == "只锁定描述内容" ) ){
		if( temp_data['lockedContextEnabled'] == true ){
			context = temp_data['lockedContext'];		//自定义的锁定时描述内容
		}else{
			context = DrillUp.g_SSpK_locked_context;	//用语-锁定的描述内容
		}
	}else{
...
if( typeof(_drill_COWC_drawText_functionExist) == "undefined" ){
	alert( DrillUp.drill_SSpK_getPluginTip_NeedUpdate_drawText() );
}
