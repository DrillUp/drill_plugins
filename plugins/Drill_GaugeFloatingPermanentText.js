 ...
 * 你可以通过插件指令，设置定时重绘：
 * 
 * 插件指令：>地图永久漂浮文字 : 漂浮文字[1] : 开启定时重绘
 * 插件指令：>地图永久漂浮文字 : 漂浮文字[1] : 关闭定时重绘
 * 插件指令：>地图永久漂浮文字 : 漂浮文字[1] : 设置定时重绘 : 间隔时间[60]
 * 
 ...
 * （注意，冒号左右都有一个空格）
 * 
 * 插件指令：>地图永久漂浮文字 : 漂浮文字[1] : 启用逐个绘制
 * 插件指令：>地图永久漂浮文字 : 漂浮文字[1] : 关闭逐个绘制
 * 
 * 插件指令：>地图永久漂浮文字 : 漂浮文字[1] : 修改逐个绘制设置 : 间隔时间[4]
 * 插件指令：>地图永久漂浮文字 : 漂浮文字[1] : 重新逐个绘制
 * 
 * 1.显示图片后，需要手动"启用逐个绘制"。
 *   删除图片会自动关闭逐个绘制。
...
//			2.插件对比思路：
//				将相似插件复制副本，然后副本的文本替换为：地图、Map、GFPT
//				然后再将该插件与副本比较，除了注释、界面差异和镜头差异，大部分脚本都是一模一样的。
//		
//		★其它说明细节：
//			暂无
//
...
//=============================================================================
	...
	// * 提示信息 - 报错 - 底层版本过低
	//==============================
	DrillUp.drill_GFPT_getPluginTip_LowVersion = function(){
	...
	DrillUp.drill_GFPT_getPluginTip_DataNotFind = function( text_id ){
		return "【" + DrillUp.g_GFPT_PluginTip_curName + "】（此提示可在插件中关闭）\n" +   //『可关闭提示信息』
	...
	// * 提示信息 - 报错 - 窗口字符底层校验
	//==============================
	DrillUp.drill_GFPT_getPluginTip_NeedUpdate_drawText = function(){
...
//=============================================================================
	...
	/*-----------------杂项------------------*/
	DrillUp.g_GFPT_TipEnabled_DataNotFind = String(DrillUp.parameters["DEBUG-是否提示找不到配置数据"] || "true") === "true";
...
Game_Interpreter.prototype.pluginCommand = function( command, args ){
	...
	this.drill_GFPT_pluginCommand( command, args );
}
//==============================
// * 插件指令 - 指令执行
//==============================
Game_Interpreter.prototype.drill_GFPT_pluginCommand = function( command, args ){
	if( command === ">地图永久漂浮文字" ){
		...
		if( text_id != null && args.length == 4 ){	
			...
			if( type == "开启定时重绘" ){
				if( this.drill_GFPT_isDataExist( text_id ) == false ){ return; }
				$gameSystem._drill_GFPT_dataTank[ text_id ]['timedRedraw_enabled'] = true;
				$gameSystem._drill_GFPT_dataTank[ text_id ]['timing_enabled'] = false;		//（功能互斥）
			}
			if( type == "关闭定时重绘" ){
				if( this.drill_GFPT_isDataExist( text_id ) == false ){ return; }
				$gameSystem._drill_GFPT_dataTank[ text_id ]['timedRedraw_enabled'] = false;
			}
		}
		if( text_id != null && args.length == 6 ){	
			var type = String(args[3]);
			...
			if( type == "设置定时重绘" ){
				...
				if( temp1.indexOf("间隔时间[") != -1 ){
					...
					temp1 = temp1.replace("]","");
					$gameSystem._drill_GFPT_dataTank[ text_id ]['timedRedraw_time'] = Number(temp1);
				}
			}
		}
		...
		if( text_id != null && args.length == 4 ){
			...
			if( type == "启用逐个绘制" ){
				if( this.drill_GFPT_isDataExist( text_id ) == false ){ return; }
				$gameSystem._drill_GFPT_dataTank[ text_id ]['timing_enabled'] = true;
				$gameSystem._drill_GFPT_dataTank[ text_id ]['timedRedraw_enabled'] = false;	//（功能互斥）
			}
			if( type == "关闭逐个绘制" ){
				if( this.drill_GFPT_isDataExist( text_id ) == false ){ return; }
				$gameSystem._drill_GFPT_dataTank[ text_id ]['timing_enabled'] = false;
			}
		}
		if( text_id != null && args.length == 6 ){
			var type = String(args[3]);
			...
			if( type == "修改逐个绘制设置" ){
				...
				if( temp1.indexOf("间隔时间[") != -1 ){
					temp1 = temp1.replace("间隔时间[","");
					temp1 = temp1.replace("]","");
					$gameSystem._drill_GFPT_dataTank[ text_id ]['timing_interval'] = Number(temp1);
				}
			}
		}
		if( text_id != null && args.length == 4 ){	
			...
			if( type == "重新逐个绘制" ){
				if( this.drill_GFPT_isDataExist( text_id ) == false ){ return; }
				$gameSystem._drill_GFPT_dataTank[ text_id ]['timing_needRedraw'] = true;
			}
		}
	};
};
...
Game_Interpreter.prototype.drill_GFPT_isDataExist = function( text_id ){
	...
	if( data == undefined ){
		if( DrillUp.g_GFPT_TipEnabled_DataNotFind == true ){  //『可关闭提示信息』
...
// * 预加载 - 初始化
//==============================
var _drill_GFPT_preload_initialize = Game_Temp.prototype.initialize;
Game_Temp.prototype.initialize = function() {
    _drill_GFPT_preload_initialize.call(this);
	this.drill_GFPT_preloadInit();
}
//==============================
// * 预加载 - 版本校验
//==============================
if( Utils.generateRuntimeId == undefined ){
	alert( DrillUp.drill_GFPT_getPluginTip_LowVersion() );
}
//==============================
// * 预加载 - 执行资源预加载
//
...
//==============================
Game_Temp.prototype.drill_GFPT_preloadInit = function() {
	this._drill_GFPT_cacheId = Utils.generateRuntimeId();	//资源缓存id
    this._drill_GFPT_preloadTank = [];						//bitmap容器
	for( var i = 0; i < DrillUp.g_GFPT_style.length; i++ ){
		var temp_data = DrillUp.g_GFPT_style[i];
		if( temp_data == undefined ){ continue; }
		
		// > 『窗口皮肤的预加载』
		if( temp_data['window_type'] == "自定义窗口皮肤" ){
			this._drill_GFPT_preloadTank.push( 
				ImageManager.reserveBitmap( "img/system/", temp_data['window_sys_src'], 0, true, this._drill_GFPT_cacheId ) 
			);
		}
		if( temp_data['window_type'] == "自定义背景图片" ){
			this._drill_GFPT_preloadTank.push( 
				ImageManager.reserveBitmap( "img/Map__ui/", temp_data['window_pic_src'], 0, true, this._drill_GFPT_cacheId ) 
			);
		}
	}
}
...
//			说明：	> data对象中的参数【可以缺项】。
//==============================
Drill_GFPT_Window.prototype.drill_resetData_Skin = function( data ){
	
	// > 插件自定义值『窗口皮肤』
	var window_pic_file1 = "img/Map__ui/";
...
// * C窗口皮肤 - 帧刷新 - 高宽变化『窗口皮肤』
//==============================
Drill_GFPT_Window.prototype.drill_updateSkin_skinBackground = function() {
...
// * D窗口内容 - 帧刷新
//==============================
Drill_GFPT_Window.prototype.drill_updateMessage = function(){
	
	var data = this._drill_data;
	if( data['timedRedraw_enabled'] == true ){
		
		// > 『定时重绘』 时间+1
		this._drill_GFPT_curTime += 1;
		
		// > 『定时重绘』 执行
		if( this._drill_GFPT_curTime % data['timedRedraw_time'] == 1 ){	//（不要在0值刷新，可能会浪费刷很多次）
...
if( typeof(_drill_COWC_drawText_functionExist) == "undefined" ){
	alert( DrillUp.drill_GFPT_getPluginTip_NeedUpdate_drawText() );
}
