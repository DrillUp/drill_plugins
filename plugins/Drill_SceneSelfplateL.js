...
//=============================================================================
	...
	/*-----------------面板跳转------------------*/
	DrillUp.g_SSpL_add_to_menu = String(DrillUp.parameters["是否在主菜单窗口中显示"] || "true") === "true";	
    DrillUp.g_SSpL_menu_name = String(DrillUp.parameters["主菜单窗口显示名"] || "");
	...
	if( DrillUp.parameters["用语-锁定的描述内容"] != undefined && 
		...
		DrillUp.g_SSpL_locked_context = JSON.parse( DrillUp.parameters["用语-锁定的描述内容"] );
	}else{
...
Drill_SSpL_SelectWindow.prototype.drill_window_drawItem = function( index ){
	...
	// > 参数准备 - 窗口字符对齐方式
	var cur_align = DrillUp.g_SSpL_command_window['align'];
...
Drill_SSpL_DescWindow.prototype.drill_refreshDescText = function( cur_index ){
	...
	if( $gameTemp.drill_SSpL_isLocked( temp_data['index'] ) == true && 
		...
		if( temp_data['lockedContextEnabled'] == true ){
			...
			context = DrillUp.g_SSpL_locked_context;	//用语-锁定的描述内容
		}
	}else{
