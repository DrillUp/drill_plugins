...
Game_Event.prototype.setupPage = function() {
    ...
    this.drill_EUOT_event_readPage();
};
//==============================
// * 事件注释 - 读取 页
//==============================
Game_Event.prototype.drill_EUOT_event_readPage = function() {	
	
	// > 第一次出生，强制读取第一页注释（防止离开地图后，回来，开关失效）
	if( !this._erased && this.event() && this.event().pages[0] && this._drill_EUOT_isFirstBirth == true ){ 
		this.drill_EUOT_event_readList( this.event().pages[0].list );
		this._drill_EUOT_isFirstBirth = undefined;		//『节约临时参数存储空间』（放后面，注释通过这个识别"跨事件页/不跨事件页"。"跨事件页"的注释必须放在第一页才能生效。）
	}
	
	// > 读取当前页注释
	if( !this._erased && this.page() ){ 
		this.drill_EUOT_event_readList( this.list() );
	}
}
//==============================
// * 事件注释 - 读取 注释
//==============================
Game_Event.prototype.drill_EUOT_event_readList = function( pageOfList ){
