...
/*:
 * @plugindesc [v1.3]        面板 - 全自定义信息面板F
 * @author Drill_up
 * 
 * @Drill_LE_param "阶段-%d"
 * @Drill_LE_parentKey ""
 * @Drill_LE_var "DrillUp.g_SSpF_stepList_length"
 * 
 * 
 * @help  
 * =============================================================================
 ...
 * 你可以通过插件指令修改阶段设置：
 * 
 * 插件指令：>信息面板F : 显示阶段 : 阶段[1]
 * 插件指令：>信息面板F : 显示阶段 : 阶段变量[21]
 * 插件指令：>信息面板F : 隐藏阶段 : 阶段[1]
 * 插件指令：>信息面板F : 隐藏阶段 : 阶段变量[21]
 * 插件指令：>信息面板F : 显示全部阶段
 * 插件指令：>信息面板F : 隐藏全部阶段
 * 
...
//=============================================================================
	...
	DrillUp.g_SSpF_PluginTip_baseList = [
...
//=============================================================================
	...
	//				（~struct~SSpFStep）
	//==============================
	DrillUp.drill_SSpF_initStep = function( dataFrom ){
		var data = {};
	...
	/*-----------------阶段------------------*/
	DrillUp.g_SSpF_stepList_length = 40;
	DrillUp.g_SSpF_stepList = [];
	for( var i = 0; i < DrillUp.g_SSpF_stepList_length; i++ ){
		if( DrillUp.parameters["阶段-" + String(i+1) ] != undefined &&
			...
			DrillUp.g_SSpF_stepList[i] = DrillUp.drill_SSpF_initStep( data );
		}else{
			DrillUp.g_SSpF_stepList[i] = null;
		}
	}
	...
	/*-----------------面板跳转------------------*/
	DrillUp.g_SSpF_add_to_menu = String(DrillUp.parameters["是否在主菜单窗口中显示"] || "true") === "true";	
    DrillUp.g_SSpF_menu_name = String(DrillUp.parameters["主菜单窗口显示名"] || "");
	...
	/*-----------------全局存储对象------------------*/
	DrillUp.g_SSpF_globalSetting_enabled = String(DrillUp.parameters["数据是否全局存储"] || "false") === "true";
    DrillUp.g_SSpF_globalSetting_fileId = Number(DrillUp.parameters["全局存储的文件路径"] || 1);
	DrillUp.global_SSpF_stepVisibleTank = null;
...
// * 『全局存储』 - 载入时检查数据 - 阶段显示情况
//==============================
DrillUp.drill_SSpF_gCheckData_enable = function(){
	for( var i = 0; i < DrillUp.g_SSpF_stepList.length; i++ ){
		var temp_data = DrillUp.g_SSpF_stepList[i];
		
		// > 指定数据为空时
		if( DrillUp.global_SSpF_stepVisibleTank[i] == null ){
			if( temp_data == null ){		//（无内容配置，跳过）
				DrillUp.global_SSpF_stepVisibleTank[i] = null;
			}else{							//（有内容配置，初始化默认）
				DrillUp.global_SSpF_stepVisibleTank[i] = temp_data['stepVisible'];
			}
...
// * 『全局存储』 - 载入
//==============================
	var global_fileId = DrillUp.g_SSpF_globalSetting_fileId;
	var global_data = StorageManager.drill_COGS_loadData( global_fileId, "SSpF" );  //『全局存储执行函数』
	
	// > 显示情况
	if( DrillUp.global_SSpF_stepVisibleTank == null ){		//（游戏没关时，不会为null)
		var data = global_data["global_stepVisibleTank"];
		if( data == undefined ){ data = [] };
		DrillUp.global_SSpF_stepVisibleTank = data;
		DrillUp.drill_SSpF_gCheckData_enable();				//（检查时自动赋新值）
	}
	
//==============================
// * 『全局存储』 - 存储
//==============================
StorageManager.drill_SSpF_saveData = function(){
	var file_id = DrillUp.g_SSpF_globalSetting_fileId;
	var data = {};
	data["global_stepVisibleTank"] = DrillUp.global_SSpF_stepVisibleTank;
	this.drill_COGS_saveData( file_id, "SSpF", data );  //『全局存储执行函数』
};
...
//##############################
DrillUp.g_SSpF_saveEnabled = true;
//##############################
// * 存储数据 - 初始化
//          
//			说明：	> 下方为固定写法，不要动。
//##############################
var _drill_SSpF_sys_initialize = Game_System.prototype.initialize;
Game_System.prototype.initialize = function() {
    _drill_SSpF_sys_initialize.call(this);
	this.drill_SSpF_initSysData();
};
//##############################
// * 存储数据 - 载入存档
//          
//			说明：	> 下方为固定写法，不要动。
//##############################
var _drill_SSpF_sys_extractSaveContents = DataManager.extractSaveContents;
DataManager.extractSaveContents = function( contents ){
	_drill_SSpF_sys_extractSaveContents.call( this, contents );
	
	// > 参数存储 启用时（检查数据）
	if( DrillUp.g_SSpF_saveEnabled == true ){	
		$gameSystem.drill_SSpF_checkSysData();
		
	// > 参数存储 关闭时（直接覆盖）
	}else{
		$gameSystem.drill_SSpF_initSysData();
	}
};
//##############################
// * 存储数据 - 初始化数据【标准函数】
...
//##############################
Game_System.prototype.drill_SSpF_initSysData = function() {
	this.drill_SSpF_initSysData_Private();
};
//##############################
// * 存储数据 - 载入存档时检查数据【标准函数】
//			
...
//##############################
Game_System.prototype.drill_SSpF_checkSysData = function() {
	this.drill_SSpF_checkSysData_Private();
};
//=============================================================================
// ** 存储数据（接口实现）
//=============================================================================
...
// * 存储数据 - 初始化数据（私有）
//==============================
Game_System.prototype.drill_SSpF_initSysData_Private = function() {
	
	this._drill_SSpF_stepVisibleTank = [];				//阶段显示情况
	for( var i = 0; i < DrillUp.g_SSpF_stepList.length; i++ ){
		var temp_data = DrillUp.g_SSpF_stepList[i];
		if( temp_data == undefined ){ continue; }
		this._drill_SSpF_stepVisibleTank[i] = temp_data['stepVisible'];
	}
};
//==============================
// * 存储数据 - 载入存档时检查数据（私有）
//==============================
Game_System.prototype.drill_SSpF_checkSysData_Private = function() {
	
	// > 旧存档数据自动补充
	if( this._drill_SSpF_stepVisibleTank == undefined ){
		this.drill_SSpF_initSysData();
	}
	...
	for( var i = 0; i < DrillUp.g_SSpF_stepList.length; i++ ){
		...
		if( temp_data != undefined ){
			...
			// > 未存储的，重新初始化
			if( this._drill_SSpF_stepVisibleTank[i] == undefined ){
				this._drill_SSpF_stepVisibleTank[i] = temp_data['stepVisible'];
...
Game_Interpreter.prototype.pluginCommand = function( command, args ){
	...
	this.drill_SSpF_pluginCommand( command, args );
}
//==============================
// * 插件指令 - 指令执行
//==============================
Game_Interpreter.prototype.drill_SSpF_pluginCommand = function( command, args ){
	if( command === ">信息面板F" ){
		...
		if( args.length == 4 ){
			...
			if( type == "显示阶段" ){
				DrillUp.global_SSpF_stepVisibleTank[ Number(temp1)-1 ] = true;			//全局存储
				$gameSystem._drill_SSpF_stepVisibleTank[ Number(temp1)-1 ] = true;		//正常存储
				StorageManager.drill_SSpF_saveData();
			}
			if( type == "隐藏阶段" ){
				DrillUp.global_SSpF_stepVisibleTank[ Number(temp1)-1 ] = false;			//全局存储
				$gameSystem._drill_SSpF_stepVisibleTank[ Number(temp1)-1 ] = false;		//正常存储
				StorageManager.drill_SSpF_saveData();
			}
		}
		if( args.length == 2 ){
			...
			if( type == "显示全部阶段" ){
				for( var i = 0; i < DrillUp.g_SSpF_stepList.length; i++ ){
					DrillUp.global_SSpF_stepVisibleTank[i] = true;			//全局存储
					$gameSystem._drill_SSpF_stepVisibleTank[i] = true;		//正常存储
				}
				StorageManager.drill_SSpF_saveData();
			}
			if( type == "隐藏全部阶段" ){
				for( var i = 0; i < DrillUp.g_SSpF_stepList.length; i++ ){
					DrillUp.global_SSpF_stepVisibleTank[i] = false;			//全局存储
					$gameSystem._drill_SSpF_stepVisibleTank[i] = false;		//正常存储
				}
				StorageManager.drill_SSpF_saveData();
			}
		}
...
Game_Temp.prototype.drill_SSpF_isVisible = function( context_realIndex ){
	...
	// > 全局存储控制
	if( DrillUp.g_SSpF_globalSetting_enabled == true ){
		if( DrillUp.global_SSpF_stepVisibleTank[ context_realIndex ] == true ){
			return true;
		}else{
			return false;
		}
	...
	// > 正常存储控制
		...
		if( $gameSystem._drill_SSpF_stepVisibleTank[ context_realIndex ] == true ){
			return true;
		}else{
			return false;
		}
...
// * A主体 - 创建
//==============================
Scene_Drill_SSpF.prototype.drill_createAttr = function() {
	...
	// > 整体布局贴图
	this._drill_layout = new Sprite( ImageManager.loadBitmap("img/Menu__self/", DrillUp.g_SSpF_layout, 0, true) );
...
// * B内容遮罩 - 创建
//==============================
Scene_Drill_SSpF.prototype.drill_createMask = function() {
	var mask_src = DrillUp.g_SSpF_contextMask;
...
Scene_Drill_SSpF.prototype.drill_createRoller = function() {
	...
	for( var i = 0; i < step_data.length; i++ ){
		...
		if( temp_data == undefined ){ continue; }
		temp_data['stepVisible'] = $gameTemp.drill_SSpF_isVisible( i );
	}
	
	// > 长画布贴图 - 数据
	var sprite_data = {
		"opacityShow": DrillUp.g_SSpF_opacityShow,		//开始时渐变显示 开关
		"opacitySpeed": DrillUp.g_SSpF_opacitySpeed,	//开始时渐变显示 速度
...
// * C长画布控制 - 帧刷新
//==============================
Scene_Drill_SSpF.prototype.drill_updateRoller = function() {
...
//==============================
// * 信息面板F（菜单界面基类） - 当前角色切换时
//==============================
Scene_Drill_SSpF.prototype.onActorChange = function() {
