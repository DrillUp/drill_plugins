...
/*:
 * @plugindesc [v2.0]        标题 - 多层标题背景
 * @author Drill_up
 * 
 * @Drill_LE_param "背景-%d"
 * @Drill_LE_parentKey "---背景组%d至%d---"
 * @Drill_LE_var "DrillUp.g_TBa_style_length"
 * 
 * 
 * @help
 * =============================================================================
 ...
 * 插件指令：>标题背景 : 隐藏全部
 * 
 * 1."背景[3]"中的数字表示对应插件配置的编号，只能一个一个改。
...
//				->不考虑销毁情况
//			
//			->标题背景控制器【Drill_TBa_Controller】
//			->标题背景贴图【Drill_TBa_Sprite】
//
//
//		★家谱：
//			无
...
//=============================================================================
	...
	DrillUp.g_TBa_PluginTip_baseList = [
	...
	// * 提示信息 - 报错 - 找不到样式
	//==============================
	DrillUp.drill_TBa_getPluginTip_StyleNotFind = function( style_id ){
		return "【" + DrillUp.g_TBa_PluginTip_curName + "】\n对象创建失败，id为"+style_id+"的样式配置为空或不存在。";
	};
	//==============================
	// * 提示信息 - 报错 - NaN校验值
	//==============================
	DrillUp.drill_TBa_getPluginTip_ParamIsNaN = function( param_name, check_tank ){
		var text = "【" + DrillUp.g_TBa_PluginTip_curName + "】\n检测到参数"+param_name+"出现了NaN值，请及时检查你的函数。";
		if( check_tank ){
	...
	// * 提示信息 - 报错 - 底层版本过低
	//==============================
	DrillUp.drill_TBa_getPluginTip_LowVersion = function(){
		return "【" + DrillUp.g_TBa_PluginTip_curName + "】\n游戏底层版本过低，插件基本功能无法执行。\n你可以去看\"rmmv软件版本（必看）.docx\"中的 \"旧工程升级至1.6版本\" 章节，来升级你的游戏底层版本。";
	};
	//==============================
	// * 提示信息 - 报错 - 强制更新提示
	//==============================
	DrillUp.drill_TBa_getPluginTip_NeedUpdate_Ballistics = function(){
		return "【" + DrillUp.g_TBa_PluginTip_curName + "】\n弹道核心插件版本过低，你需要更新 弹道核心 至少v2.2及以上版本。";
	};
	//==============================
	// * 提示信息 - 报错 - 控制器的非数字参数
	//==============================
	DrillUp.drill_TBa_getPluginTip_controllerData_NotId = function( class_name ){
...
//=============================================================================
	...
	DrillUp.drill_TBa_backgroundInit = function( dataFrom ){
		...
		data['speedX'] = Number( dataFrom["背景X速度"] || 0.0);
		data['speedY'] = Number( dataFrom["背景Y速度"] || 0.0);
		...
		Drill_TBa_Controller.drill_controller_initData( data );
	...
	// * 静态数据 - 最后继承1级
	//==============================
	var _drill_TBa_scene_initialize = SceneManager.initialize;
	SceneManager.initialize = function() {
		_drill_TBa_scene_initialize.call(this);
		
		/*-----------------背景------------------*/
		for( var i = 0; i < DrillUp.g_TBa_style.length; i++ ){
			var temp = DrillUp.g_TBa_style[i];
			if( temp != undefined ){
				DrillUp.g_TBa_style[i] = DrillUp.drill_TBa_backgroundInit( temp );
			}
		}
	}
	/*-----------------背景------------------*/
	DrillUp.g_TBa_style_length = 80;
	DrillUp.g_TBa_style = [];
	for( var i = 0; i < DrillUp.g_TBa_style_length; i++ ){
		if( DrillUp.parameters["背景-" + String(i+1) ] != undefined &&
			DrillUp.parameters["背景-" + String(i+1) ] != "" ){
			DrillUp.g_TBa_style[i] = JSON.parse(DrillUp.parameters["背景-" + String(i+1) ]);
		}else{
			DrillUp.g_TBa_style[i] = undefined;		//（设为空值，节约静态数据占用容量）
		}
	}
...
if( Imported.Drill_CoreOfBallistics &&
	...
	if( typeof(Drill_COBa_ExtendTool) == "undefined" ){	//（弹道核心版本检测）
		alert( DrillUp.drill_TBa_getPluginTip_NeedUpdate_Ballistics() );
	}
...
Game_Interpreter.prototype.pluginCommand = function( command, args ){
	...
	this.drill_TBa_pluginCommand( command, args );
}
//==============================
// * 插件指令 - 指令执行
//==============================
Game_Interpreter.prototype.drill_TBa_pluginCommand = function( command, args ){
	if( command === ">标题背景" ){
...
//==============================
var _drill_TBa_scene_initialize2 = SceneManager.initialize;
SceneManager.initialize = function() {
	_drill_TBa_scene_initialize2.call(this);
	...
	DrillUp.drill_TBa_gCheckData_visible = function(){
		for( var i = 0; i < DrillUp.g_TBa_style_length ; i++ ){
			var temp_c = DrillUp.g_TBa_style[i];
...
//##############################
Scene_Title.prototype.drill_TBa_layerAddSprite = function( sprite, layer_index ){
    this.drill_TBa_layerAddSprite_Private(sprite, layer_index);
};
//##############################
// * 标题层级 - 去除贴图【标准函数】
//				
...
//##############################
Scene_Title.prototype.drill_TBa_layerRemoveSprite = function( sprite ){
	this.drill_TBa_layerRemoveSprite_Private( sprite );
};
//##############################
// * 标题层级 - 图片层级排序【标准函数】
//				
...
Scene_Title.prototype.drill_TBa_sortByZIndex = function () {
    this.drill_TBa_sortByZIndex_Private();
};
//=============================================================================
// ** 标题层级（接口实现）
//=============================================================================
...
// * 标题层级 - 层级初始化1
//==============================
var _drill_TBa_titleLayer_createBackground = Scene_Title.prototype.createBackground;
Scene_Title.prototype.createBackground = function() {
	_drill_TBa_titleLayer_createBackground.call(this);
...
// * 标题层级 - 层级初始化2
//==============================
var _drill_TBa_titleLayer_update = Scene_Title.prototype.update;
Scene_Title.prototype.update = function() {
	_drill_TBa_titleLayer_update.call(this);
...
// * 标题层级 - 图片层级排序（私有）
//==============================
Scene_Title.prototype.drill_TBa_sortByZIndex_Private = function() {
...
// * 标题层级 - 去除贴图（私有）
//==============================
Scene_Title.prototype.drill_TBa_layerRemoveSprite_Private = function( sprite ){
...
// * 标题层级 - 添加贴图到层级（私有）
//==============================
Scene_Title.prototype.drill_TBa_layerAddSprite_Private = function( sprite, layer_index ){
...
if( Utils.generateRuntimeId == undefined ){
	alert( DrillUp.drill_TBa_getPluginTip_LowVersion() );
}
//==============================
// * 预加载 - 最后继承1级
//
...
//==============================
var _drill_TBa_scene_initialize3 = SceneManager.initialize;
SceneManager.initialize = function() {
	_drill_TBa_scene_initialize3.call(this);
	
	// > 执行资源预加载
	DrillUp.drill_TBa_preloadInit();
}
//==============================
// * 预加载 - 执行资源预加载
//
...
//==============================
DrillUp.drill_TBa_preloadInit = function(){
	this._drill_TBa_cacheId = Utils.generateRuntimeId();	//资源缓存id
	this._drill_TBa_preloadTank = [];						//bitmap容器
	for( var i = 0; i < DrillUp.g_TBa_style.length; i++ ){
		var temp_data = DrillUp.g_TBa_style[i];
		if( temp_data == undefined ){ continue; }
		if( temp_data['preload'] != true ){ continue; }
		
		this._drill_TBa_preloadTank.push( 
			ImageManager.reserveBitmap( temp_data['src_img_file'], temp_data['src_img'], temp_data['tint'], temp_data['smooth'], this._drill_TBa_cacheId ) 
		);
	}
}
...
Scene_Title.prototype.createBackground = function() {
	...
	SceneManager._drill_TBa_created = false;
   	...
   	this._drill_TBa_controllerTank = [];
...
Scene_Title.prototype.terminate = function() {
	_drill_TBa_terminate.call(this);
	SceneManager._drill_TBa_created = false;	//（下次进入界面需重新创建）
};
//==============================
// * 贴图创建标记 - 帧刷新
//==============================
...
Scene_Title.prototype.update = function() {
	...
	if( SceneManager.isCurrentSceneStarted() && 
		...
		this.drill_TBa_create();
	}
	// > 帧刷新
	if( SceneManager._drill_TBa_created == true ){
...
Scene_Title.prototype.drill_TBa_create = function() {	
	...
	// > 防止报错
	if( this._drill_TBa_spriteTank == undefined ){
		this._drill_TBa_spriteTank = [];
	}
	if( this._drill_TBa_controllerTank == undefined ){
		this._drill_TBa_controllerTank = [];
	}
	...
	for( var i = 0; i < DrillUp.g_TBa_style.length; i++ ){
		...
		// > 创建控制器
		var temp_controller = new Drill_TBa_Controller( i );
		this._drill_TBa_controllerTank.push( temp_controller );
		
		// > 创建贴图
		var temp_sprite = new Drill_TBa_Sprite();
		...
		this.drill_TBa_layerAddSprite( temp_sprite, temp_data['layerIndex'] );
	}
...
Scene_Title.prototype.drill_TBa_update = function() {
	this.drill_TBa_updateController();
	this.drill_TBa_updateAttr();
}
//==============================
// * 贴图控制 - 帧刷新 控制器
//==============================
Scene_Title.prototype.drill_TBa_updateController = function(){
	for(var i = 0; i < this._drill_TBa_controllerTank.length; i++ ){
		var temp_controller = this._drill_TBa_controllerTank[i];
...
// * 贴图控制 - 帧刷新 基础特性
//==============================
Scene_Title.prototype.drill_TBa_updateAttr = function(){
	...
	for(var i = 0; i < this._drill_TBa_spriteTank.length; i++){
		...
		if( temp_sprite.layerIndex != temp_controller._drill_layerIndex ){
			...
			this.drill_TBa_layerAddSprite( temp_sprite, temp_controller._drill_layerIndex );
...
// * 控制器 - 定义
//==============================
function Drill_TBa_Controller(){
    this.initialize.apply(this, arguments);
};
//==============================
// * 控制器 - 初始化
//==============================
Drill_TBa_Controller.prototype.initialize = function( data_id ){
...
//##############################
Drill_TBa_Controller.prototype.drill_controller_update = function(){
...
//##############################
Drill_TBa_Controller.prototype.drill_controller_resetData = function( data_id ){
...
//			说明：	> 可放在帧刷新函数中实时调用。
//##############################
Drill_TBa_Controller.prototype.drill_controller_setVisible = function( visible ){
...
//			说明：	> 可放在帧刷新函数中实时调用。
//##############################
Drill_TBa_Controller.prototype.drill_controller_setPause = function( pause ){
...
//			返回：	> 布尔
//##############################
Drill_TBa_Controller.prototype.drill_controller_destroy = function(){
...
//			返回：	> 布尔
//##############################
Drill_TBa_Controller.prototype.drill_controller_isDead = function(){
...
//==============================
Drill_TBa_Controller.METHOD__drill_controller_setVisible                   = 11;
Drill_TBa_Controller.METHOD__drill_controller_setPause                     = 12;
Drill_TBa_Controller.METHOD__drill_controller_periodizeCommand_setPause    = 13;
Drill_TBa_Controller.METHOD__drill_controller_periodizeCommand_clear       = 14;

Drill_TBa_Controller.METHOD__drill_controller_setBlendMode                 = 21;
Drill_TBa_Controller.METHOD__drill_controller_setLayerIndex                = 22;
Drill_TBa_Controller.METHOD__drill_controller_setZIndex                    = 23;
Drill_TBa_Controller.METHOD__drill_controller_restoreBlendMode             = 24;
Drill_TBa_Controller.METHOD__drill_controller_restoreLayerIndex            = 25;
Drill_TBa_Controller.METHOD__drill_controller_restoreZIndex                = 26;

Drill_TBa_Controller.METHOD__drill_controller_commandChange_setOpacity     = 31;
Drill_TBa_Controller.METHOD__drill_controller_commandChange_setSpeedX      = 32;
Drill_TBa_Controller.METHOD__drill_controller_commandChange_setSpeedY      = 33;
Drill_TBa_Controller.METHOD__drill_controller_commandChange_setRotate      = 34;

Drill_TBa_Controller.METHOD__drill_controller_commandChange_setScaleX      = 41;
Drill_TBa_Controller.METHOD__drill_controller_commandChange_setScaleY      = 42;
Drill_TBa_Controller.METHOD__drill_controller_commandChange_setSkewX       = 43;
Drill_TBa_Controller.METHOD__drill_controller_commandChange_setSkewY       = 44;
Drill_TBa_Controller.METHOD__drill_controller_commandChange_restoreAttr    = 45;

Drill_TBa_Controller.METHOD__drill_controller_commandChange_setMove        = 51;
Drill_TBa_Controller.METHOD__drill_controller_commandChange_restoreMove    = 52;
...
//			返回：	> 无
//##############################
Drill_TBa_Controller.prototype.drill_controller_setBlendMode = function( blendMode ){
...
//			返回：	> 无
//##############################
Drill_TBa_Controller.prototype.drill_controller_setLayerIndex = function( layerIndex ){
...
//			返回：	> 无
//##############################
Drill_TBa_Controller.prototype.drill_controller_setZIndex = function( zIndex ){
...
//			返回：	> 无
//##############################
Drill_TBa_Controller.prototype.drill_controller_restoreBlendMode = function(){
...
//			返回：	> 无
//##############################
Drill_TBa_Controller.prototype.drill_controller_restoreLayerIndex = function(){
	var data = this.drill_data();
...
//			返回：	> 无
//##############################
Drill_TBa_Controller.prototype.drill_controller_restoreZIndex = function(){
	var data = this.drill_data();
...
//##############################
Drill_TBa_Controller.drill_controller_initData = function( data ){
	...
	if( data['layerIndex'] == undefined ){ data['layerIndex'] = "菜单后面层" };						//A主体 - 标题层级
...
// * 控制器 - 初始化子功能『控制器与贴图』
//==============================
Drill_TBa_Controller.prototype.drill_controller_initChild = function(){
...
// * 控制器 - 重设数据（私有）
//==============================
Drill_TBa_Controller.prototype.drill_controller_resetData_Private = function( data_id ){
	
	// > 参数检查
	if( typeof data_id != "number" ){
		alert( DrillUp.drill_TBa_getPluginTip_controllerData_NotId("Drill_TBa_Controller") );
		throw Error( DrillUp.drill_TBa_getPluginTip_controllerData_NotId("Drill_TBa_Controller") );
		return;
	}
...
//##############################
DrillUp.g_TBa_notFindStyleAlertOnce = true;
//##############################
// * 控制器 - 『控制器与贴图的样式-静态数据』空的静态数据
//			
//			说明：	> 空数据会在initData时会进行默认值初始化，在其他地方只读。
//##############################
Drill_TBa_Controller.emptyData = {};
//##############################
...
//##############################
Drill_TBa_Controller.prototype.drill_data = function(){
	var cur_styleId   = this._drill_data_id +1;
	var cur_styleData = DrillUp.g_TBa_style[ this._drill_data_id ];
	if( cur_styleData == undefined ){
		
		// > 『控制器与贴图的样式-静态数据』 - 校验+提示信息（只执行一次）
		if( DrillUp.g_TBa_notFindStyleAlertOnce == true ){
			DrillUp.g_TBa_notFindStyleAlertOnce = false;
			alert( DrillUp.drill_TBa_getPluginTip_StyleNotFind(cur_styleId) );
		}
		return Drill_TBa_Controller.emptyData;
	}
...
// * A主体 - 初始化子功能
//==============================
Drill_TBa_Controller.prototype.drill_controller_initAttr = function(){
	var data = this.drill_data();
...
// * A主体 - 帧刷新
//==============================
Drill_TBa_Controller.prototype.drill_controller_updateAttr = function(){
...
//==============================
Drill_TBa_Controller.prototype.drill_controller_isVisible = function(){
	if( DrillUp.global_TBa_visibleTank[ this._drill_data_id ] === false ){ return false; }
	return this._drill_visible;
}
//==============================
// * A主体 - 是否 暂停/继续（开放函数）
//==============================
Drill_TBa_Controller.prototype.drill_controller_isPause = function(){
...
// * B变换特性 - 初始化子功能
//==============================
Drill_TBa_Controller.prototype.drill_controller_initChange = function(){
	var data = this.drill_data();
...
// * B变换特性 - 帧刷新 位置
//==============================
Drill_TBa_Controller.prototype.drill_controller_updateChange_Position = function(){
...
//##############################
Drill_TBa_Controller.prototype.drill_controller_finalTransform_x = function(){
	return this._drill_change_x + this._drill_change_selfXAcc + this._drill_move_originOffsetX;
}
//##############################
// * B变换特性 - 数据最终变换值 - 位置Y（可继承，开放函数）
//##############################
Drill_TBa_Controller.prototype.drill_controller_finalTransform_y = function(){
	return this._drill_change_y + this._drill_change_selfYAcc + this._drill_move_originOffsetY;
}
//##############################
// * B变换特性 - 数据最终变换值 - 移动速度X（可继承，开放函数）
//##############################
Drill_TBa_Controller.prototype.drill_controller_finalTransform_selfXSpeed = function(){
	return this._drill_change_selfXSpeed;
}
//##############################
// * B变换特性 - 数据最终变换值 - 移动速度Y（可继承，开放函数）
//##############################
Drill_TBa_Controller.prototype.drill_controller_finalTransform_selfYSpeed = function(){
	return this._drill_change_selfYSpeed;
}
//##############################
// * B变换特性 - 数据最终变换值 - 缩放X（可继承，开放函数）
//##############################
Drill_TBa_Controller.prototype.drill_controller_finalTransform_scaleX = function(){
	return this._drill_change_scaleX;
}
//##############################
// * B变换特性 - 数据最终变换值 - 缩放Y（可继承，开放函数）
//##############################
Drill_TBa_Controller.prototype.drill_controller_finalTransform_scaleY = function(){
	return this._drill_change_scaleY;
}
//##############################
// * B变换特性 - 数据最终变换值 - 透明度（可继承，开放函数）
//##############################
Drill_TBa_Controller.prototype.drill_controller_finalTransform_opacity = function(){
	return this._drill_change_opacity;
}
//##############################
// * B变换特性 - 数据最终变换值 - 斜切X（可继承，开放函数）
//##############################
Drill_TBa_Controller.prototype.drill_controller_finalTransform_skewX = function(){
	return this._drill_change_skewX;
}
//##############################
// * B变换特性 - 数据最终变换值 - 斜切Y（可继承，开放函数）
//##############################
Drill_TBa_Controller.prototype.drill_controller_finalTransform_skewY = function(){
	return this._drill_change_skewY;
}
//##############################
// * B变换特性 - 数据最终变换值 - 旋转（可继承，开放函数）
//##############################
Drill_TBa_Controller.prototype.drill_controller_finalTransform_rotate = function(){
	return this._drill_change_rotate;
}
//==============================
// * B变换特性 - 帧刷新 平铺范围
//==============================
Drill_TBa_Controller.prototype.drill_controller_updateChange_MoveRange = function(){
	...
	var ww = Graphics.boxWidth;
	var hh = Graphics.boxHeight;
	var rect = $gameTemp.drill_TBa_Math2D_getRectWithTransform(
		0, 0,
		ww, hh,
		0.5, 0.5,
...
//==============================
Game_Temp.prototype.drill_TBa_Math2D_getRectWithTransform = function( 
					rect_x,rect_y,						//矩形位置 
					rect_w,rect_h, 						//矩形宽高 
	...
	// > 变换后的四个点
	var pp1 = $gameTemp.drill_TBa_Math2D_getPointWithTransform( p1_x,p1_y, c_x,c_y, rotation, scale_x,scale_y, skew_x,skew_y );
	var pp2 = $gameTemp.drill_TBa_Math2D_getPointWithTransform( p2_x,p2_y, c_x,c_y, rotation, scale_x,scale_y, skew_x,skew_y );
	var pp3 = $gameTemp.drill_TBa_Math2D_getPointWithTransform( p3_x,p3_y, c_x,c_y, rotation, scale_x,scale_y, skew_x,skew_y );
	var pp4 = $gameTemp.drill_TBa_Math2D_getPointWithTransform( p4_x,p4_y, c_x,c_y, rotation, scale_x,scale_y, skew_x,skew_y );
...
//					  旋转+缩放+斜切 可为负数。
//==============================
Game_Temp.prototype.drill_TBa_Math2D_getPointWithTransform = function( 
					cur_x,cur_y,						//需要变换的点 
...
// * C校验值 - 初始化子功能
//==============================
Drill_TBa_Controller.prototype.drill_controller_initCheck = function(){
	//（无）
}
//==============================
// * C校验值 - 校验标记
//==============================
DrillUp.g_TBa_checkNaN = true;
//==============================
// * C校验值 - 帧刷新
//==============================
Drill_TBa_Controller.prototype.drill_controller_updateCheckNaN = function(){
	if( $gameTemp == undefined ){ return; }		//（测试版开启功能，发布版关闭功能）
	if( $gameTemp.isPlaytest() != true ){ return; }
	
	// > 校验值
	if( DrillUp.g_TBa_checkNaN == true ){
		if( isNaN( this.drill_controller_finalTransform_x() ) ){
			DrillUp.g_TBa_checkNaN = false;
			var check_tank = {};
			...
			alert( DrillUp.drill_TBa_getPluginTip_ParamIsNaN( "drill_controller_finalTransform_x", check_tank ) );
		}
		if( isNaN( this.drill_controller_finalTransform_y() ) ){
			...
			alert( DrillUp.drill_TBa_getPluginTip_ParamIsNaN( "drill_controller_finalTransform_y", check_tank ) );
		}
		if( isNaN( this.drill_controller_finalTransform_opacity() ) ){
			DrillUp.g_TBa_checkNaN = false;
			alert( DrillUp.drill_TBa_getPluginTip_ParamIsNaN( "drill_controller_finalTransform_opacity" ) );
		}
		if( isNaN( this.drill_controller_finalTransform_rotate() ) ){
			DrillUp.g_TBa_checkNaN = false;
			alert( DrillUp.drill_TBa_getPluginTip_ParamIsNaN( "drill_controller_finalTransform_rotate" ) );
		}
	}
}
...
//==============================
Drill_TBa_Controller.prototype.drill_controller_initCommandChange = function(){
...
// * D指令叠加变化 - 帧刷新
//==============================
Drill_TBa_Controller.prototype.drill_controller_updateCommandChange = function(){
...
// * D指令叠加变化 - 立即还原所有单属性
//==============================
Drill_TBa_Controller.prototype.drill_controller_commandChange_restoreAttr = function(){
...
// * D指令叠加变化 - 立即归位
//==============================
Drill_TBa_Controller.prototype.drill_controller_commandChange_restoreMove = function(){
	this["_drill_command_move_data"] = undefined;
}
//==============================
// * D指令叠加变化 - 修改单属性 - 移动到
//==============================
Drill_TBa_Controller.prototype.drill_controller_commandChange_setMove = function( change_type, tar_valueA, tar_valueB, tar_time ){
	var data = this.drill_data();
...
// * D指令叠加变化 - 修改单属性 - 透明度
//==============================
Drill_TBa_Controller.prototype.drill_controller_commandChange_setOpacity = function( change_type, tar_value, tar_time ){
	var data = this.drill_data();
...
// * D指令叠加变化 - 修改单属性 - 移动速度X
//==============================
Drill_TBa_Controller.prototype.drill_controller_commandChange_setSpeedX = function( change_type, tar_value, tar_time ){
	var data = this.drill_data();
	Drill_COBa_ExtendTool.drill_COBa_Common_controller_setTarget(
...
// * D指令叠加变化 - 修改单属性 - 移动速度Y
//==============================
Drill_TBa_Controller.prototype.drill_controller_commandChange_setSpeedY = function( change_type, tar_value, tar_time ){
	var data = this.drill_data();
	Drill_COBa_ExtendTool.drill_COBa_Common_controller_setTarget(
...
// * D指令叠加变化 - 修改单属性 - 旋转
//==============================
Drill_TBa_Controller.prototype.drill_controller_commandChange_setRotate = function( change_type, tar_value, tar_time ){
	var data = this.drill_data();
	Drill_COBa_ExtendTool.drill_COBa_Common_controller_setTarget(
...
// * D指令叠加变化 - 修改单属性 - 缩放X
//==============================
Drill_TBa_Controller.prototype.drill_controller_commandChange_setScaleX = function( change_type, tar_value, tar_time ){
	var data = this.drill_data();
	Drill_COBa_ExtendTool.drill_COBa_Common_controller_setTarget(
...
// * D指令叠加变化 - 修改单属性 - 缩放Y
//==============================
Drill_TBa_Controller.prototype.drill_controller_commandChange_setScaleY = function( change_type, tar_value, tar_time ){
	var data = this.drill_data();
	Drill_COBa_ExtendTool.drill_COBa_Common_controller_setTarget(
...
// * D指令叠加变化 - 修改单属性 - 斜切X
//==============================
Drill_TBa_Controller.prototype.drill_controller_commandChange_setSkewX = function( change_type, tar_value, tar_time ){
	var data = this.drill_data();
	Drill_COBa_ExtendTool.drill_COBa_Common_controller_setTarget(
...
// * D指令叠加变化 - 修改单属性 - 斜切Y
//==============================
Drill_TBa_Controller.prototype.drill_controller_commandChange_setSkewY = function( change_type, tar_value, tar_time ){
	var data = this.drill_data();
	Drill_COBa_ExtendTool.drill_COBa_Common_controller_setTarget(
...
// * E延迟指令 - 初始化子功能
//==============================
Drill_TBa_Controller.prototype.drill_controller_initDelayingCommand = function(){
...
//==============================
Drill_TBa_Controller.prototype.drill_controller_updateDelayingCommandImportant = function(){
	...
	for(var i = 0; i < this._drill_curDelayingCommandTank.length; i++ ){
		...
		if( dc_data['left_time'] < 0 ){
			...
			if( method == Drill_TBa_Controller.METHOD__drill_controller_setPause ){
				this.drill_controller_setPause( paramList[0] );
			}else if( method == Drill_TBa_Controller.METHOD__drill_controller_periodizeCommand_setPause ){  //暂停周期指令(延迟)
				this.drill_controller_periodizeCommand_setPause( paramList[0] );
			}else if( method == Drill_TBa_Controller.METHOD__drill_controller_periodizeCommand_clear ){  //清空周期指令(延迟)
...
// * E延迟指令 - 帧刷新 - 执行延迟指令
//==============================
Drill_TBa_Controller.prototype.drill_controller_updateDelayingCommand = function(){
	if( this._drill_curDelayingCommandTank.length == 0 ){ return; }
	...
	for(var i = 0; i < this._drill_curDelayingCommandTank.length; i++ ){
		...
		if( dc_data['left_time'] < 0 ){
			...
			switch( method ){
				case Drill_TBa_Controller.METHOD__drill_controller_setVisible:
					this.drill_controller_setVisible( paramList[0] );
					break;
				
				case Drill_TBa_Controller.METHOD__drill_controller_setBlendMode:
					this.drill_controller_setBlendMode( paramList[0] );
					break;
				case Drill_TBa_Controller.METHOD__drill_controller_setLayerIndex:
					this.drill_controller_setLayerIndex( paramList[0] );
					break;
				case Drill_TBa_Controller.METHOD__drill_controller_setZIndex:
					this.drill_controller_setZIndex( paramList[0] );
					break;
				case Drill_TBa_Controller.METHOD__drill_controller_restoreBlendMode:
					this.drill_controller_restoreBlendMode();
					break;
				case Drill_TBa_Controller.METHOD__drill_controller_restoreLayerIndex:
					this.drill_controller_restoreLayerIndex();
					break;
				case Drill_TBa_Controller.METHOD__drill_controller_restoreZIndex:
					this.drill_controller_restoreZIndex();
					break;
				
				case Drill_TBa_Controller.METHOD__drill_controller_commandChange_setOpacity:
					this.drill_controller_commandChange_setOpacity( paramList[0], paramList[1], paramList[2] );
					break;
				case Drill_TBa_Controller.METHOD__drill_controller_commandChange_setSpeedX:
					this.drill_controller_commandChange_setSpeedX( paramList[0], paramList[1], paramList[2] );
					break;
				case Drill_TBa_Controller.METHOD__drill_controller_commandChange_setSpeedY:
					this.drill_controller_commandChange_setSpeedY( paramList[0], paramList[1], paramList[2] );
					break;
				case Drill_TBa_Controller.METHOD__drill_controller_commandChange_setRotate:
					this.drill_controller_commandChange_setRotate( paramList[0], paramList[1], paramList[2] );
					break;
					
				case Drill_TBa_Controller.METHOD__drill_controller_commandChange_setScaleX:
					this.drill_controller_commandChange_setScaleX( paramList[0], paramList[1], paramList[2] );
					break;
				case Drill_TBa_Controller.METHOD__drill_controller_commandChange_setScaleY:
					this.drill_controller_commandChange_setScaleY( paramList[0], paramList[1], paramList[2] );
					break;
				case Drill_TBa_Controller.METHOD__drill_controller_commandChange_setSkewX:
					this.drill_controller_commandChange_setSkewX( paramList[0], paramList[1], paramList[2] );
					break;
				case Drill_TBa_Controller.METHOD__drill_controller_commandChange_setSkewY:
					this.drill_controller_commandChange_setSkewY( paramList[0], paramList[1], paramList[2] );
					break;
				case Drill_TBa_Controller.METHOD__drill_controller_commandChange_restoreAttr:
					this.drill_controller_commandChange_restoreAttr();
					break;
				
				case Drill_TBa_Controller.METHOD__drill_controller_commandChange_setMove:
					this.drill_controller_commandChange_setMove( paramList[0], paramList[1], paramList[2], paramList[3] );
					break;
				case Drill_TBa_Controller.METHOD__drill_controller_commandChange_restoreMove:
...
// * E延迟指令 - 设置指令（开放函数）
//==============================
Drill_TBa_Controller.prototype.drill_controller_setDelayingCommand = function( method, paramList, delay_time ){
	var enable = false;
	switch( method ){
		case Drill_TBa_Controller.METHOD__drill_controller_setVisible:
		case Drill_TBa_Controller.METHOD__drill_controller_setPause:
		case Drill_TBa_Controller.METHOD__drill_controller_periodizeCommand_setPause:
		case Drill_TBa_Controller.METHOD__drill_controller_periodizeCommand_clear:
...
// * E延迟指令 - 增加延迟（开放函数）
//==============================
Drill_TBa_Controller.prototype.drill_controller_addDelayingTime = function( time ){
...
// * E延迟指令 - 清空全部（开放函数）
//==============================
Drill_TBa_Controller.prototype.drill_controller_clearDelayingCommand = function(){
	this._drill_curDelayingCommandTank = [];
}
...
// * F周期指令 - 初始化子功能
//==============================
Drill_TBa_Controller.prototype.drill_controller_initPeriodizeCommand = function(){
...
//			说明：	> 此处的时间流逝不会因为 暂停 而停止流逝。
//==============================
Drill_TBa_Controller.prototype.drill_controller_updatePeriodizeCommandImportant = function(){
...
// * F周期指令 - 帧刷新 - 执行周期指令
//==============================
Drill_TBa_Controller.prototype.drill_controller_updatePeriodizeCommand = function(){
	if( this._drill_curPeriodizeCommandTank.length == 0 ){ return; }
...
// * F周期指令 - 设置指令（开放函数）
//==============================
Drill_TBa_Controller.prototype.drill_controller_setPeriodizeCommand = function( method, paramList, time_period, time_start ){
	var enable = false;
	switch( method ){
		case Drill_TBa_Controller.METHOD__drill_controller_setVisible:
		case Drill_TBa_Controller.METHOD__drill_controller_setPause:
...
//==============================
Drill_TBa_Controller.prototype.drill_controller_periodizeCommand_getWaitAndClearTime = function(){
...
//==============================
Drill_TBa_Controller.prototype.drill_controller_periodizeCommand_setPause = function( enabled ){
...
//==============================
Drill_TBa_Controller.prototype.drill_controller_periodizeCommand_waitAndPause = function(){
...
//==============================
Drill_TBa_Controller.prototype.drill_controller_periodizeCommand_clear = function(){
	this._drill_curPeriodizeCommandTank = [];
...
//==============================
Drill_TBa_Controller.prototype.drill_controller_periodizeCommand_waitAndClear = function(){
...
// * G自变化效果 - 初始化子功能
//==============================
Drill_TBa_Controller.prototype.drill_controller_initEffect = function(){
	this._drill_curEffectTime = 0;
}
//==============================
// * G自变化效果 - 帧刷新
//==============================
Drill_TBa_Controller.prototype.drill_controller_updateEffect = function(){
...
// * 背景贴图 - 定义
//==============================
function Drill_TBa_Sprite() {
    this.initialize.apply(this, arguments);
};
Drill_TBa_Sprite.prototype = Object.create(Sprite.prototype);
Drill_TBa_Sprite.prototype.constructor = Drill_TBa_Sprite;
//==============================
// * 背景贴图 - 初始化
//==============================
Drill_TBa_Sprite.prototype.initialize = function(){
...
// * 背景贴图 - 帧刷新
//==============================
Drill_TBa_Sprite.prototype.update = function(){
...
//##############################
Drill_TBa_Sprite.prototype.drill_sprite_setController = function( controller ){
...
//##############################
Drill_TBa_Sprite.prototype.drill_sprite_initChild = function(){
...
//			说明：	> 这里完全 不考虑 延迟加载问题。
//##############################
Drill_TBa_Sprite.prototype.drill_sprite_isReady = function(){
...
//##############################
Drill_TBa_Sprite.prototype.drill_sprite_isOptimizationPassed = function(){
    return true;
};
//##############################
// * 背景贴图 - 是否需要销毁【标准函数】
...
//##############################
Drill_TBa_Sprite.prototype.drill_sprite_isNeedDestroy = function(){
...
//##############################
Drill_TBa_Sprite.prototype.drill_sprite_destroy = function(){
...
// * 背景贴图 - 初始化自身『控制器与贴图』
//==============================
Drill_TBa_Sprite.prototype.drill_sprite_initSelf = function(){
...
// * 背景贴图 - 销毁子功能『控制器与贴图』
//==============================
Drill_TBa_Sprite.prototype.drill_sprite_destroyChild = function(){
...
// * 背景贴图 - 销毁自身『控制器与贴图』
//==============================
Drill_TBa_Sprite.prototype.drill_sprite_destroySelf = function(){
	this._drill_controller = null;				//控制器对象
	this._drill_curSerial = -1;					//当前序列号
};
...
// * A贴图主体 - 初始化子功能
//==============================
Drill_TBa_Sprite.prototype.drill_sprite_initAttr = function(){
...
// * A贴图主体 - 帧刷新
//==============================
Drill_TBa_Sprite.prototype.drill_sprite_updateAttr = function(){
	...
	//（其它 基础特性 的帧刷新赋值见：drill_TBa_updateAttr）
}
...
// * B变换特性 - 初始化子功能
//==============================
Drill_TBa_Sprite.prototype.drill_sprite_initChange = function(){
...
// * B变换特性 - 帧刷新
//==============================
Drill_TBa_Sprite.prototype.drill_sprite_updateChange = function(){
...
//					> 参数使用字符串进行控制，默认为 null 值。
//==============================
Drill_TBa_Sprite.prototype.drill_sprite_initCommandChange = function(){
...
// * D指令叠加变化-控制器用 - 帧刷新
//==============================
Drill_TBa_Sprite.prototype.drill_sprite_updateCommandChange = function(){
	var data = this._drill_controller.drill_data();
...
// * E延迟指令 - 初始化子功能
//==============================
Drill_TBa_Sprite.prototype.drill_sprite_initDelayingCommand = function(){
	//（无）
}
...
// * F周期指令 - 初始化子功能
//==============================
Drill_TBa_Sprite.prototype.drill_sprite_initPeriodizeCommand = function(){
	//（无）
}
...
// * G自变化效果 - 初始化子功能
//==============================
Drill_TBa_Sprite.prototype.drill_sprite_initEffect = function(){
	//（无）
}
//==============================
// * G自变化效果 - 帧刷新
//==============================
Drill_TBa_Sprite.prototype.drill_sprite_updateEffect = function(){
	var data = this._drill_controller.drill_data();
