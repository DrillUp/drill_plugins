...
//			
//		★私有类如下：
//			* Scene_Drill_SGaC				画廊C
//			* Drill_SGaC_SelectWindow		选项窗口
//			* Drill_SGaC_DescWindow			显示窗口
//			* Drill_SGaC_CompletionWindow	完成度窗口
//			* Drill_SGaC_ImgExpandWindow	原图查看器窗口
//
//		★必要注意事项：
//			1.替换以下字符变成新面板：
//				SGaC
...
　　var DrillUp = DrillUp || {}; 
	...
	DrillUp.g_SGaC_arrow_flash = String(DrillUp.parameters['是否使用闪烁效果'] || "false") === "true";	
...
// * 正常 - 检查数据
//==============================
Game_System.prototype.drill_SGaC_sysCheckData = function() {
	if( this._drill_SGaC_enableTank == null ){ this._drill_SGaC_enableTank = []; }
	if( this._drill_SGaC_lockTank == null   ){ this._drill_SGaC_lockTank = []; }
	
	for( var i = 1; i <= DrillUp.g_SGaC_context_list_length ; i++ ){
		var temp_c = DrillUp.g_SGaC_context_list[i];
		
		// > 指定数据为空时
		if( this._drill_SGaC_enableTank[i] == null ){
			if( temp_c == null ){		//（无内容配置，跳过）
				this._drill_SGaC_enableTank[i] = null;
			}else{						//（有内容配置，初始化默认）
				this._drill_SGaC_enableTank[i] = temp_c['enabled'];
			}
	...
	for( var i = 1; i <= DrillUp.g_SGaC_context_list_length ; i++ ){
		...
		// > 指定数据为空时
		if( this._drill_SGaC_lockTank[i] == null ){
			if( temp_c == null ){		//（无内容配置，跳过）
				this._drill_SGaC_lockTank[i] = null;
			}else{						//（有内容配置，初始化默认）
				this._drill_SGaC_lockTank[i] = temp_c['locked'];
			}
...
Game_Interpreter.prototype.pluginCommand = function(command, args) {
	...
	if( command === ">画廊C" ){
		$gameSystem.drill_SGaC_sysCheckData();	//（初始化）
		if(args.length == 2){
...
Scene_Drill_SGaC.prototype.initialize = function() {
	...
	this._cur_index = -1;
	$gameSystem.drill_SGaC_sysCheckData();
};
//==============================
// * 画廊C - 创建
//============================== 
...
// * 创建 - 资源预加载
//==============================
Scene_Drill_SGaC.prototype.drill_preloadSrc = function() {
...
// * 帧刷新 - 箭头 - 窗口点击事件
//==============================
Scene_Drill_SGaC.prototype.drill_checkImgTouch = function() {
...
// * 帧刷新 - 箭头 - 鼠标点击图片范围判断
//==============================
Scene_Drill_SGaC.prototype.drill_isOnArrow = function(sprite_arrow) {
...
// * 帧刷新 - 箭头 - 箭头高亮范围
//==============================
Scene_Drill_SGaC.prototype.drill_isOnHoverArrow = function(sprite_arrow) {
	if( sprite_arrow.bitmap == null ){ return false };
	if(!sprite_arrow.bitmap.isReady() ){ return false };
	if( sprite_arrow.visible === false ){ return false };
	var pw = sprite_arrow.bitmap.width /2 + 20;
...
// * 帧刷新 - 箭头
//==============================
Scene_Drill_SGaC.prototype.drill_updateArrow = function() {
	//线性变化量
	...
	if( DrillUp.g_SGaC_arrow_float_lr ){
		this._arrow_left.x = this._arrow_left._org_x + this._arrow_linear* DrillUp.g_SGaC_arrow_float_val ; 	
		this._arrow_left.y = this._arrow_left._org_y ; 
		this._arrow_right.x = this._arrow_right._org_x - this._arrow_linear* DrillUp.g_SGaC_arrow_float_val ; 
		this._arrow_right.y = this._arrow_right._org_y ;
	}
	//上下浮动
	if( DrillUp.g_SGaC_arrow_float_ud ){
		this._arrow_up.x = this._arrow_up._org_x;
		this._arrow_up.y = this._arrow_up._org_y + this._arrow_linear* DrillUp.g_SGaC_arrow_float_val ; 	
		this._arrow_down.x = this._arrow_down._org_x;
		this._arrow_down.y = this._arrow_down._org_y - this._arrow_linear* DrillUp.g_SGaC_arrow_float_val ; 	
	}
	//缩放
...
//=============================================================================
		...
		alert(
			"【Drill_SceneGalleryC.js 面板-全自定义画廊C】\n缺少基础插件，去看看下列插件是不是 未添加 / 被关闭 / 顺序不对："+
