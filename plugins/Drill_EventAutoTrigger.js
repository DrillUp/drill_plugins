 ...
 *   更多相关内容，去看看 "9.物体触发 > 关于玩家接近触发.docx"。
 ...
 *   (2.你可以设计成某种接近后自动触发的开关，可参考 物体管理层示例 的亮片。
...
//=============================================================================
	...
	//==============================
	var DrillUp = DrillUp || {}; 
	DrillUp.g_EAT_PluginTip_curName = "Drill_EventAutoTrigger.js 物体触发-固定区域 & 玩家接近 & 条件触发";
	DrillUp.g_EAT_PluginTip_baseList = ["Drill_CoreOfFixedArea.js 物体触发-固定区域核心"];
	//==============================
	// * 提示信息 - 报错 - 缺少基础插件
	//			
	...
	DrillUp.drill_EAT_getPluginTip_NoBasePlugin = function(){
		...
		var message = "【" + DrillUp.g_EAT_PluginTip_curName + "】\n缺少基础插件，去看看下列插件是不是 未添加 / 被关闭 / 顺序不对：";
		for(var i=0; i < DrillUp.g_EAT_PluginTip_baseList.length; i++){
			message += "\n- ";
			message += DrillUp.g_EAT_PluginTip_baseList[i];
		}
		return message;
	};
	//==============================
	// * 提示信息 - 报错 - 找不到事件
	//==============================
	DrillUp.drill_EAT_getPluginTip_EventNotFind = function( e_id ){
		return "【" + DrillUp.g_EAT_PluginTip_curName + "】\n插件指令错误，当前地图并不存在id为"+e_id+"的事件。";
	};
	//==============================
	// * 提示信息 - 报错 - 重复关键字
	//==============================
	DrillUp.drill_EAT_getPluginTip_keyWord = function( keyword ){
		return "【" + DrillUp.g_EAT_PluginTip_curName + "】\n错误，玩家触发-"+keyword+"的触发关键字，与之前的设置重复，你需要手动修改确保 触发关键字 唯一。";
	};
...
//=============================================================================
	...
	for( var i = 0; i < DrillUp.g_EAT_area_length; i++ ){
		if( DrillUp.parameters["玩家触发-" + String(i+1) ] != undefined &&
			DrillUp.parameters["玩家触发-" + String(i+1) ] != "" ){
	...
	for(var i = 0; i < DrillUp.g_EAT_area.length; i++ ){
		...
		if( DrillUp.g_EAT_area_keyWordList.contains(area['keyword']) ){
			alert( DrillUp.drill_EAT_getPluginTip_keyWord( String(i+1) ) );
			continue;
		}
...
Game_Map.prototype.drill_EAT_isEventExist = function( e_id ){
	...
	if( e == undefined ){
		alert( DrillUp.drill_EAT_getPluginTip_EventNotFind( e_id ) );
		return false;
	}
	return true;
};
...
// * 事件注释 - 第一页标记
//==============================
var _drill_EAT_event_initMembers = Game_Event.prototype.initMembers;
Game_Event.prototype.initMembers = function() {
	_drill_EAT_event_initMembers.call(this);
	this._drill_EAT_isFirstBirth = true;
};
//==============================
// * 事件注释 - 读取绑定
//==============================
...
Game_Event.prototype.setupPage = function() {
    ...
    this.drill_EAT_event_readPage();
};
//==============================
// * 事件注释 - 读取 页
//==============================
Game_Event.prototype.drill_EAT_event_readPage = function() {
	
	// > 第一次出生，强制读取第一页注释（防止离开地图后，回来，开关失效）
	if( !this._erased && this.event() && this.event().pages[0] && this._drill_EAT_isFirstBirth == true ){ 
		this.drill_EAT_event_readList( this.event().pages[0].list );
		this._drill_EAT_isFirstBirth = undefined;		//『节约临时参数存储空间』（放后面，注释通过这个识别"跨事件页/不跨事件页"。"跨事件页"的注释必须放在第一页才能生效。）
	}
	
	// > 读取当前页注释
	if( !this._erased && this.page() ){ 
		this.drill_EAT_event_readList( this.list() );
	}
};
//==============================
// * 事件注释 - 读取 注释
//==============================
Game_Event.prototype.drill_EAT_event_readList = function( pageOfList ) {	
...
//=============================================================================
		...
		var pluginTip = DrillUp.drill_EAT_getPluginTip_NoBasePlugin();
		alert( pluginTip );
}
