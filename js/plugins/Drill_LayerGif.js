...
// * 地图 - 帧刷新 镜头位置
//==============================
var _drill_LGi_Map_updateScroll = Game_Map.prototype.updateScroll;
Game_Map.prototype.updateScroll = function(){
    _drill_LGi_Map_updateScroll.call(this);
...
Scene_Map.prototype.drill_LGi_updateBase = function() {
	...
	for(var i=0; i< sprite_tank.length; i++){
		...
		if( temp_sprite_bitmap['_drill_src_bitmaps'].length != 0 ){
			...
			// > 位移
			temp_sprite.x = temp_data['x'] + temp_data['cameraX'] * (1 - temp_data['XPer']);
			temp_sprite.y = temp_data['y'] + temp_data['cameraY'] * (1 - temp_data['YPer']);
			//（初始位移 + 镜头位移 * 位移比 ）
		}
	}
};
