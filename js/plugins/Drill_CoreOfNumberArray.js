 ...
 * 插件指令：>变量数组核心 : 数组[时间值容器] : 获取值 : 索引[1] : 变量[21]
 * 
 * 插件指令：>变量数组核心 : 数组[4] : 获取值 : 索引[1] : 变量[21]
...
//
//		★必要注意事项：
//			1.这里相对完整地复刻了rmmv变量、开关的程序结构，将变量数组对象化。
//			
//			
//		★其它说明细节：
//			1.刚开始使用的是集群存储法： 
...
Game_Interpreter.prototype.pluginCommand = function(command, args) {
	...
	if( command === ">变量数组核心" ){		//>变量数组核心 : 变量数组[1] : 修改变量数组 : 某\c[2]变量数组
		...
		/*-----------------集群操作 - 删除值------------------*/
			...
			else if( type == "删除值" && temp1.indexOf("索引变量[") != -1 ){	
				temp1 = temp1.replace("索引变量[","");
				temp1 = temp1.replace("]","");
				temp1 = $gameVariables.value( Number(temp1) ) -1;  	//（注意索引-1）
		...
		/*-----------------集群操作 - 添加值------------------*/
			...
			else if( type == "添加值" && temp1.indexOf("变量[") != -1 ){	
		...
		if(args.length == 6){
			...
			if( type == "重置数组" && temp1.indexOf("数组[") != -1 ){	
