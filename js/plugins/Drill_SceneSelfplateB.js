...
//		全局存储变量	DrillUp.global_SSpB_enableTank
//		覆盖重写方法	无
//
//		工作类型		持续执行
//		时间复杂度		o(n^2)*o(场景元素) 每帧
...
//==============================
	...
	if( DrillUp.global_SSpB_enableTank == null ){	//（游戏没关时，不会为null)
		if( _drill_global && _drill_global[0] && _drill_global[0]["_global_Drill_SSpB_enable"] ){		//游戏关闭后，重开读取global中的配置
			DrillUp.global_SSpB_enableTank = _drill_global[0]["_global_Drill_SSpB_enable"];
		}else{
			DrillUp.global_SSpB_enableTank = [];		//（若没有全局存储的值，则初始化）
			for( var i = 1; i <= DrillUp.g_SSpB_context_list_length ; i++ ){
	...
	if( DrillUp.global_SSpB_lockTank == null ){	
		if( _drill_global && _drill_global[0] && _drill_global[0]["_global_Drill_SSpB_lock"] ){
			DrillUp.global_SSpB_lockTank = _drill_global[0]["_global_Drill_SSpB_lock"];
		}else{
			DrillUp.global_SSpB_lockTank = [];			//（若没有全局存储的值，则初始化）
			for( var i = 1; i <= DrillUp.g_SSpB_context_list_length ; i++ ){
				var temp_c = DrillUp.g_SSpB_context_list[i];
				if( temp_c == null ){ continue; }
...
DataManager.saveGlobalInfo = function(info) {	//第0个存档为全局存档
	if(!info[0]){info[0] = []};
	info[0]["_global_Drill_SSpB_enable"] = DrillUp.global_SSpB_enableTank;
	info[0]["_global_Drill_SSpB_lock"] = DrillUp.global_SSpB_lockTank;
...
Game_System.prototype.initialize = function() {	
	...
	this._drill_SSpB_context_list = null;		//旧存档数据
	this._drill_SSpB_enableTank = null;				//显示情况
	this._drill_SSpB_lockTank = null;				//锁定情况
};
//==============================
// * 正常 - 读取 正常存储 的值
//==============================
Game_System.prototype.drill_SSpB_initDataIfNeed = function() {
	if( this._drill_SSpB_enableTank != null ){ return; }
	
	// > 一般初始化
	if( this._drill_SSpB_context_list == null ){
		
		this._drill_SSpB_enableTank = [];
		for( var i = 1; i <= DrillUp.g_SSpB_context_list_length ; i++ ){
			var temp_c = DrillUp.g_SSpB_context_list[i];
			if( temp_c == null ){ continue; }
			this._drill_SSpB_enableTank[i] = temp_c['enabled'];
		}
		this._drill_SSpB_lockTank = [];
		for( var i = 1; i <= DrillUp.g_SSpB_context_list_length ; i++ ){
			var temp_c = DrillUp.g_SSpB_context_list[i];
			if( temp_c == null ){ continue; }
			this._drill_SSpB_lockTank[i] = temp_c['locked'];
		}
	...
	// > 从旧存档中初始化
		...
		this._drill_SSpB_enableTank = [];
		for( var i = 1; i <= DrillUp.g_SSpB_context_list_length ; i++ ){
			var temp_c = this._drill_SSpB_context_list[i];
			if( temp_c == null ){ continue; }
			if( temp_c == "" ){ continue; }
			this._drill_SSpB_enableTank[i] = temp_c['enabled'];
		}
		...
		this._drill_SSpB_context_list = null;		//（强制清除旧插件的冗余存档数据）
	}
};
...
Game_Interpreter.prototype.pluginCommand = function(command, args) {
	...
	if( command === ">信息面板B" ){
		$gameSystem.drill_SSpB_initDataIfNeed();	//（初始化）
		if(args.length == 2){
	...
	if( command === ">信息面板B" ){
		...
		if(args.length == 4){
			...
			if( type == "显示选项" ){
				DrillUp.global_SSpB_enableTank[temp1] = true;			//全局存储
				$gameSystem._drill_SSpB_enableTank[temp1] = true;		//正常存储
				DataManager.forceSaveGlobalInfo();
			}
			if( type == "隐藏选项" ){
				DrillUp.global_SSpB_enableTank[temp1] = false;			//全局存储
				$gameSystem._drill_SSpB_enableTank[temp1] = false;		//正常存储
				DataManager.forceSaveGlobalInfo();
			}
			if( type == "锁定选项" ){
				DrillUp.global_SSpB_lockTank[temp1] = true;				//全局存储
				$gameSystem._drill_SSpB_lockTank[temp1] = true;			//正常存储
				DataManager.forceSaveGlobalInfo();
			}
			if( type == "解锁选项" ){
				DrillUp.global_SSpB_lockTank[temp1] = false;			//全局存储
				$gameSystem._drill_SSpB_lockTank[temp1] = false;		//正常存储
				DataManager.forceSaveGlobalInfo();
			}
			if( type == "选中页" ){
				var temp = temp1 -1;
