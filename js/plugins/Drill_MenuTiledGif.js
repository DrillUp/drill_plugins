...
　　var DrillUp = DrillUp || {}; 
	...
	for (var i = 0; i < DrillUp.g_MTG_list_length; i++) {
		if( DrillUp.parameters["平铺GIF-" + String(i+1) ] != undefined &&
			...
			DrillUp.g_MTG_list[i]['id'] = Number(i)+1;
			DrillUp.g_MTG_list[i]['inited'] = true;
		}else{
			DrillUp.g_MTG_list[i] = DrillUp.drill_MTG_tiledGifInit( {} );
			DrillUp.g_MTG_list[i]['id'] = Number(i)+1;
			DrillUp.g_MTG_list[i]['inited'] = false;
		}
	}
...
Scene_MenuBase.prototype.createBackground = function() {
	...
	_drill_MTG_createBackground.call(this);		//与背景一同创建
};
//==============================
// ** 菜单 - 退出界面
//==============================
...
Scene_MenuBase.prototype.terminate = function() {
	_drill_MTG_terminate.call(this);			//设置需要下次重新创建
	SceneManager._drill_MTG_created = false;
};
//==============================
// ** 菜单 - 层级排序
...
Scene_MenuBase.prototype.update = function() {
	...
	if( SceneManager.isCurrentSceneStarted() && !SceneManager._drill_MTG_created ) {
		this.drill_MTG_create();				//创建，进入界面后只执行一次
	}
	if( SceneManager._drill_MTG_created ){
...
Scene_MenuBase.prototype.drill_MTG_create = function() {	
	...
	if(!this._drill_MTG_spriteTank){
		this._drill_MTG_spriteTank = [];		//防止某些覆写的菜单报错
		this._drill_MTG_dataTank = [];
	}
...
Scene_MenuBase.prototype.drill_MTG_update = function() {
	for(var i = 0; i < this._drill_MTG_spriteTank.length; i++ ){
		var t_gif = this._drill_MTG_spriteTank[i];
		var t_gif_data = this._drill_MTG_dataTank[i];
		
		// > 播放gif
		t_gif['_time'] += 1;
		var inter = this._drill_MTG_spriteTank[i]['_time'] ;
